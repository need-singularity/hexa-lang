...
long test_module();
long test_memory();
long test_concurrency();
long pure_add(long a, long b);
long test_effect();
long test_proof();
long test_meta();
...
static long GLOBAL_C = 30;
static long GLOBAL_S = 40;
static long v;
...
long test_control_flow() {
    ...
    long __fi_i_1 = 0;
    while ((__fi_i_1 < hexa_str_len(hexa_arr_lit((long[]){(long)(1)}, 1)))) {
        long i = (hexa_arr_lit((long[]){(long)(1)}, 1)).d[__fi_i_1];
        printf("%s", "for ");
        __fi_i_1 = (__fi_i_1 + 1);
    }
    long w = 0;
...
long test_variable() {
    ...
    long b = 20;
    b = (b + 1);
    printf("%s %ld %ld\n", "let/mut:", (long)(a), (long)(b));
    return printf("%s %ld %ld\n", "const/static:", (long)(GLOBAL_C), (long)(GLOBAL_S));
}
...
long test_memory() {
    hexa_arr x = hexa_arr_lit((long[]){1, 2, 3}, 3);
    long y = move;
    x;
    printf("%s %ld\n", "move:", (long)(y));
    drop;
...
long test_concurrency() {
    ...
    spawn;
    0;
    return /* unsupported method .send */ 0;
}
...
long test_effect() {
    return printf("%s %ld\n", "@pure:", (long)(pure_add(5, 6)));
}

long test_proof() {
    if (!(((2 + 2) == 4))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    return printf("%s\n", "assert: ok");
}
...
int main(int argc, char** argv) {
    ...
    Drawable;
    long __fi_Point_2 = 0;
    while ((__fi_Point_2 < hexa_str_len(0))) {
        long Point = 0[__fi_Point_2];
        /* unsupported stmt */
        __fi_Point_2 = (__fi_Point_2 + 1);
    }
    async;
    v = /* unsupported method .recv */ 0;
    printf("%s %ld\n", "spawn+channel:", (long)(v));
    0;
    printf("%s\n", "=== 53 Keywords Verification ===");
    test_control_flow();
    test_type_decl();
    test_function();
    test_variable();
