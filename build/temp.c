...
double f_to_c(long f);
double c_to_k(double c);
double k_to_c(long k);
long convert(double value, const char* unit);
...
int main(int argc, char** argv) {
    ...
    if ((hexa_argc() >= 3)) {
        double v = hexa_to_float(hexa_arg(1));
        const char* u = hexa_arg(2);
        long _r = convert(v, u);
    } else {
        printf("%s\n", "=== temperature tests ===");
        long _r1 = convert(0.0, "C");
        long _r2 = convert(100.0, "C");
        long _r3 = convert(32.0, "F");
        long _r4 = convert(212.0, "F");
        long _r5 = convert(273.15, "K");
        long _r6 = convert(0.0, "K");
    }
    printf("%s\n", "done");
    return 0;
}
