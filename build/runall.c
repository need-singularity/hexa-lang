...
int main(int argc, char** argv) {
    ...
    targets = hexa_arr_lit((long[]){(long)("hello_min"), (long)("calc_min"), (long)("fizzbuzz_min"), (long)("factorial_min"), (long)("array_sum"), (long)("sieve_min"), (long)("struct_min"), (long)("float_min"), (long)("forin_min"), (long)("match_min"), (long)("dynarr_min"), (long)("method_min"), (long)("strcat_min"), (long)("strops_min"), (long)("strarr_min"), (long)("split_min"), (long)("strmethods_min"), (long)("mini_transpiler"), (long)("physics")}, 19);
    printf("%s\n", "═══════════════════════════════════════");
    printf("%s\n", "  hexa self-host 전체 실행 러너");
    printf("%s\n", "═══════════════════════════════════════");
    long __fi_name_1 = 0;
    while ((__fi_name_1 < targets.n)) {
        const char* name = ((const char*)targets.d[__fi_name_1]);
        const char* bin = hexa_concat("./build/", name);
        const char* rc_str = hexa_exec(hexa_concat(hexa_concat("test -x ", bin), " && echo yes || echo no"));
        if ((strcmp(rc_str, "yes") == 0)) {
            const char* out = hexa_exec(hexa_concat(bin, " 2>&1 | head -1"));
            printf("%s %s %s %s\n", "✓", name, "→", out);
            passed = (passed + 1);
        } else {
            printf("%s %s %s\n", "✗", name, "(not built)");
        ...
        __fi_name_1 = (__fi_name_1 + 1);
    }
    printf("%s\n", "═══════════════════════════════════════");
    printf("%s %ld %s %ld\n", "  PASS:", (long)(passed), "  FAIL:", (long)(failed));
    printf("%s\n", "═══════════════════════════════════════");
    return 0;
}
