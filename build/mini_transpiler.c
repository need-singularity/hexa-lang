...
const char* emit_println_int(long v);
const char* emit_program();

static const char* c_src;

const char* emit_println_int(long v) {
    return hexa_concat(hexa_concat("    printf(\"%ld\\n\", ", hexa_int_to_str((long)(v))), ");\n");
}

const char* emit_program() {
    const char* out = "#include <stdio.h>\n\nint main(void) {\n";
    hexa_arr xs = hexa_arr_new();
    xs = hexa_arr_push(xs, 1);
    xs = hexa_arr_push(xs, 2);
    xs = hexa_arr_push(xs, 3);
    xs = hexa_arr_push(xs, 6);
    xs = hexa_arr_push(xs, 12);
    long i = 0;
    while ((i < xs.n)) {
        out = hexa_concat(out, emit_println_int(xs.d[i]));
        i = (i + 1);
    }
...
int main(int argc, char** argv) {
    ...
    c_src = emit_program();
    hexa_write_file("build/mini_out.c", c_src);
    printf("%s %ld %s\n", "wrote build/mini_out.c", (long)(hexa_str_len(c_src)), "bytes");
    printf("%s\n", "done");
    return 0;
}
