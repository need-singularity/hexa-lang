...
static const char* bridge_state_path;
static const char* content;
static const char* after_conn;
...
int main(int argc, char** argv) {
    ...
    project = ((const char*)a.d[2]);
    bridge_state_path = hexa_concat(hexa_exec("printenv HOME"), "/Dev/nexus/shared/bridge_state.json");
    content = hexa_read_file(bridge_state_path);
    if ((strcmp(content, "") == 0)) {
        printf("%s\n", "error");
        return 0;
    }
    if ((!hexa_contains(content, "connections"))) {
        printf("%s\n", "no");
        return 0;
    }
    after_conn = ((const char*)(hexa_split(content, "connections")).d[1]);
    if (hexa_contains(after_conn, hexa_concat(hexa_concat("\"", project), "\""))) {
        printf("%s\n", "yes");
    } else {
        printf("%s\n", "no");
    }
