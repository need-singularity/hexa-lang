...
long push(long v);
long pop();
long is_digit_str(const char* s);
long eval_rpn(const char* expr);

static hexa_arr stack;
static long sp = 0;
...
long is_digit_str(const char* s) {
    ...
    const char* c = hexa_substr(s, 0, 1);
    if (((strcmp(c, "0") >= 0) && (strcmp(c, "9") <= 0))) {
        return 1;
    }
    if (((strcmp(c, "-") == 0) && (hexa_str_len(s) > 1))) {
        return 1;
    }
    return 0;
}
...
long eval_rpn(const char* expr) {
    sp = 0;
    hexa_arr tokens = hexa_split(expr, " ");
    long i = 0;
    while ((i < tokens.n)) {
        const char* tok = ((const char*)tokens.d[i]);
        if ((hexa_str_len(tok) > 0)) {
            if ((is_digit_str(tok) == 1)) {
                long _r = push(hexa_parse_int(tok));
            } else {
                if ((strcmp(tok, "+") == 0)) {
                    long b = pop();
                    long a = pop();
    ...
    printf("%s %ld\n", "error: stack not consumed, sp =", (long)(sp));
    return 0;
}
...
int main(int argc, char** argv) {
    ...
    stack = hexa_arr_fill((long)(100), (long)(0));
    if ((hexa_argc() >= 2)) {
        const char* expr = hexa_arg(1);
        printf("%s %s %ld\n", expr, "=", (long)(eval_rpn(expr)));
    } else {
        printf("%s\n", "=== RPN tests ===");
        printf("%s %ld\n", "3 4 +          →", (long)(eval_rpn("3 4 +")));
        printf("%s %ld\n", "3 4 + 2 *      →", (long)(eval_rpn("3 4 + 2 *")));
        printf("%s %ld\n", "5 1 2 + 4 * + 3 - →", (long)(eval_rpn("5 1 2 + 4 * + 3 -")));
        printf("%s %ld\n", "100 5 /        →", (long)(eval_rpn("100 5 /")));
        printf("%s %ld\n", "2 3 * 4 5 * +  →", (long)(eval_rpn("2 3 * 4 5 * +")));
        printf("%s\n", "expected: 7, 14, 14, 20, 26");
    }
    printf("%s\n", "done");
    return 0;
}
