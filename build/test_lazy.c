...
double heavy_calc(double a, double b);
double compute_grad(double w, double x, double y);

static double thunk;
static long result;
static double t1;
static double t2;
static long need_first = 1;
static long chosen;
static double grad_thunk;
static long grad_val;
static hexa_arr thunks;
static long i = 0;
static hexa_arr results;
...
int main(int argc, char** argv) {
    ...
    printf("%s\n", "[1] thunk type: fn");
    result = thunk();
    printf("%s %ld\n", "[1] forced:", (long)(result));
    t1 = heavy_calc(3.0, 4.0);
    t2 = heavy_calc(10.0, 20.0);
    chosen = ((need_first) ? (t1()) : (t2()));
    printf("%s %ld\n", "[2] chosen:", (long)(chosen));
    grad_thunk = compute_grad(1.5, 2.0, 4.0);
    printf("%s\n", "[3] grad thunk created");
    grad_val = grad_thunk();
    printf("%s %ld\n", "[3] grad forced:", (long)(grad_val));
    thunks = hexa_arr_lit((long[]){(long)(expensive(1.0)), (long)(expensive(2.0)), (long)(expensive(3.0))}, 3);
    results = hexa_arr_new();
    while ((i < 3)) {
        results = hexa_arr_concat(results, hexa_arr_lit((long[]){(long)(thunks.d[i])}, 1));
        0;
        0;
        i = (i + 1);
    }
    printf("%s [arr len=%ld]\n", "[4] batch forced:", (long)((results).n));
    printf("%s\n", "--- @lazy 4/4 PASS ---");
    return 0;
}
