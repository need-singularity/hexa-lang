...
int main(int argc, char** argv) {
    ...
    words = hexa_arr_lit((long[]){(long)("apple"), (long)("banana"), (long)("cherry"), (long)("date")}, 4);
    printf("%s %ld\n", "words len =", (long)(words.n));
    while ((i < words.n)) {
        printf("%ld %s %s\n", (long)(i), "->", ((const char*)words.d[i]));
        i = (i + 1);
    }
    while ((j < words.n)) {
        if ((j > 0)) {
            joined = hexa_concat(joined, ", ");
        }
        joined = hexa_concat(joined, ((const char*)words.d[j]));
        j = (j + 1);
    }
    printf("%s %s\n", "joined:", joined);
    printf("%s\n", "done");
