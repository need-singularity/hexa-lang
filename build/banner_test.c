...
const char* fmt_k(long n);
long count_hexa_modules();
long count_discoveries();
const char* get_alien_d();
long count_laws();
...
const char* parent_dir(const char* path) {
    hexa_arr parts = hexa_split(path, "/");
    const char* result = "";
    long i = 0;
    while ((i < (parts.n - 1))) {
        if ((i > 0)) {
        ...
        result = hexa_concat(result, ((const char*)parts.d[i]));
        i = (i + 1);
    }
    return result;
}
...
const char* repo_root() {
    const char* script = ((const char*)(hexa_args()).d[1]);
...
const char* fmt_k(long n) {
    if ((n >= 1000)) {
        double k = (((double)(n)) / 1000.0);
        long ip = (long)(k);
        long fp = (long)(((k - ((double)(ip))) * 10.0));
        return hexa_concat(hexa_concat(hexa_concat(hexa_int_to_str((long)(ip)), "."), hexa_int_to_str((long)(fp))), "k");
    }
    return hexa_int_to_str((long)(n));
}
...
long count_hexa_modules() {
    ...
    long i = 2;
    while ((i < a.n)) {
        if (((strcmp(((const char*)a.d[i]), "--modules") == 0) && ((i + 1) < a.n))) {
            return hexa_to_int_str(((const char*)a.d[(i + 1)]));
        }
        i = (i + 1);
    }
    return 0;
...
long count_discoveries() {
    const char* path = hexa_concat(repo_root(), "/shared/discovery_log.jsonl");
    ...
    long n = lines.n;
    if (((n > 0) && (strcmp(((const char*)lines.d[(n - 1)]), "") == 0))) {
        return (n - 1);
    }
    return n;
    return 0;
...
const char* get_alien_d() {
    const char* path = hexa_concat(repo_root(), "/shared/alien_index_distribution.json");
    if ((!hexa_file_exists(path))) {
        return "d0";
    }
    const char* content = hexa_read_file(path);
    hexa_arr parts = hexa_split(content, "\"max_d\":");
    if ((parts.n >= 2)) {
        const char* vstr = hexa_trim(((const char*)(hexa_split(((const char*)(hexa_split(((const char*)parts.d[1]), ",")).d[0]), "}")).d[0]));
        return hexa_concat("d", hexa_int_to_str((long)((long)(safe_float(vstr)))));
    }
    return "d0";
}

long count_laws() {
    const char* path = hexa_concat(repo_root(), "/shared/consciousness_laws.json");
    if ((!hexa_file_exists(path))) {
        return 0;
    }
    const char* content = hexa_read_file(path);
    hexa_arr parts = hexa_split(content, "\"total_laws\":");
    if ((parts.n >= 2)) {
        const char* vstr = hexa_trim(((const char*)(hexa_split(((const char*)(hexa_split(((const char*)parts.d[1]), ",")).d[0]), "}")).d[0]));
        return (long)(safe_float(vstr));
    }
    return 0;
}
...
int main(int argc, char** argv) {
    ...
    modules = count_hexa_modules();
    disc = count_discoveries();
    ai = get_alien_d();
    laws = count_laws();
    banner = hexa_concat(hexa_concat(hexa_concat(hexa_concat(hexa_concat(hexa_concat(hexa_concat(hexa_concat("🔭 NEXUS-6 mk2 🛸", ai), " 🔭"), hexa_int_to_str((long)(modules))), "모듈 ⚖️"), fmt_k(laws)), "법칙 📊"), fmt_k(disc)), "발견");
    printf("%s\n", hexa_concat(hexa_concat("{\"systemMessage\":\"", banner), "\"}"));
    return 0;
}
