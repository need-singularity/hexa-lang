...
long hanoi(long n, const char* src, const char* dst, const char* via);

static long move_count = 0;
static long n;
static long _;
static long optimal = 1;
static long p = 0;

long hanoi(long n, const char* src, const char* dst, const char* via) {
    if ((n == 1)) {
        move_count = (move_count + 1);
        printf("%s %ld %s %s %s %s\n", "move", (long)(move_count), ":  disk 1  ", src, "→", dst);
        return 0;
    }
    hanoi((n - 1), src, via, dst);
    move_count = (move_count + 1);
    printf("%s %ld %s %ld %s %s %s %s\n", "move", (long)(move_count), ":  disk", (long)(n), " ", src, "→", dst);
...
int main(int argc, char** argv) {
    ...
    n = (((hexa_argc() >= 2)) ? (hexa_parse_int(hexa_arg(1))) : (4));
    printf("%s %ld\n", "Tower of Hanoi: n =", (long)(n));
    printf("%s\n", "─────────────");
    _ = hanoi(n, "A", "C", "B");
    printf("%s\n", "─────────────");
    while ((p < n)) {
    ...
    printf("%s %ld %s %ld %s\n", "total moves:", (long)(move_count), "  (optimal 2^n - 1 =", (long)((optimal - 1)), ")");
    return 0;
}
