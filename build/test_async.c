...
long compute_async(long x);
long add_async(long a, long b);
...
int main(int argc, char** argv) {
    ...
    spawn;
    0;
    /* unsupported method .send */ 0;
    0;
    v1 = /* unsupported method .recv */ 0;
    printf("%s %ld\n", "spawn+channel basic:", (long)(v1));
    if (!((v1 == 42))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    pair2 = channel();
    tx2 = pair2[0];
    rx2 = pair2[1];
    spawn;
    ...
    /* unsupported method .send */ 0;
    0;
    a = /* unsupported method .recv */ 0;
    b = /* unsupported method .recv */ 0;
    c = /* unsupported method .recv */ 0;
    printf("%s %ld %ld %ld\n", "multi-send:", (long)(a), (long)(b), (long)(c));
    if (!((((a + b) + c) == 60))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    pair3 = channel();
    tx3 = pair3[0];
    rx3 = pair3[1];
    spawn;
    ...
    /* unsupported method .send */ 0;
    0;
    total = /* unsupported method .recv */ 0;
    printf("%s %ld\n", "spawn compute sum(1..10):", (long)(total));
    ...
    /* unsupported method .send */ 0;
    0;
    va = /* unsupported method .recv */ 0;
    vb = /* unsupported method .recv */ 0;
    printf("%s %ld %ld\n", "two spawns:", (long)(va), (long)(vb));
    if (!(((va + vb) == 300))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    pair5 = channel();
    tx5 = pair5[0];
    rx5 = pair5[1];
    spawn;
    ...
    /* unsupported method .send */ 0;
    0;
    v5 = /* unsupported method .recv */ 0;
    printf("%s %ld\n", "spawn capture:", (long)(v5));
    if (!((v5 == 42))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    async;
    future = compute_async(6);
    result = await;
    future;
    printf("%s %ld\n", "async fn await:", (long)(result));
    if (!((result == 36))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    async;
    f1 = add_async(10, 20);
    f2 = add_async(30, 40);
    r1 = await;
    ...
    r2 = await;
    f2;
    printf("%s %ld %ld\n", "multiple async:", (long)(r1), (long)(r2));
    if (!((r1 == 30))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    if (!((r2 == 70))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    pair8 = channel();
    tx8 = pair8[0];
    rx8 = pair8[1];
    spawn;
    ...
    /* unsupported method .send */ 0;
    0;
    fib10 = /* unsupported method .recv */ 0;
    printf("%s %ld\n", "spawn fib(10):", (long)(fib10));
    if (!((fib10 == 55))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    printf("%s\n", "");
