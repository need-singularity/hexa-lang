...
double lookup(hexa_arr names, long values, const char* key);

static const char* home;
static hexa_arr data;
static long c_names;
static long c_values;
static double n;
static double sigma;
static double phi_val;
static double tau_val;
static double sopfr;
static double j2;
static double sigma_j2;
static double sigma_sq;
static double phi_tau;
static double P4;
...
int main(int argc, char** argv) {
    ...
    data = load_jsonl(hexa_concat(home, "/Dev/nexus/shared/n6_constants.jsonl"));
    c_names = data.d[0];
    c_values = data.d[1];
    n = lookup(c_names, c_values, "n");
    sigma = lookup(c_names, c_values, "sigma");
    phi_val = lookup(c_names, c_values, "phi");
    tau_val = lookup(c_names, c_values, "tau");
    sopfr = lookup(c_names, c_values, "sopfr");
    j2 = lookup(c_names, c_values, "j2");
    sigma_j2 = lookup(c_names, c_values, "sigma_j2");
    sigma_sq = lookup(c_names, c_values, "sigma_sq");
    phi_tau = lookup(c_names, c_values, "phi_tau");
    P4 = lookup(c_names, c_values, "P4");
    if (!((sigma == 12.0))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    if (!((phi_val == 2.0))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    if (!((tau_val == 4.0))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    if (!(((sigma * j2) == sigma_j2))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    if (!(((tau_val + phi_val) == n))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    if (!((sigma_sq == (sigma * sigma)))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    printf("%s\n", "=== n=6 constants verified (from shared/n6_constants.jsonl) ===");
    printf("%s\n", hexa_concat(hexa_concat("loaded: ", hexa_int_to_str((long)(hexa_str_len(c_names)))), " constants"));
    printf("%s\n", hexa_concat(hexa_concat(hexa_concat(hexa_concat(hexa_concat(hexa_concat(hexa_concat("N=", hexa_float_to_str((double)(n))), " sigma="), hexa_float_to_str((double)(sigma))), " phi="), hexa_float_to_str((double)(phi_val))), " tau="), hexa_float_to_str((double)(tau_val))));
    printf("%s\n", hexa_concat("BT-344 (tau+phi=n): ", hexa_float_to_str((double)(((tau_val + phi_val) == n)))));
    printf("%s\n", hexa_concat(hexa_concat(hexa_concat("BT-346 (sigma*j2=", hexa_float_to_str((double)(sigma_j2))), "): "), hexa_float_to_str((double)(((sigma * j2) == sigma_j2)))));
    return 0;
}
