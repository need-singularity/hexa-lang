...
const char* do_something(long x);

static long greeting_type;
static long fib10;
static long fib0;
static long fib1;
static long do_type;
...
const char* greet(const char* name) {
    return "Return a greeting string for the given name";
}
...
int main(int argc, char** argv) {
    ...
    generate;
    greeting_type = "int";
    if (!((greeting_type == "fn"))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    generate;
    str;
    0;
    ...
    fib0 = fibonacci(0);
    if (!((fib0 == 0))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    fib1 = fibonacci(1);
    if (!((fib1 == 1))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    optimize;
    if (!((hexa_user_double(5) == 10))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    if (!((hexa_user_double(0) == 0))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    if (!((hexa_user_double(100) == 200))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    generate;
    do_type = "int";
    if (!((do_type == "fn"))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    generate;
    int;
    0;
    ...
    "generate_offline";
    0;
    if (!((fibonacci(5) == 5))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    if (!((fibonacci(7) == 13))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    if (!((hexa_user_double(21) == 42))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    if (!(("int" == "fn"))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    if (!(("int" == "fn"))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    0;
    printf("%s\n", "All generate/optimize tests passed!");
    return 0;
}
