...
int main(int argc, char** argv) {
    ...
    fib = hexa_arr_fill((long)((n + 1)), (long)(0));
    if ((n >= 1)) {
        fib.d[1] = 1;
    }
    while ((i <= n)) {
        fib.d[i] = (fib.d[(i - 1)] + fib.d[(i - 2)]);
        i = (i + 1);
    }
    printf("%s\n", "=== Fibonacci (memoized) ===");
    while ((k <= n)) {
        if ((((k <= 20) || (k == n)) || (k == (n - 1)))) {
            printf("%s %ld %s %ld\n", "fib(", (long)(k), ") =", (long)(fib.d[k]));
        }
        if ((k == 21)) {
            printf("%s\n", "...");
        }
    ...
    if ((n >= 10)) {
        long ratio = ((fib.d[n] * 1000) / fib.d[(n - 1)]);
        printf("%s\n", "");
        printf("%s %ld %s\n", "φ ≈ fib(n)/fib(n-1) * 1000 =", (long)(ratio), "(true φ ≈ 1618)");
    }
    printf("%s\n", "done");
    return 0;
}
