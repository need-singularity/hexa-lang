...
long main_diff() {
    ...
    const char* a_path = hexa_arg(1);
    const char* b_path = hexa_arg(2);
    const char* a = hexa_read_file(a_path);
    const char* b = hexa_read_file(b_path);
    hexa_arr a_lines = hexa_split(a, "\n");
    hexa_arr b_lines = hexa_split(b, "\n");
    printf("%s %s %s %s %s\n", "─── diff", a_path, "→", b_path, "───");
    long deleted = 0;
    long i = 0;
    while ((i < a_lines.n)) {
        const char* line_a = ((const char*)a_lines.d[i]);
        if ((hexa_str_len(line_a) > 0)) {
            long found = 0;
            long j = 0;
            while ((j < b_lines.n)) {
                const char* line_b = ((const char*)b_lines.d[j]);
    ...
    long k = 0;
    while ((k < b_lines.n)) {
        const char* line_b = ((const char*)b_lines.d[k]);
        if ((hexa_str_len(line_b) > 0)) {
            long found = 0;
            long m = 0;
            while ((m < a_lines.n)) {
                const char* line_a = ((const char*)a_lines.d[m]);
                if ((strcmp(line_b, line_a) == 0)) {
                    found = 1;
                }
                m = (m + 1);
            }
