...
int main(int argc, char** argv) {
    ...
    n = lines.n;
    seen = hexa_arr_new();
    counts = hexa_arr_fill((long)(10000), (long)(0));
    while ((i < n)) {
        ...
        if ((hexa_str_len(line) > 0)) {
            ...
            long j = 0;
            while ((j < unique_n)) {
                if ((strcmp(((const char*)seen.d[j]), line) == 0)) {
                    found = j;
                }
                j = (j + 1);
            }
            if ((found >= 0)) {
                ...
                seen = hexa_arr_concat(seen, hexa_arr_lit((long[]){(long)(line)}, 1));
                counts.d[unique_n] = 1;
                unique_n = (unique_n + 1);
            }
        }
    ...
    printf("%s %ld %s\n", "─── unique lines (", (long)(unique_n), ") ───");
    while ((k < unique_n)) {
        long cnt = counts.d[k];
        const char* line = ((const char*)seen.d[k]);
        printf("%ld %s %s\n", (long)(cnt), " ", line);
        k = (k + 1);
    }
    printf("%s %ld\n", "total input lines:", (long)(n));
    return 0;
}
