...
static const char* log_path = "/tmp/hexa_log.txt";
static const char* got;
...
int main(int argc, char** argv) {
    ...
    printf("%s %ld\n", "argc =", (long)(a.n));
    while ((i < a.n)) {
        printf("%s %ld %s %s\n", "arg", (long)(i), "=", ((const char*)a.d[i]));
        i = (i + 1);
    }
    date = hexa_trim(hexa_exec("date '+%Y-%m-%d'"));
    printf("%s %s\n", "today =", date);
    ls = hexa_trim(hexa_exec("ls /tmp 2>/dev/null | wc -l"));
    printf("%s %s\n", "/tmp entries =", ls);
    hexa_write_file(log_path, "first line\n");
    hexa_append_file(log_path, "second line\n");
    hexa_append_file(log_path, "third line\n");
    got = hexa_read_file(log_path);
    printf("%s\n", "log content:");
    printf("%s\n", got);
    printf("%s\n", "done");
    return 0;
