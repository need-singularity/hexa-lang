...
long score_password(const char* pw);
const char* strength_label(long s);
long check(const char* pw);


long score_password(const char* pw) {
    long score = 0;
    hexa_arr cs = hexa_chars(pw);
    long n = cs.n;
    if ((n >= 8)) {
        score = (score + 1);
    }
    ...
    long j = 1;
    while ((j < n)) {
        if ((strcmp(hexa_substr(pw, j, (j + 1)), hexa_substr(pw, (j - 1), j)) == 0)) {
...
long check(const char* pw) {
    long s = score_password(pw);
    const char* label = strength_label(s);
    return printf("%s %s %s %ld %s %s %s\n", "'", pw, "' score =", (long)(s), " [", label, "]");
}
