...
static hexa_arr parts;
static const char* after;
static const char* val_part;
...
int main(int argc, char** argv) {
    ...
    field = ((const char*)a.d[2]);
    input = hexa_read_stdin();
    if ((strcmp(input, "") == 0)) {
        return 0;
    }
    key = hexa_concat(hexa_concat("\"", field), "\"");
    if ((!hexa_contains(input, key))) {
        return 0;
    }
    parts = hexa_split(input, key);
    if ((parts.n < 2)) {
        return 0;
    }
    after = hexa_trim(((const char*)parts.d[1]));
    if ((!hexa_starts_with(after, ":"))) {
        return 0;
    }
    val_part = hexa_trim(((const char*)(hexa_split(after, ":")).d[1]));
    if (hexa_starts_with(val_part, "\"")) {
        hexa_arr inner = hexa_split(val_part, "\"");
        if ((inner.n >= 2)) {
            printf("%s\n", ((const char*)inner.d[1]));
        }
    } else {
        const char* clean = val_part;
