...
static const char* test_path = "/tmp/hexa_file_io_test.txt";
static long passed = 0;
static long failed = 0;
static const char* content;
static const char* content2;
static const char* err_result;
...
int main(int argc, char** argv) {
    ...
    hexa_write_file(test_path, "hello hexa");
    printf("%s\n", "Test 1 write_file: OK");
    passed = (passed + 1);
    if (hexa_file_exists(test_path)) {
        printf("%s\n", "Test 2 file_exists(true): OK");
        passed = (passed + 1);
    } else {
        printf("%s\n", "Test 2 file_exists(true): FAIL");
        failed = (failed + 1);
    }
    content = hexa_read_file(test_path);
    if ((strcmp(content, "hello hexa") == 0)) {
        printf("%s\n", "Test 3 read_file: OK");
        passed = (passed + 1);
    } else {
        printf("%s\n", hexa_concat("Test 3 read_file: FAIL — got: ", content));
        failed = (failed + 1);
    }
    hexa_append_file(test_path, "\nworld");
    content2 = hexa_read_file(test_path);
    if ((strcmp(content2, "hello hexa\nworld") == 0)) {
        printf("%s\n", "Test 4 append_file: OK");
        passed = (passed + 1);
    } else {
        printf("%s\n", hexa_concat("Test 4 append_file: FAIL — got: ", content2));
        failed = (failed + 1);
    }
    remove(test_path);
    if ((hexa_file_exists(test_path) == 0)) {
        printf("%s\n", "Test 5 delete_file: OK");
        passed = (passed + 1);
    } else {
        printf("%s\n", "Test 5 delete_file: FAIL — file still exists");
        failed = (failed + 1);
    }
    if ((hexa_file_exists("/tmp/hexa_nonexistent_xyz_abc.txt") == 0)) {
        printf("%s\n", "Test 6 file_exists(false): OK");
        passed = (passed + 1);
    } else {
        printf("%s\n", "Test 6 file_exists(false): FAIL");
        failed = (failed + 1);
    }
    err_result = hexa_read_file("/tmp/hexa_nonexistent_xyz_abc.txt");
    printf("%s\n", hexa_concat("Test 7 read_file error: ", hexa_int_to_str((long)("string"))));
    home = hexa_env("HOME");
    if (("string" == "string")) {
        printf("%s\n", hexa_concat("Test 8 env_var(HOME): OK — ", home));
        passed = (passed + 1);
    } else {
        printf("%s\n", "Test 8 env_var(HOME): FAIL");
        failed = (failed + 1);
    }
    a = hexa_args();
    if (("array" == "array")) {
        printf("%s\n", hexa_concat("Test 9 args(): OK — length ", hexa_int_to_str((long)(a.n))));
        passed = (passed + 1);
    } else {
        printf("%s\n", "Test 9 args(): FAIL");
        failed = (failed + 1);
    ...
    printf("%s\n", "");
    printf("%s\n", hexa_concat(hexa_concat(hexa_concat(hexa_concat("=== File I/O Tests: ", hexa_int_to_str((long)(passed))), " passed, "), hexa_int_to_str((long)(failed))), " failed ==="));
    if ((failed > 0)) {
        exit(1);
    }
    return 0;
