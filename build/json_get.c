...
const char* extract_field(const char* json, const char* key);
...
const char* extract_field(const char* json, const char* key) {
    const char* patterns_a = hexa_concat(hexa_concat("\"", key), "\":\"");
    const char* patterns_b = hexa_concat(hexa_concat("\"", key), "\": \"");
    long p = hexa_index_of(json, patterns_a);
    long skip_len = hexa_str_len(patterns_a);
    if ((p < 0)) {
        p = hexa_index_of(json, patterns_b);
        skip_len = hexa_str_len(patterns_b);
    }
    if ((p >= 0)) {
        long start = (p + skip_len);
    ...
    const char* patterns_c = hexa_concat(hexa_concat("\"", key), "\":");
    long pn = hexa_index_of(json, patterns_c);
    if ((pn < 0)) {
        return "";
    }
    long nstart = (pn + hexa_str_len(patterns_c));
    const char* rest = hexa_trim(hexa_substr(json, nstart, hexa_str_len(json)));
    long comma = hexa_index_of(rest, ",");
    long brace = hexa_index_of(rest, "}");
    long eidx = hexa_str_len(rest);
    ...
    return hexa_trim(hexa_substr(rest, 0, eidx));
}
