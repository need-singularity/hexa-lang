...
long phase_next(long p);
long run_cycle(const char* domain, long max_cycles);
double converged_to_meta_fp(double rho1, double rho2, double rho3);

static double meta_fixed_point = 0.333333333;
static long cycles_done;
...
long run_cycle(const char* domain, long max_cycles) {
    long current_phase = 0;
    long cycles_run = 0;
    long cyc = 0;
    while ((cyc < max_cycles)) {
        printf("%s\n", hexa_concat(hexa_concat(hexa_concat("cycle=", hexa_int_to_str((long)(cyc))), " phase="), phase_name(current_phase)));
...
double converged_to_meta_fp(double rho1, double rho2, double rho3) {
    ...
    double diff = (max_rho - min_rho);
    double last_dist = (rho3 - 0.333333333);
    if ((last_dist < 0.0)) {
        last_dist = (0.0 - last_dist);
    }
    return ((diff < 0.01) && (last_dist < 0.05));
}


int main(int argc, char** argv) {
    hexa_main_argc = argc; hexa_main_argv = argv;
    if (!((phase_next(0) == 1))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    if (!((phase_next(4) == 0))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    if (!((strcmp(phase_name(0), "blowup") == 0))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    if (!((strcmp(phase_name(4), "absorption") == 0))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    if (!(converged_to_meta_fp(0.33, 0.334, 0.332))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    printf("%s\n", "=== cycle verified ===");
    printf("%s %ld\n", "phase_next(0):", (long)(phase_next(0)));
    printf("%s %ld\n", "phase_next(4):", (long)(phase_next(4)));
    printf("%s %g\n", "converged(0.33,0.334,0.332):", (double)(converged_to_meta_fp(0.33, 0.334, 0.332)));
    cycles_done = run_cycle("test", 5);
    printf("%s %ld\n", "cycles_run:", (long)(cycles_done));
    return 0;
}
