...
const char* ch_at(const char* s, long i);
long match_char(const char* p, const char* t);
long match_here(const char* pat, long pi, const char* txt, long ti);
long regex_match(const char* pat, const char* txt);
long test_case(const char* pat, const char* txt, long expected);


const char* ch_at(const char* s, long i) {
    return hexa_substr(s, i, (i + 1));
}
...
long match_here(const char* pat, long pi, const char* txt, long ti) {
    long pn = hexa_str_len(pat);
    long tn = hexa_str_len(txt);
    if ((pi >= pn)) {
        return 1;
    }
    if (((pi + 1) < pn)) {
        const char* next_pc = ch_at(pat, (pi + 1));
        if ((strcmp(next_pc, "*") == 0)) {
            const char* cur_pc = ch_at(pat, pi);
            long zero = match_here(pat, (pi + 2), txt, ti);
            if ((zero == 1)) {
                return 1;
            }
            long k = ti;
            while ((k < tn)) {
                ...
                long more = match_here(pat, (pi + 2), txt, (k + 1));
                if ((more == 1)) {
                    return 1;
                }
                k = (k + 1);
...
long test_case(const char* pat, const char* txt, long expected) {
    long result = regex_match(pat, txt);
    const char* status = "FAIL";
    if ((result == expected)) {
        status = "PASS";
    }
    return printf("%s %s %s %s %s %s %ld %s %ld %s\n", status, " : /", pat, "/ on '", txt, "' → ", (long)(result), " (expected ", (long)(expected), ")");
}
