...
long* alien_new(long d, long r);
long can_promote(long d, long r);
long* breakthrough(long d, long r);
long* record_new(const char* id, long d, long r);
long* record_promote(const char* id, long d, long r);
...
long* alien_new(long d, long r) {
    return hexa_struct_alloc((long[]){(long)(d), (long)(r)}, 2);
}
...
long* breakthrough(long d, long r) {
    if ((r == 10)) {
        return alien_new((d + 1), 0);
    } else {
        return alien_new((-1), (-1));
    }
}

long* record_new(const char* id, long d, long r) {
    return hexa_struct_alloc((long[]){(long)(id), (long)(d), (long)(r), (long)((r == 10)), (long)("")}, 5);
}

long* record_promote(const char* id, long d, long r) {
    long* next = breakthrough(d, r);
    if ((next[0] == (-1))) {
        return record_new(id, d, r);
    } else {
        const char* child_id = hexa_concat(hexa_concat(id, "_d"), hexa_int_to_str((long)(next[0])));
        return record_new(child_id, next[0], next[1]);
    }
}
...
int main(int argc, char** argv) {
    ...
    ai = alien_new(0, 10);
    if (!((/* unknown field d */ 0 == 0))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    if (!((/* unknown field r */ 0 == 10))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    if (!(can_promote(0, 10))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    bt = breakthrough(0, 10);
    if (!((/* unknown field d */ 0 == 1))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    if (!((/* unknown field r */ 0 == 0))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    no_bt = breakthrough(0, 5);
    if (!((/* unknown field d */ 0 == (-1)))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    printf("%s\n", "=== record verified ===");
    printf("%s %ld %s %ld\n", "alien_new(0,10) -> d=", (long)(/* unknown field d */ 0), " r=", (long)(/* unknown field r */ 0));
    printf("%s %ld %s %ld\n", "breakthrough(0,10) -> d=", (long)(/* unknown field d */ 0), " r=", (long)(/* unknown field r */ 0));
    printf("%s %ld\n", "promote:", (long)(can_promote(0, 10)));
    return 0;
}
