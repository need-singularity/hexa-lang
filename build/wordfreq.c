...
long is_word_char(const char* c);
hexa_arr tokenize_words(const char* text);

static const char* path;
static long top_n = 10;
static const char* content;
static hexa_arr words;
static hexa_arr names;
static hexa_arr counts;
static long wi = 0;
static long i = 0;
...
hexa_arr tokenize_words(const char* text) {
    hexa_arr words = hexa_arr_new();
    hexa_arr cs = hexa_chars(text);
    long n = cs.n;
    long i = 0;
    const char* cur = "";
    while ((i < n)) {
        const char* c = hexa_substr(text, i, (i + 1));
        if ((is_word_char(c) == 1)) {
            cur = hexa_concat(cur, c);
        } else {
            if ((hexa_str_len(cur) > 0)) {
                words = hexa_arr_concat(words, hexa_arr_lit((long[]){(long)(hexa_to_lower(cur))}, 1));
                cur = "";
            }
        }
        i = (i + 1);
...
int main(int argc, char** argv) {
    ...
    if ((hexa_argc() < 2)) {
        printf("%s\n", "usage: wordfreq <file> [top_n]");
        exit(1);
    }
    path = hexa_arg(1);
    if ((hexa_argc() >= 3)) {
        top_n = hexa_parse_int(hexa_arg(2));
    }
    ...
    while ((wi < words.n)) {
        ...
        if ((found >= 0)) {
            counts.d[found] = (counts.d[found] + 1);
        } else {
            names = hexa_arr_concat(names, hexa_arr_lit((long[]){(long)(w)}, 1));
    ...
    while ((i < top_n)) {
        if ((i >= counts.n)) {
            ...
            long j = (i + 1);
            while ((j < counts.n)) {
                if ((counts.d[j] > counts.d[max_idx])) {
                    max_idx = j;
                }
                j = (j + 1);
            }
            if ((max_idx != i)) {
                long tmp_c = counts.d[i];
                long tmp_n = names.d[i];
                counts.d[i] = counts.d[max_idx];
                names.d[i] = names.d[max_idx];
                counts.d[max_idx] = tmp_c;
                names.d[max_idx] = tmp_n;
            }
            printf("%ld %s %ld %s %ld %s\n", (long)((i + 1)), ".", (long)(names.d[i]), "(", (long)(counts.d[i]), ")");
            i = (i + 1);
        }
    }
    printf("%s\n", "done");
