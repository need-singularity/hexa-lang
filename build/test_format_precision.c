...
static const char* s11;
static const char* s12;
static const char* s13;
static const char* s14;
...
int main(int argc, char** argv) {
    ...
    printf("%s\n", "=== Format Precision Tests ===");
    s1 = hexa_concat("x=", hexa_int_to_str((long)(42)));
    printf("%s %s\n", "basic int:", s1);
    if (!((strcmp(s1, "x=42") == 0))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    s2 = hexa_concat(hexa_concat(hexa_concat(hexa_concat(hexa_concat("", hexa_int_to_str((long)(1))), " + "), hexa_int_to_str((long)(2))), " = "), hexa_int_to_str((long)(3)));
    printf("%s %s\n", "multi-arg:", s2);
    if (!((strcmp(s2, "1 + 2 = 3") == 0))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    s3 = "{:.2}";
    printf("%s %s\n", "pi 2dp:", s3);
    if (!((strcmp(s3, "3.14") == 0))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    s4 = "{:.6}";
    printf("%s %s\n", "pi 6dp:", s4);
    if (!((strcmp(s4, "3.141593") == 0))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    s5 = "{:.0}";
    printf("%s %s\n", "pi 0dp:", s5);
    if (!((strcmp(s5, "3") == 0))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    s6 = "{:.1}";
    printf("%s %s\n", "2.75 1dp:", s6);
    if (!((strcmp(s6, "2.8") == 0))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    s7 = "{:.2}";
    printf("%s %s\n", "int 42 as 2dp:", s7);
    if (!((strcmp(s7, "42.00") == 0))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    s8 = "{:.3}";
    printf("%s %s\n", "negative 3dp:", s8);
    if (!((strcmp(s8, "-1.235") == 0))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    s9 = "{:.4}";
    printf("%s %s\n", "zero 4dp:", s9);
    if (!((strcmp(s9, "0.0000") == 0))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    s10 = hexa_concat("val={:.2}, count=", hexa_float_to_str((double)(3.14159)));
    printf("%s %s\n", "mixed:", s10);
    if (!((strcmp(s10, "val=3.14, count=10") == 0))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    s11 = hexa_concat("hello ", "world");
    printf("%s %s\n", "string arg:", s11);
    if (!((strcmp(s11, "hello world") == 0))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    s12 = "{:.2}";
    printf("%s %s\n", "large float:", s12);
    if (!((strcmp(s12, "123456.79") == 0))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    s13 = "{:.6}";
    printf("%s %s\n", "tiny float:", s13);
    if (!((strcmp(s13, "0.000001") == 0))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    s14 = "{:.1} and {:.3}";
    printf("%s %s\n", "multi-prec:", s14);
    if (!((strcmp(s14, "1.0 and 2.000") == 0))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    printf("%s\n", "--- loop format ---");
    for (long i = 0; i < 5; i++) {
    ...
    e_str = "{:.4}";
    printf("%s %s\n", "E 4dp:", e_str);
    if (!((strcmp(e_str, "2.7183") == 0))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    pi_str = "{:.10}";
    printf("%s %s\n", "PI 10dp:", pi_str);
    printf("%s\n", "");
    printf("%s\n", "=== All format precision tests passed! ===");
    return 0;
}
