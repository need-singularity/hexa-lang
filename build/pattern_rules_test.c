...
hexa_arr load_recent_entries();
long detect_high_freq_constants(hexa_arr entries);

static const char* version = "1.0.0";
static long max_entries = 500;
static long freq_threshold = 3;
...
const char* extract_field(const char* line, const char* field) {
    const char* key = hexa_concat(hexa_concat("\"", field), "\":\"");
    if ((!hexa_contains(line, key))) {
        return "";
    }
    hexa_arr parts = hexa_split(line, key);
    if ((parts.n < 2)) {
        return "";
    }
    const char* val = ((const char*)(hexa_split(((const char*)parts.d[1]), "\"")).d[0]);
    return val;
}
...
hexa_arr load_recent_entries() {
    ...
    while ((__fi_line_1 < lines.n)) {
        ...
        if (((strcmp(hexa_trim(line), "") != 0) && hexa_contains(line, "\"constant\""))) {
            entries = hexa_arr_concat(entries, hexa_arr_lit((long[]){(long)(hexa_trim(line))}, 1));
        }
...
long detect_high_freq_constants(hexa_arr entries) {
    ...
    hexa_arr counts = hexa_arr_new();
    long __fi_entry_2 = 0;
    while ((__fi_entry_2 < entries.n)) {
        const char* entry = ((const char*)entries.d[__fi_entry_2]);
        const char* c = extract_field(entry, "constant");
        if ((strcmp(c, "") == 0)) {
            continue;
        }
        long found = 0;
        ...
        if ((!found)) {
            names = hexa_arr_concat(names, hexa_arr_lit((long[]){(long)(c)}, 1));
            counts = hexa_arr_concat(counts, hexa_arr_lit((long[]){(long)(1)}, 1));
        }
    ...
    long i = 0;
    while ((i < names.n)) {
        if ((counts.d[i] >= freq_threshold)) {
            double conf = (((counts.d[i] >= 20)) ? (0.95) : (0));
            i = (i + 1);
        }
        results;
    }
...
int main(int argc, char** argv) {
    ...
    shared_dir = hexa_concat(home, "/Dev/nexus/shared");
    log_path = hexa_concat(shared_dir, "/discovery_log.jsonl");
    rules_path = hexa_concat(shared_dir, "/pattern_rules.jsonl");
    return 0;
}
