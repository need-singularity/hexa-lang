...
static long __memo_ack_keys[4096 * 2];
static long __memo_ack_vals[4096];
static char __memo_ack_valid[4096];

long __memo_orig_ack(long m, long n) {
    if ((m == 0)) {
        return (n + 1);
    }
    if ((n == 0)) {
...
long ack(long m, long n) {
    ...
    __h = (__h ^ (unsigned long)(m)) * 1099511628211UL;
    __h = (__h ^ (unsigned long)(n)) * 1099511628211UL;
    long __idx = (long)(__h & 4095);
    long __base = __idx * 2;
    if (__memo_ack_valid[__idx] && __memo_ack_keys[__base + 0] == m && __memo_ack_keys[__base + 1] == n) {
        return __memo_ack_vals[__idx];
    }
    long __r = __memo_orig_ack(m, n);
    __memo_ack_keys[__base + 0] = m;
    __memo_ack_keys[__base + 1] = n;
    __memo_ack_vals[__idx] = __r;
    __memo_ack_valid[__idx] = 1;
    return __r;
}
