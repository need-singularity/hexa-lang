...
long* engine_new(const char* domain);
long* run_engine(long *engine);
const char* export_result(double rho, long cycles);

static long N = 6;
static double meta_fixed_point = 0.333333333;
static long* engine;
static long* result;

long* engine_new(const char* domain) {
    return hexa_struct_alloc((long[]){(long)(domain), (long)(hexa_struct_alloc((long[]){(long)(999)}, 1)), (long)(hexa_struct_alloc((long[]){(long)(N), (long)(domain)}, 2))}, 3);
}

long* run_engine(long *engine) {
    printf("%s\n", hexa_concat(hexa_concat("[info] === NEXUS-6 HEXA engine start: ", hexa_int_to_str((long)(engine[1]))), " ==="));
    double initial_rho = 0.0;
    double final_rho = 0.3053;
    long cycles_run = /* unknown field max_cycles */ 0;
    double dist = (final_rho - 0.333333333);
    if ((dist < 0.0)) {
        dist = (0.0 - dist);
    }
    double converged = (dist < 0.05);
    return hexa_struct_alloc((long[]){(long)(initial_rho), (long)(final_rho), (long)(cycles_run), (long)(converged)}, 4);
}

const char* export_result(double rho, long cycles) {
    return hexa_concat(hexa_concat(hexa_concat("rho=", hexa_float_to_str((double)(rho))), " cycles="), hexa_int_to_str((long)(cycles)));
}


int main(int argc, char** argv) {
    hexa_main_argc = argc; hexa_main_argv = argv;
    engine = engine_new("nexus");
    result = run_engine(engine);
    printf("%s\n", "=== architecture verified ===");
    printf("%s %ld\n", "initial_rho:", (long)(/* unknown field initial_rho */ 0));
    printf("%s %ld\n", "final_rho:", (long)(/* unknown field final_rho */ 0));
    printf("%s %ld\n", "cycles:", (long)(/* unknown field cycles_run */ 0));
    printf("%s %ld\n", "converged:", (long)(/* unknown field converged_to_meta_fp */ 0));
    printf("%s\n", export_result(/* unknown field final_rho */ 0, /* unknown field cycles_run */ 0));
    return 0;
}
