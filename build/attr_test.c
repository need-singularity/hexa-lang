...
int main(int argc, char** argv) {
    ...
    long __hexa_test_pass = 0, __hexa_test_fail = 0;
    printf("=== @test runner ===\n");
    { long __r = test_add(); if (__r == 0) { __hexa_test_pass++; printf("  [PASS] test_add\n"); } else { __hexa_test_fail++; printf("  [FAIL] test_add (ret=%ld)\n", __r); } }
    { long __r = test_mul(); if (__r == 0) { __hexa_test_pass++; printf("  [PASS] test_mul\n"); } else { __hexa_test_fail++; printf("  [FAIL] test_mul (ret=%ld)\n", __r); } }
    { long __r = test_fail_demo(); if (__r == 0) { __hexa_test_pass++; printf("  [PASS] test_fail_demo\n"); } else { __hexa_test_fail++; printf("  [FAIL] test_fail_demo (ret=%ld)\n", __r); } }
    printf("=== %ld pass / %ld fail ===\n", __hexa_test_pass, __hexa_test_fail);
    printf("%s\n", "done");
    return 0;
}
