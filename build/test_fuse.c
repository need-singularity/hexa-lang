...
double double_add1(double x);
double weight_update(double pre, double post, double lr, double decay);
double normalize_reward(double r, double mean, double std);

static double r1;
static double r2;
static hexa_arr pre;
static hexa_arr post;
static double updates;
static hexa_arr rewards;
static double normed;
static hexa_arr big;
static long i = 0;
static double fused;
...
int main(int argc, char** argv) {
    ...
    r1 = double_add1(hexa_arr_lit((long[]){(long)(1.0), (long)(2.0), (long)(3.0), (long)(4.0)}, 4));
    printf("%s %g\n", "[1] double_add1([1,2,3,4]) =", (double)(r1));
    r2 = double_add1(5.0);
    printf("%s %g\n", "[2] double_add1(5.0) =", (double)(r2));
    ...
    updates = weight_update(pre, post, 0.01, 0.99);
    printf("%s %g\n", "[3] weight_update =", (double)(updates));
    rewards = hexa_arr_lit((long[]){(long)(0.79), (long)(0.7), (long)(0.19)}, 3);
    normed = normalize_reward(rewards, 0.56, 0.27);
    printf("%s %g\n", "[4] normalized =", (double)(normed));
    big = hexa_arr_new();
    while ((i < 10000)) {
        big = hexa_arr_concat(big, hexa_arr_lit((long[]){(long)((i * 1.0))}, 1));
        i = (i + 1);
    }
    fused = double_add1(big);
    printf("%s %ld %ld %ld\n", "[5] fused(10K), first 3:", (long)(fused[0]), (long)(fused[1]), (long)(fused[2]));
    printf("%s %ld\n", "    last:", (long)(fused[9999]));
    printf("%s\n", "--- @fuse 5/5 PASS ---");
    return 0;
}
