...
static long n;
static long i = 0;
static hexa_arr words;
static hexa_arr key_arr;
static hexa_arr cnt_arr;
static long unique_n = 0;
static long wi = 0;
static long top_n;
static long ti = 0;
static long pi = 0;
...
int main(int argc, char** argv) {
    ...
    content = hexa_read_file(hexa_arg(1));
    if ((hexa_str_len(content) == 0)) {
        printf("%s\n", "(empty)");
        exit(0);
    }
    cs = hexa_chars(content);
    n = cs.n;
    while ((i < n)) {
        long c = cs.d[i];
        long is_alnum = 0;
        if ((((c >= 97) && (c <= 122)) || ((c >= 48) && (c <= 57)))) {
            normalized = hexa_concat(normalized, hexa_substr(content, i, (i + 1)));
        } else {
            if (((c >= 65) && (c <= 90))) {
                normalized = hexa_concat(normalized, hexa_to_lower(hexa_substr(content, i, (i + 1))));
            } else {
                normalized = hexa_concat(normalized, " ");
            }
        }
        i = (i + 1);
    }
    words = hexa_split(normalized, " ");
    key_arr = hexa_arr_lit((long[]){(long)("__init__")}, 1);
    cnt_arr = hexa_arr_fill((long)(5000), (long)(0));
    cnt_arr.d[0] = 0;
    while ((wi < words.n)) {
        const char* w = ((const char*)words.d[wi]);
        if ((hexa_str_len(w) > 0)) {
            long found = (-1);
            long ki = 0;
            while ((ki < unique_n)) {
                if ((strcmp(((const char*)key_arr.d[ki]), w) == 0)) {
                    found = ki;
                }
                ki = (ki + 1);
            }
            if ((found >= 0)) {
                cnt_arr.d[found] = (cnt_arr.d[found] + 1);
            } else {
                key_arr = hexa_arr_concat(key_arr, hexa_arr_lit((long[]){(long)(w)}, 1));
                cnt_arr.d[unique_n] = 1;
                unique_n = (unique_n + 1);
            }
        }
        wi = (wi + 1);
    }
    top_n = (((unique_n < 10)) ? (unique_n) : (10));
    while ((ti < top_n)) {
        long max_idx = ti;
        long tj = (ti + 1);
        while ((tj < unique_n)) {
            if ((cnt_arr.d[tj] > cnt_arr.d[max_idx])) {
                max_idx = tj;
            }
            tj = (tj + 1);
        }
        if ((max_idx != ti)) {
            long tc = cnt_arr.d[ti];
            cnt_arr.d[ti] = cnt_arr.d[max_idx];
            cnt_arr.d[max_idx] = tc;
            const char* tk = ((const char*)key_arr.d[ti]);
            key_arr.d[ti] = (long)(((const char*)key_arr.d[max_idx]));
            key_arr.d[max_idx] = (long)(tk);
        }
        ti = (ti + 1);
    }
    printf("%s %ld %s\n", "=== top", (long)(top_n), "words ===");
    while ((pi < top_n)) {
        printf("%ld %s %s %s %ld %s\n", (long)((pi + 1)), ".", ((const char*)key_arr.d[pi]), "(", (long)(cnt_arr.d[pi]), ")");
        pi = (pi + 1);
    }
    printf("%s %ld\n", "total unique:", (long)(unique_n));
    printf("%s\n", "done");
    return 0;
}
