...
long skip_ws() {
    while ((pos < hexa_str_len(src))) {
        const char* c = hexa_substr(src, pos, (pos + 1));
...
long parse_factor() {
    ...
    if ((strcmp(c, "(") == 0)) {
        ...
        skip_ws();
        if ((pos < hexa_str_len(src))) {
            if ((strcmp(hexa_substr(src, pos, (pos + 1)), ")") == 0)) {
                pos = (pos + 1);
            }
        }
        return v;
