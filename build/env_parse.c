...
int main(int argc, char** argv) {
    ...
    while ((i < lines.n)) {
        ...
        const char* clean_val = val;
        if ((hexa_str_len(clean_val) >= 2)) {
            const char* first = hexa_substr(clean_val, 0, 1);
            const char* last = hexa_substr(clean_val, (hexa_str_len(clean_val) - 1), hexa_str_len(clean_val));
            if ((strcmp(first, "\"") == 0)) {
                if ((strcmp(last, "\"") == 0)) {
                    clean_val = hexa_substr(clean_val, 1, (hexa_str_len(clean_val) - 1));
                }
            }
        }
        printf("%ld %s %s %s %s\n", (long)((count + 1)), ".", key, "=", clean_val);
        count = (count + 1);
    }
    printf("%s\n", "─── summary ───");
    printf("%s %ld\n", "entries:", (long)(count));
