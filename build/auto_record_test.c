...
int main(int argc, char** argv) {
    ...
    value = ((const char*)a.d[2]);
    constant = ((const char*)a.d[3]);
    source = ((const char*)a.d[4]);
    timestamp = hexa_trim(hexa_exec("date '+%Y-%m-%d %H:%M:%S'"));
    log_path = hexa_concat(hexa_exec("printenv HOME"), "/Dev/nexus/shared/discovery_log.jsonl");
    entry = hexa_concat(hexa_concat(hexa_concat(hexa_concat(hexa_concat(hexa_concat(hexa_concat(hexa_concat("{\"timestamp\":\"", timestamp), "\",\"value\":\""), value), "\",\"constant\":\""), constant), "\",\"source\":\""), source), "\",\"auto\":true}");
    hexa_append_file(log_path, hexa_concat(entry, "\n"));
    return 0;
}
