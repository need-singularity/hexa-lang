...
static hexa_arr history;
static long i = 0;

long bump() {
    counter = (counter + 1);
    history = hexa_arr_concat(history, hexa_arr_lit((long[]){(long)(counter)}, 1));
}


int main(int argc, char** argv) {
    hexa_main_argc = argc; hexa_main_argv = argv;
    history = hexa_arr_new();
    bump();
    bump();
    bump();
    bump();
    ...
    printf("%s %ld\n", "history len =", (long)(history.n));
    while ((i < history.n)) {
        printf("%s %ld %s %ld\n", "history[", (long)(i), "] =", (long)(history.d[i]));
        i = (i + 1);
    }
    printf("%s\n", "done");
    return 0;
