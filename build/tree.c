...
long count_slashes(const char* s);
const char* last_component(const char* s);

static const char* dir;
static long max_depth = 3;
static long base_slashes;
...
long count_slashes(const char* s) {
    long n = 0;
    long i = 0;
    hexa_arr cs = hexa_chars(s);
    while ((i < cs.n)) {
        if ((strcmp(hexa_substr(s, i, (i + 1)), "/") == 0)) {
            n = (n + 1);
        }
        i = (i + 1);
    }
...
const char* last_component(const char* s) {
    long i = (hexa_str_len(s) - 1);
    while ((i >= 0)) {
        if ((strcmp(hexa_substr(s, i, (i + 1)), "/") == 0)) {
            return hexa_substr(s, (i + 1), hexa_str_len(s));
        }
        i = (i - 1);
    }
    return s;
...
int main(int argc, char** argv) {
    ...
    if ((hexa_argc() < 2)) {
        printf("%s\n", "usage: tree <dir> [max_depth]");
        exit(1);
    }
    dir = hexa_arg(1);
    if ((hexa_argc() >= 3)) {
        max_depth = hexa_parse_int(hexa_arg(2));
    }
    base_slashes = count_slashes(dir);
    cmd = hexa_concat(hexa_concat(hexa_concat(hexa_concat("find ", dir), " -maxdepth "), hexa_int_to_str((long)(max_depth))), " 2>/dev/null | sort");
    output = hexa_exec(cmd);
    lines = hexa_split(output, "\n");
    long __fi_line_1 = 0;
    while ((__fi_line_1 < lines.n)) {
        ...
        long depth = (count_slashes(line) - base_slashes);
        const char* name = last_component(line);
        const char* indent = "";
        long d = 0;
        while ((d < depth)) {
            indent = hexa_concat(indent, "  ");
            d = (d + 1);
        }
        const char* is_dir = hexa_exec(hexa_concat(hexa_concat("test -d '", line), "' && echo d || echo f"));
        if ((strcmp(is_dir, "d") == 0)) {
            printf("%s\n", hexa_concat(hexa_concat(hexa_concat(indent, "📁 "), name), "/"));
            dirs = (dirs + 1);
        } else {
            printf("%s\n", hexa_concat(hexa_concat(indent, "  "), name));
            files = (files + 1);
        }
        __fi_line_1 = (__fi_line_1 + 1);
    }
    printf("%s\n", "─────");
    printf("%s %ld %s %ld\n", "dirs:", (long)(dirs), " files:", (long)(files));
    return 0;
}
