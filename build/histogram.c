...
int main(int argc, char** argv) {
    ...
    avg = (sum / total);
    printf("%s\n", "=== histogram ===");
    printf("%s %ld %s %ld %s %ld %s %ld %s %ld\n", "n:", (long)(total), "  min:", (long)(lo), "  max:", (long)(hi), "  avg:", (long)(avg), "  sum:", (long)(sum));
    range = (hi - lo);
    if ((range == 0)) {
        printf("%s %ld\n", "all values =", (long)(lo));
        exit(0);
    ...
    bucket_w = ((range / 10) + 1);
    buckets = hexa_arr_fill((long)(10), (long)(0));
    while ((j < total)) {
        long v = nums.d[j];
        long bi = ((v - lo) / bucket_w);
        if ((bi >= 10)) {
            buckets.d[9] = (buckets.d[9] + 1);
        } else {
            buckets.d[bi] = (buckets.d[bi] + 1);
        }
        j = (j + 1);
    }
    while ((b < 10)) {
        if ((buckets.d[b] > max_cnt)) {
            max_cnt = buckets.d[b];
        }
        b = (b + 1);
    }
    printf("%s\n", "");
    while ((bi < 10)) {
        long cnt = buckets.d[bi];
        long low = (lo + (bi * bucket_w));
        long high = ((low + bucket_w) - 1);
        long bar_len = (((max_cnt > 0)) ? (((cnt * 40) / max_cnt)) : (0));
        const char* bar = "";
        long bl = 0;
        while ((bl < bar_len)) {
            bar = hexa_concat(bar, "█");
        ...
        printf("%ld %s %ld %s %s %ld\n", (long)(low), "-", (long)(high), ":", bar, (long)(cnt));
        bi = (bi + 1);
    }
    printf("%s\n", "done");
    return 0;
