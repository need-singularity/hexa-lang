...
long find_acc(const char* name);
long deposit(const char* name, long amount);
long withdraw(const char* name, long amount);
long transfer(const char* from_n, const char* to_n, long amount);
long show_all();

static hexa_arr acc_names;
static hexa_arr acc_balances;
static long _t1;
static long _t2;
static long _t3;
static long _t4;
...
long find_acc(const char* name) {
    long i = 0;
    while ((i < acc_names.n)) {
        if ((strcmp(((const char*)acc_names.d[i]), name) == 0)) {
            return i;
        }
        i = (i + 1);
    }
...
long deposit(const char* name, long amount) {
    long i = find_acc(name);
    if ((i < 0)) {
        printf("%s %s\n", "error: account not found:", name);
        return (-1);
    }
    acc_balances.d[i] = (acc_balances.d[i] + amount);
    printf("%s %ld %s %s %s %ld %s\n", "deposit", (long)(amount), "→", name, "(new balance:", (long)(acc_balances.d[i]), ")");
    return acc_balances.d[i];
}
...
long withdraw(const char* name, long amount) {
    ...
    if ((acc_balances.d[i] < amount)) {
        printf("%s %s %s %ld %s %ld %s\n", "error: insufficient funds for", name, "(have:", (long)(acc_balances.d[i]), ", need:", (long)(amount), ")");
        return (-1);
    }
    acc_balances.d[i] = (acc_balances.d[i] - amount);
    printf("%s %ld %s %s %s %ld %s\n", "withdraw", (long)(amount), "←", name, "(new balance:", (long)(acc_balances.d[i]), ")");
    return acc_balances.d[i];
}

long transfer(const char* from_n, const char* to_n, long amount) {
    long _w = withdraw(from_n, amount);
    if ((_w < 0)) {
        return (-1);
    }
    long _d = deposit(to_n, amount);
    printf("%s %ld %s %s %s %s\n", "  ↳ transferred", (long)(amount), "from", from_n, "to", to_n);
    return 0;
}
...
long show_all() {
    ...
    long i = 0;
    while ((i < acc_names.n)) {
        printf("%s %s %s %ld\n", " ", ((const char*)acc_names.d[i]), ":", (long)(acc_balances.d[i]));
        total = (total + acc_balances.d[i]);
        i = (i + 1);
    }
    return printf("%s %ld\n", "  ── total:", (long)(total));
}
...
int main(int argc, char** argv) {
    ...
    acc_names = hexa_arr_lit((long[]){(long)("alice"), (long)("bob"), (long)("charlie")}, 3);
    acc_balances = hexa_arr_lit((long[]){(long)(1000), (long)(500), (long)(0)}, 3);
    show_all();
    printf("%s\n", "");
    printf("%s\n", "=== transactions ===");
    _t1 = deposit("bob", 200);
    _t2 = withdraw("alice", 300);
    _t3 = transfer("alice", "charlie", 400);
    _t4 = withdraw("charlie", 1000);
    _t5 = transfer("bob", "alice", 50);
    printf("%s\n", "");
    show_all();
    printf("%s\n", "done");
    return 0;
