...
int main(int argc, char** argv) {
    ...
    result = hexa_exec("echo hello");
    printf("%s\n", result);
    if (!((strcmp(result, "hello") == 0))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    info = hexa_exec_with_status("echo world");
    printf("%s\n", info);
    text = hexa_exec("printf abc");
    if (!((strcmp(text, "abc") == 0))) { fprintf(stderr, "assertion failed\n"); exit(1); }
