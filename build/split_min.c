...
static const char* csv = "apple,banana,cherry,date,elderberry";
...
int main(int argc, char** argv) {
    ...
    parts = hexa_split(csv, ",");
    printf("%s %ld\n", "parts len =", (long)(parts.n));
    while ((i < parts.n)) {
        printf("%ld %s %s\n", (long)(i), ":", ((const char*)parts.d[i]));
        i = (i + 1);
    }
    lines = hexa_split(text, "\n");
    printf("%s %ld\n", "lines =", (long)(lines.n));
    while ((j < lines.n)) {
        printf("%s %ld %s %s\n", "  [", (long)(j), "]", ((const char*)lines.d[j]));
        j = (j + 1);
    }
    printf("%s\n", "done");
    return 0;
