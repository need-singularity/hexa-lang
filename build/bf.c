...
long bf_run(const char* code);


long bf_run(const char* code) {
    hexa_arr tape = hexa_arr_new();
    long t = 0;
    while ((t < 30000)) {
        tape = hexa_arr_concat(tape, hexa_arr_lit((long[]){(long)(0)}, 1));
        t = (t + 1);
    }
    long ptr = 0;
    long pc = 0;
    long n = hexa_str_len(code);
    while ((pc < n)) {
        const char* c = hexa_substr(code, pc, (pc + 1));
        if ((strcmp(c, ">") == 0)) {
            ptr = (ptr + 1);
        } else {
            if ((strcmp(c, "<") == 0)) {
                ...
                if ((strcmp(c, "+") == 0)) {
                    tape.d[ptr] = (tape.d[ptr] + 1);
                } else {
                    if ((strcmp(c, "-") == 0)) {
                        tape.d[ptr] = (tape.d[ptr] - 1);
                    } else {
                        if ((strcmp(c, ".") == 0)) {
                            long ch_code = tape.d[ptr];
                            printf("%ld", (long)(ch_code));
                            printf("%s", " ");
                        } else {
                            if ((strcmp(c, "[") == 0)) {
                                if ((tape.d[ptr] == 0)) {
                                    ...
                                    long scan = (pc + 1);
                                    while ((scan < n)) {
                                        const char* c2 = hexa_substr(code, scan, (scan + 1));
                                        if ((strcmp(c2, "[") == 0)) {
                                            depth = (depth + 1);
                                        } else {
                                            if ((strcmp(c2, "]") == 0)) {
