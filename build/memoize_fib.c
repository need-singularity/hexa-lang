...
static long __memo_fib_keys[4096 * 1];
static long __memo_fib_vals[4096];
static char __memo_fib_valid[4096];

long __memo_orig_fib(long n) {
    if ((n <= 1)) {
        return n;
    }
...
long fib(long n) {
    ...
    long __base = __idx * 1;
    if (__memo_fib_valid[__idx] && __memo_fib_keys[__base + 0] == n) {
        return __memo_fib_vals[__idx];
    }
    long __r = __memo_orig_fib(n);
    __memo_fib_keys[__base + 0] = n;
    __memo_fib_vals[__idx] = __r;
    __memo_fib_valid[__idx] = 1;
    return __r;
}
