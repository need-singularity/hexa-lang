...
long zeller(long y, long m, long d);
const char* month_name(long m);

static long year = 2026;
static long month = 4;
static long dim;
static long h;
static long start_col;
...
long zeller(long y, long m, long d) {
    ...
    long k = (yy % 100);
    long j = (yy / 100);
    long h = ((((((d + ((13 * (mm + 1)) / 5)) + k) + (k / 4)) + (j / 4)) + (5 * j)) % 7);
    return h;
}
...
int main(int argc, char** argv) {
    ...
    if ((hexa_argc() >= 3)) {
        year = hexa_parse_int(hexa_arg(1));
        month = hexa_parse_int(hexa_arg(2));
    }
    dim = days_in_month(year, month);
    printf("%s %s %ld %s %ld %s %s %s\n", "=====", month_name(month), (long)(year), "(", (long)(dim), "days,", (((is_leap(year) == 1)) ? ("leap") : ("common")), ") =====");
    ...
    while ((day <= dim)) {
        const char* pad = (((day < 10)) ? (" ") : (""));
        line = hexa_concat(hexa_concat(hexa_concat(line, pad), hexa_int_to_str((long)(day))), " ");
        col = (col + 1);
        if ((col == 7)) {
            printf("%s\n", line);
            line = "";
