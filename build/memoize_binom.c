...
static long __memo_binom_keys[4096 * 2];
static long __memo_binom_vals[4096];
static char __memo_binom_valid[4096];

long __memo_orig_binom(long n, long k) {
    if ((k == 0)) {
        return 1;
    }
    if ((k == n)) {
...
long binom(long n, long k) {
    unsigned long __h = 1469598103934665603UL;
    __h = (__h ^ (unsigned long)(n)) * 1099511628211UL;
    __h = (__h ^ (unsigned long)(k)) * 1099511628211UL;
    long __idx = (long)(__h & 4095);
    long __base = __idx * 2;
    if (__memo_binom_valid[__idx] && __memo_binom_keys[__base + 0] == n && __memo_binom_keys[__base + 1] == k) {
        return __memo_binom_vals[__idx];
    }
    long __r = __memo_orig_binom(n, k);
    __memo_binom_keys[__base + 0] = n;
    __memo_binom_keys[__base + 1] = k;
    __memo_binom_vals[__idx] = __r;
    __memo_binom_valid[__idx] = 1;
    return __r;
}
