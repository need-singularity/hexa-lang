...
static long sum = 0;
static long k = 0;
static long mean_x100;
static hexa_arr sorted_arr;
static long ci = 0;
static long bi = 0;
static long median;
static long var_sum = 0;
static long vi = 0;
static long variance_x10000;
static long stddev_x100 = 0;
static long s;
...
int main(int argc, char** argv) {
    ...
    n = cs.n;
    while ((i < n)) {
        const char* c = hexa_substr(raw, i, (i + 1));
        if ((((((strcmp(c, " ") == 0) || (strcmp(c, "\t") == 0)) || (strcmp(c, "\n") == 0)) || (strcmp(c, "\r") == 0)) || (strcmp(c, ",") == 0))) {
            if ((hexa_str_len(cur) > 0)) {
                nums = hexa_arr_concat(nums, hexa_arr_lit((long[]){(long)(hexa_parse_int(cur))}, 1));
                cur = "";
            }
        } else {
    ...
    mean_x100 = ((sum * 100) / total);
    sorted_arr = hexa_arr_fill((long)(total), (long)(0));
    while ((ci < total)) {
        sorted_arr.d[ci] = nums.d[ci];
        ci = (ci + 1);
    }
    while ((bi < total)) {
        long bj = 0;
        while ((bj < ((total - bi) - 1))) {
            if ((sorted_arr.d[bj] > sorted_arr.d[(bj + 1)])) {
                long t = sorted_arr.d[bj];
                sorted_arr.d[bj] = sorted_arr.d[(bj + 1)];
                sorted_arr.d[(bj + 1)] = t;
            }
            bj = (bj + 1);
        }
    ...
    median = sorted_arr.d[(total / 2)];
    while ((vi < total)) {
        long diff = ((nums.d[vi] * 100) - mean_x100);
        var_sum = (var_sum + (diff * diff));
        vi = (vi + 1);
    }
    variance_x10000 = (var_sum / total);
    s = (variance_x10000 / 2);
    if ((s > 0)) {
        long iters = 0;
        while ((iters < 20)) {
    ...
    printf("%s %ld\n", "max    :", (long)(hi));
    printf("%s %ld\n", "sum    :", (long)(sum));
    printf("%s %ld %s %ld\n", "mean   :", (long)((mean_x100 / 100)), ".", (long)((mean_x100 % 100)));
    printf("%s %ld\n", "median :", (long)(median));
    printf("%s %ld %s %ld\n", "stddev :", (long)((stddev_x100 / 100)), ".", (long)((stddev_x100 % 100)));
    printf("%s\n", "done");
    return 0;
}
