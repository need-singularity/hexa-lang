...
long rot13_char(long c);
const char* char_from_code(long n);
const char* rot13(const char* s);
...
const char* char_from_code(long n) {
    const char* abc = " !\"#$%&'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstuvwxyz{|}~";
    if ((n >= 32)) {
        if ((n < 127)) {
            long idx = (n - 32);
            return hexa_substr(abc, idx, (idx + 1));
        }
    }
    return "?";
}
...
const char* rot13(const char* s) {
    ...
    while ((i < n)) {
        long c = cs.d[i];
        long r = rot13_char(c);
        out = hexa_concat(out, char_from_code(r));
        i = (i + 1);
    }
    return out;
}
...
int main(int argc, char** argv) {
    ...
    if ((hexa_argc() < 2)) {
        ...
        printf("%s %s\n", "rot13!    →", rot13("rot13!"));
        printf("%s\n", "round-trip:");
        const char* enc = rot13("Hello, World!");
        printf("%s %s\n", "  enc     →", enc);
        printf("%s %s\n", "  dec     →", rot13(enc));
    } else {
        const char* txt = hexa_arg(1);
        printf("%s\n", rot13(txt));
    }
    printf("%s\n", "done");
    return 0;
