...
const char* b64_char(long n);
const char* encode(const char* s);

static const char* alphabet = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";

const char* b64_char(long n) {
    return hexa_substr(alphabet, n, (n + 1));
}
...
const char* encode(const char* s) {
    ...
    while ((i < n)) {
        ...
        long tri = (((b0 * 65536) + (b1 * 256)) + b2);
        long v0 = (tri / 262144);
        long v1 = ((tri / 4096) % 64);
        long v2 = ((tri / 64) % 64);
        long v3 = (tri % 64);
        out = hexa_concat(out, b64_char(v0));
        out = hexa_concat(out, b64_char(v1));
        if ((have1 == 1)) {
            out = hexa_concat(out, b64_char(v2));
        } else {
            out = hexa_concat(out, "=");
        }
        if ((have2 == 1)) {
            out = hexa_concat(out, b64_char(v3));
        } else {
            out = hexa_concat(out, "=");
        }
        i = (i + 3);
...
int main(int argc, char** argv) {
    ...
    printf("%s %s\n", "'' →", encode(""));
    printf("%s %s\n", "'HEXA' →", encode("HEXA"));
    printf("%s %s\n", "'The quick brown fox' →", encode("The quick brown fox"));
    printf("%s\n", "done");
    return 0;
}
