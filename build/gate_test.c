...
long verdict_passed(const char* tag);
long source_gate_inspect(const char* whitelist, const char* repo);
long phi_gate_inspect(double threshold, double phi_prev, double phi_curr);
double run_perturbation(long seed);
long invariant_gate_inspect(double stability_threshold);
long evaluate_breakthrough(long paths, long blowup, double mk2_conf, double p_value);
const char* mk_consensus_gate(long lens_agreements);
const char* mk_classify_gate(double confidence);

static long Pass = 0;
static long Quarantine = 1;

static long tau = 4;
static long phi = 2;
static long N = 6;
static long sigma_j2 = 288;
static long pert_breakthrough = 2401;

long verdict_passed(const char* tag) {
    return (strcmp(tag, "pass") == 0);
}

long source_gate_inspect(const char* whitelist, const char* repo) {
    if ((strcmp(repo, whitelist) == 0)) {
        return 0;
    } else {
        return 1;
    }
...
long phi_gate_inspect(double threshold, double phi_prev, double phi_curr) {
    double delta = (phi_curr - phi_prev);
    if ((delta < 0.0)) {
        delta = (0.0 - delta);
    }
    if ((delta > threshold)) {
...
long invariant_gate_inspect(double stability_threshold) {
    double r0 = run_perturbation(0);
    double r1 = run_perturbation(1);
    double r2 = run_perturbation(2);
    double all_pass = (((r0 > stability_threshold) && (r1 > stability_threshold)) && (r2 > stability_threshold));
    if (all_pass) {
        return 0;
    } else {
        return 1;
...
int main(int argc, char** argv) {
    ...
    if (!((evaluate_breakthrough(3, 1, 0.8, 0.001) == 4))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    if (!((strcmp(mk_consensus_gate(12), "pass") == 0))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    if (!((strcmp(mk_classify_gate(0.8), "pass") == 0))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    printf("%s\n", "=== gate verified ===");
    printf("%s %ld\n", "breakthrough(3,true,0.8,0.001):", (long)(evaluate_breakthrough(3, 1, 0.8, 0.001)));
    return 0;
}
