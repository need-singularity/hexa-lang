...
static hexa_arr sieve;
static long i = 2;
static long primes_count = 0;
static long sum_primes = 0;
static long k = 2;
...
int main(int argc, char** argv) {
    ...
    if ((hexa_argc() >= 2)) {
        limit = hexa_parse_int(hexa_arg(1));
    }
    sieve = hexa_arr_fill((long)((limit + 1)), (long)(0));
    sieve.d[0] = 1;
    sieve.d[1] = 1;
    while (((i * i) <= limit)) {
        if ((sieve.d[i] == 0)) {
    ...
    printf("%s %ld\n", "Sieve of Eratosthenes up to", (long)(limit));
    printf("%s\n", "─────────────");
    printf("%s %ld\n", "primes found:", (long)(primes_count));
    printf("%s %ld\n", "sum of primes:", (long)(sum_primes));
    if ((primes_count <= 30)) {
        long idx = 0;
        const char* line = "";
        long n = 2;
        while ((n <= limit)) {
            if ((sieve.d[n] == 0)) {
                line = hexa_concat(hexa_concat(line, hexa_int_to_str((long)(n))), " ");
                idx = (idx + 1);
                if (((idx % 10) == 0)) {
                    printf("%s\n", line);
                    line = "";
