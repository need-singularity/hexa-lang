...
long col_index(const char* header, const char* name);
long hexa_user_main();


long col_index(const char* header, const char* name) {
    hexa_arr cols = hexa_split(header, ",");
    long i = 0;
    while ((i < cols.n)) {
        if ((strcmp(hexa_trim(((const char*)cols.d[i])), name) == 0)) {
            return i;
        }
        i = (i + 1);
    }
...
long hexa_user_main() {
    if ((hexa_argc() < 4)) {
        ...
        printf("%s\n", "  csvql sum <col> <file>");
        printf("%s\n", "  csvql avg <col> <file>");
        printf("%s\n", "  csvql filter <col> <value> <file>");
        return 1;
    }
    const char* mode = hexa_arg(1);
    const char* col_name = hexa_arg(2);
    if (((strcmp(mode, "sum") == 0) || (strcmp(mode, "avg") == 0))) {
        const char* path = hexa_arg(3);
        ...
        long idx = col_index(((const char*)lines.d[0]), col_name);
        if ((idx < 0)) {
        ...
        while ((i < lines.n)) {
            ...
            if ((hexa_str_len(hexa_trim(line)) > 0)) {
                hexa_arr cells = hexa_split(line, ",");
                if ((idx < cells.n)) {
                    total = (total + hexa_parse_int(((const char*)cells.d[idx])));
                    count = (count + 1);
                }
            }
            i = (i + 1);
        ...
        if ((strcmp(mode, "sum") == 0)) {
            printf("%s %s %ld %s %ld %s\n", col_name, "sum =", (long)(total), "(rows:", (long)(count), ")");
        } else {
            if ((count > 0)) {
                printf("%s %s %ld %s %ld %s\n", col_name, "avg =", (long)((total / count)), "(rows:", (long)(count), ")");
            } else {
                printf("%s\n", "no rows");
            }
        }
    ...
    if ((strcmp(mode, "filter") == 0)) {
        if ((hexa_argc() < 5)) {
            printf("%s\n", "usage: csvql filter <col> <value> <file>");
            return 1;
        }
        const char* val = hexa_arg(3);
        const char* path = hexa_arg(4);
        const char* content = hexa_read_file(path);
        hexa_arr lines = hexa_split(content, "\n");
        long idx = col_index(((const char*)lines.d[0]), col_name);
        if ((idx < 0)) {
        ...
        while ((i < lines.n)) {
            ...
            if ((hexa_str_len(hexa_trim(line)) > 0)) {
                hexa_arr cells = hexa_split(line, ",");
                if (((idx < cells.n) && (strcmp(hexa_trim(((const char*)cells.d[idx])), val) == 0))) {
                    printf("%s\n", line);
                    matched = (matched + 1);
                }
            }
