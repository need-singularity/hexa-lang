...
const char* to_base(long n, long base);
long from_base(const char* s, long base);
...
const char* to_base(long n, long base) {
    ...
    while ((num > 0)) {
        long d = (num % base);
        out = hexa_concat(hexa_substr(digits, d, (d + 1)), out);
        num = (num / base);
    }
    if ((neg == 1)) {
        out = hexa_concat("-", out);
...
int main(int argc, char** argv) {
    ...
    printf("%s\n", "");
    printf("%s %ld %s %s %s %ld %s\n", "round-trip: n =", (long)(n), "→ hex", hex, "→ dec", (long)(back), (((back == n)) ? ("✓") : ("✗")));
    return 0;
}
