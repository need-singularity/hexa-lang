...
static const char* path = "build/io_test.txt";
...
int main(int argc, char** argv) {
    ...
    content = hexa_concat("hexa wrote this\nline 2\nnumber = ", hexa_int_to_str((long)(42)));
    hexa_write_file(path, content);
    got = hexa_read_file(path);
    printf("%s\n", "read back:");
    printf("%s\n", got);
    printf("%s %ld\n", "len =", (long)(hexa_str_len(got)));
    printf("%s\n", "done");
    return 0;
}
