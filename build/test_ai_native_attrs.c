...
long check(const char* name, long got, long expected);
long check_approx(const char* name, long got, double expected, double eps);
...
long morph(long x);
long square(long x);
long hot_sum(long a, long b);
long cold_error(long code);
long vec_dot(long a, long b);
long par_square(long x);
long bounded_sum(long n);
double f_square(double x);
double f_linear(double x);
long dbl(long x);
long cube(long x);
long lazy_add(long a, long b);
long old_calc(long x);
long fast_add(long a, long b);
long memo_factorial(long n);

static long passed = 0;
static long failed = 0;
static long total = 26;
static long grad_val;
static long grad_lin;
static hexa_arr arr;
static long result;
static long thunk;
static long val;
...
long check(const char* name, long got, long expected) {
    if ((got == expected)) {
        ...
        printf("%s %s %s %ld %s %ld\n", "  FAIL:", name, "-> got", (long)(got), "expected", (long)(expected));
        failed = (failed + 1);
    }
}
...
long check_approx(const char* name, long got, double expected, double eps) {
    ...
    if ((diff < eps)) {
        printf("%s %s %s %ld %s %g %s\n", "  PASS:", name, "->", (long)(got), "(~=", (double)(expected), ")");
        passed = (passed + 1);
    } else {
        printf("%s %s %s %ld %s %g\n", "  FAIL:", name, "-> got", (long)(got), "expected ~=", (double)(expected));
        failed = (failed + 1);
    }
}
...
int main(int argc, char** argv) {
    ...
    printf("%s\n", "");
    printf("%s\n", "══════════════════════════════════════");
    printf("%s %ld %s %ld %s\n", "AI-Native @attr E2E Results:", (long)(passed), "/", (long)((passed + failed)), "PASS");
    if ((failed == 0)) {
        printf("%s\n", "ALL TESTS PASSED");
    } else {
        printf("%s %ld\n", "FAILED:", (long)(failed));
