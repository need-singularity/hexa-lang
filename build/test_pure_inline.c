...
long expensive_calc(long n);
long add1(long x);
long hexa_user_double(long x);
long square(long x);
long ring_neighbors(long cell_id, long n_cells);
double fast_scale(double x, double s);
...
long ring_neighbors(long cell_id, long n_cells) {
    long left = (((cell_id - 1) + n_cells) % n_cells);
    long right = ((cell_id + 1) % n_cells);
    return (left + right);
}
...
int main(int argc, char** argv) {
    ...
    r1 = expensive_calc(100);
    r2 = expensive_calc(100);
    r3 = expensive_calc(100);
    printf("%s %ld %s %ld %s %ld\n", "[1] @pure CSE: r1=", (long)(r1), "r2=", (long)(r2), "r3=", (long)(r3));
    printf("%s %ld\n", "    all equal:", (long)((r1 == r2)));
    r4 = expensive_calc(50);
    printf("%s %ld\n", "[2] @pure different args:", (long)(r4));
    printf("%s %ld\n", "    r4 != r1:", (long)((r4 != r1)));
    v1 = add1(v);
    v2 = hexa_user_double(v1);
    v3 = square(v2);
    printf("%s %ld %s %ld %s %ld\n", "[3] @inline chain: add1(5)=", (long)(v1), "double(6)=", (long)(v2), "square(12)=", (long)(v3));
    n = ring_neighbors(3, 8);
    n2 = ring_neighbors(3, 8);
    printf("%s %ld %s %ld %s %ld\n", "[4] ring_neighbors(3, 8):", (long)(n), "cached:", (long)(n2), "equal:", (long)((n == n2)));
    while ((j < 10000)) {
        sum = (sum + fast_scale((j * 1.0), 0.001));
        j = (j + 1);
    }
    printf("%s %g\n", "[5] @inline 10K calls, sum:", (double)(sum));
    printf("%s\n", "--- @pure+@inline PASS ---");
    return 0;
}
