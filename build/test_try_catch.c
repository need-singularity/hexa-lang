...
long might_fail(long x);
const char* try_parse(const char* s);
...
const char* try_parse(const char* s) {
    ...
    throw;
    return hexa_concat("parse error: ", s);
}
...
int main(int argc, char** argv) {
    ...
    r1 = 42;
    printf("%s %ld\n", "no-error:", (long)(r1));
    if (!((r1 == 42))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    throw;
    "something went wrong";
    r2 = 99;
    printf("%s %ld\n", "after throw:", (long)(r2));
    if (!((r2 == (-1)))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    r3 = safe_div(10, 2);
    printf("%s %ld\n", "safe_div(10,2):", (long)(r3));
    if (!((r3 == 5))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    r4 = safe_div(10, 0);
    printf("%s %ld\n", "safe_div(10,0):", (long)(r4));
    if (!((r4 == (-1)))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    for (long i = 0; i < 5; i++) {
        if ((i == 2)) {
            throw;
            "bad index";
    ...
    printf("%s %ld\n", "errors in loop:", (long)(errors));
    if (!((errors == 1))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    throw;
    "inner error";
    printf("%s %s\n", "nested:", r5);
    if (!((strcmp(r5, "inner caught") == 0))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    throw;
    "level 1";
    if (!((strcmp(r6, "") != 0))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    r7a = might_fail(5);
    printf("%s %ld\n", "might_fail(5):", (long)(r7a));
    if (!((r7a == 25))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    r7b = might_fail((-3));
    printf("%s %ld\n", "might_fail(-3):", (long)(r7b));
    if (!((r7b == (-1)))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    throw;
    "error code 42";
    printf("%s %s\n", "error msg:", msg);
    if (!((strcmp(msg, "") != 0))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    r9 = (r9 + 10);
    r9 = (r9 + 20);
    throw;
    "stop";
    r9 = (r9 + 100);
    printf("%s %ld\n", "partial compute:", (long)(r9));
    if (!((r9 == 31))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    r10 = try_parse("42");
    printf("%s %ld\n", "try_parse(42):", (long)(r10));
    if (!((r10 == 42))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    r11 = try_parse("abc");
    printf("%s %ld\n", "try_parse(abc):", (long)(r11));
    if (!((r11 == (-1)))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    printf("%s\n", "");
