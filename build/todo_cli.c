...
static const char* DB = "/tmp/hexa_todo.db";
static hexa_arr save_lines;
static const char* cmd;
static hexa_arr lines;

hexa_arr load() {
    const char* content = hexa_read_file(DB);
    return hexa_split(content, "\n");
}

long save_arr() {
    const char* out = "";
    long i = 0;
    while ((i < save_lines.n)) {
        const char* l = ((const char*)save_lines.d[i]);
        if ((hexa_str_len(l) > 0)) {
            if ((hexa_str_len(out) > 0)) {
                out = hexa_concat(out, "\n");
            }
            out = hexa_concat(out, l);
        }
        i = (i + 1);
    }
    hexa_write_file(DB, hexa_concat(out, "\n"));
    return 0;
}
...
int main(int argc, char** argv) {
    ...
    save_lines = hexa_arr_new();
    if ((hexa_argc() < 2)) {
        printf("%s\n", "usage: todo_cli <add|list|done|clear> [args]");
        exit(1);
    }
    cmd = hexa_arg(1);
    lines = load();
    if ((strcmp(cmd, "add") == 0)) {
        if ((hexa_argc() < 3)) {
            printf("%s\n", "usage: todo_cli add <content>");
            exit(1);
        }
        const char* item = hexa_arg(2);
        const char* new_line = hexa_concat("[ ] ", item);
        hexa_arr new_lines = hexa_arr_concat(lines, hexa_arr_lit((long[]){(long)(new_line)}, 1));
        ...
        if ((strcmp(cmd, "list") == 0)) {
            ...
            long i = 0;
            while ((i < lines.n)) {
                const char* l = ((const char*)lines.d[i]);
                if ((hexa_str_len(l) > 0)) {
                    count = (count + 1);
                    printf("%ld %s %s\n", (long)(count), ".", l);
                }
            ...
            if ((strcmp(cmd, "done") == 0)) {
                if ((hexa_argc() < 3)) {
                    printf("%s\n", "usage: todo_cli done <n>");
                    exit(1);
                }
                long n = hexa_parse_int(hexa_arg(2));
                hexa_arr new_lines = hexa_arr_new();
                long idx = 0;
                long i = 0;
                while ((i < lines.n)) {
                    const char* l = ((const char*)lines.d[i]);
                    if ((hexa_str_len(l) > 0)) {
                        idx = (idx + 1);
                        if ((idx == n)) {
                            const char* rest = hexa_substr(l, 3, hexa_str_len(l));
                            new_lines = hexa_arr_concat(new_lines, hexa_arr_lit((long[]){(long)(hexa_concat("[x]", rest))}, 1));
                            printf("%s %s\n", "done:", l);
                        } else {
                            new_lines = hexa_arr_concat(new_lines, hexa_arr_lit((long[]){(long)(l)}, 1));
                        }
                    }
                    i = (i + 1);
                }
