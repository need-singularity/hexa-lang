...
long naive_fib(long n);
long memo_fib(long n);

static double t0;
static long r1;
static double t1;
static double naive_ms;
static double t2;
static long r2;
static double t3;
static double memo_ms;
static double t4;
static long r3;
static double t5;
static double big_ms;
...
int main(int argc, char** argv) {
    ...
    printf("%s\n", "");
    printf("%s\n", "[1] naive_fib(35) — O(2^n), no caching...");
    t0 = hexa_clock();
    r1 = naive_fib(35);
    t1 = hexa_clock();
    naive_ms = ((t1 - t0) * 1000.0);
    printf("%s %ld\n", "    result:", (long)(r1));
    printf("%s %g %s\n", "    time:", (double)(naive_ms), "ms");
    printf("%s\n", "");
    printf("%s\n", "[2] memo_fib(35) — O(n), with @memoize...");
    t2 = hexa_clock();
    r2 = memo_fib(35);
    t3 = hexa_clock();
    memo_ms = ((t3 - t2) * 1000.0);
    printf("%s %ld\n", "    result:", (long)(r2));
    printf("%s %g %s\n", "    time:", (double)(memo_ms), "ms");
    printf("%s\n", "");
    if (!((r1 == r2))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    printf("%s\n", "[3] memo_fib(80) — impossible without memoize...");
    t4 = hexa_clock();
    r3 = memo_fib(80);
    t5 = hexa_clock();
    big_ms = ((t5 - t4) * 1000.0);
    printf("%s %ld\n", "    result:", (long)(r3));
    printf("%s %g %s\n", "    time:", (double)(big_ms), "ms");
    printf("%s\n", "");
    printf("%s\n", "═══════════════════════════════════════════");
    printf("%s\n", "Performance Summary:");
    printf("%s %g %s\n", "  naive fib(35):     ", (double)(naive_ms), "ms  O(2^n)");
    printf("%s %g %s\n", "  @memoize fib(35):  ", (double)(memo_ms), "ms  O(n)");
    printf("%s %g %s\n", "  @memoize fib(80):  ", (double)(big_ms), "ms  O(n)");
    printf("%s\n", "");
    if ((naive_ms > 0.0)) {
        if ((memo_ms > 0.0)) {
            double speedup = (naive_ms / memo_ms);
            printf("%s %g %s\n", "  @memoize speedup:  ~", (double)(hexa_round((double)(speedup))), "x faster");
        } else {
            printf("%s\n", "  @memoize speedup:  instant (< 1us)");
        }
    }
    printf("%s\n", "═══════════════════════════════════════════");
    printf("%s\n", "");
    printf("%s\n", "Traditional compiler: O(2^n) code -> O(2^n) machine code (faster 2^n)");
    printf("%s\n", "AI-native compiler:   O(2^n) code -> O(n) or O(log n) machine code");
    printf("%s\n", "The algorithm itself is replaced, not just optimized.");
    return 0;
}
