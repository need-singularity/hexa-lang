...
const char* state_name(long s);
const char* state_icon(long s);
long state_duration(long s);
long next_state(long s);

static long light_a = 0;
static long light_b = 1;
static long timer = 0;
static long t = 0;
...
static long dur_a;
static long dur_b;
static long elapsed_a = 0;
static long elapsed_b = 0;
...
int main(int argc, char** argv) {
    ...
    printf("%s\n", "=== traffic sim (20 ticks) ===");
    printf("%s\n", "tick | A           | B           | conflict");
    printf("%s\n", "─────┼─────────────┼─────────────┼─────────");
    dur_a = state_duration(light_a);
    dur_b = state_duration(light_b);
    while ((t < 20)) {
        long conflict = ((((light_a == 1) && (light_b == 1))) ? (1) : (0));
        if ((conflict == 1)) {
            conflicts = (conflicts + 1);
        }
        const char* cflag = (((conflict == 1)) ? (" ⚠") : (""));
        printf("%ld %s %s %s %s %s %s %s %s %s %s\n", (long)(t), " | ", state_icon(light_a), " ", state_name(light_a), " | ", state_icon(light_b), " ", state_name(light_b), " |", cflag);
