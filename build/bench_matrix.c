...
int main(int argc, char** argv) {
    ...
    r = fib(30);
    t1 = hexa_clock();
    printf("%s %ld %s %g %s\n", "fib(30) =", (long)(r), " time =", (double)((t1 - t0)), "s");
    t2 = hexa_clock();
    while ((i < 10000000)) {
        s = (s + i);
        i = (i + 1);
    ...
    t3 = hexa_clock();
    printf("%s %ld %s %g %s\n", "sum 1..10M =", (long)(s), " time =", (double)((t3 - t2)), "s");
    t4 = hexa_clock();
    while ((k <= 200000)) {
        if ((is_prime(k) == 1)) {
            count = (count + 1);
    ...
    t5 = hexa_clock();
    printf("%s %ld %s %g %s\n", "primes <= 200k =", (long)(count), " time =", (double)((t5 - t4)), "s");
    t6 = hexa_clock();
    while ((j < 1000000)) {
        ss = (ss + hexa_sqrt((double)((j * 1.0))));
        j = (j + 1);
    }
    t7 = hexa_clock();
    printf("%s %g %s %g %s\n", "sum sqrt(1..1M) =", (double)(ss), " time =", (double)((t7 - t6)), "s");
    printf("%s\n", "done");
    return 0;
}
