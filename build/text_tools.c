...
long is_palindrome(const char* s);
long count_vowels(const char* s);
long char_freq_sum(const char* s);
long is_anagram(const char* a, const char* b);


const char* reverse_str(const char* s) {
    const char* out = "";
    long n = hexa_str_len(s);
    long i = (n - 1);
    while ((i >= 0)) {
        out = hexa_concat(out, hexa_substr(s, i, (i + 1)));
        i = (i - 1);
    }
    return out;
}
...
long is_palindrome(const char* s) {
    const char* lo = hexa_to_lower(s);
    const char* rev = reverse_str(lo);
    if ((strcmp(lo, rev) == 0)) {
        return 1;
    }
    return 0;
...
long count_vowels(const char* s) {
    const char* lo = hexa_to_lower(s);
    hexa_arr cs = hexa_chars(lo);
    long n = cs.n;
    long count = 0;
    long i = 0;
    while ((i < n)) {
        const char* c = hexa_substr(lo, i, (i + 1));
        if ((((((strcmp(c, "a") == 0) || (strcmp(c, "e") == 0)) || (strcmp(c, "i") == 0)) || (strcmp(c, "o") == 0)) || (strcmp(c, "u") == 0))) {
            count = (count + 1);
        }
        i = (i + 1);
    }
...
long char_freq_sum(const char* s) {
    hexa_arr cs = hexa_chars(hexa_to_lower(s));
    long n = cs.n;
    long sum = 0;
    long i = 0;
    while ((i < n)) {
...
long is_anagram(const char* a, const char* b) {
    if ((hexa_str_len(a) != hexa_str_len(b))) {
        return 0;
    }
...
int main(int argc, char** argv) {
    ...
    printf("%s %s\n", "'HEXA'  →", reverse_str("HEXA"));
    printf("%s\n", "");
    printf("%s\n", "=== palindrome ===");
    printf("%s %ld\n", "'racecar' →", (long)(is_palindrome("racecar")));
    printf("%s %ld\n", "'hello'   →", (long)(is_palindrome("hello")));
    printf("%s %ld\n", "'Aba'     →", (long)(is_palindrome("Aba")));
    printf("%s %ld\n", "'noon'    →", (long)(is_palindrome("noon")));
    printf("%s\n", "");
    printf("%s\n", "=== vowel count ===");
    printf("%s %ld\n", "'Hello World'    →", (long)(count_vowels("Hello World")));
    printf("%s %ld\n", "'programming'    →", (long)(count_vowels("programming")));
    printf("%s %ld\n", "'rhythm'         →", (long)(count_vowels("rhythm")));
    printf("%s\n", "");
    printf("%s\n", "=== anagram (간이) ===");
    printf("%s %ld\n", "listen vs silent →", (long)(is_anagram("listen", "silent")));
    printf("%s %ld\n", "hello  vs world  →", (long)(is_anagram("hello", "world")));
    printf("%s %ld\n", "evil   vs vile   →", (long)(is_anagram("evil", "vile")));
    printf("%s\n", "done");
    return 0;
}
