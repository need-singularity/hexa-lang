...
long* primeset_empty();
long prime_index(long p);
double primeset_jaccard(long a_bits, long b_bits);
long bitand(long a, long b);
long bitor(long a, long b);
double keyword_score(long hits, long total);
double sector_score(double kw, double val, double ps);
long* classify_v2(double kw_score, double val_score, double ps_score, const char* sector_name);

static long* result;

long* primeset_empty() {
    return hexa_struct_alloc((long[]){(long)(0)}, 1);
}
...
double primeset_jaccard(long a_bits, long b_bits) {
    long inter = bitand(a_bits, b_bits);
    long union_bits = bitor(a_bits, b_bits);
    if ((union_bits == 0)) {
        return 0.0;
    } else {
        return (double)((inter / union_bits));
    }
}
...
double sector_score(double kw, double val, double ps) {
    return (((0.5 * kw) + (0.3 * val)) + (0.2 * ps));
}

long* classify_v2(double kw_score, double val_score, double ps_score, const char* sector_name) {
    double total = sector_score(kw_score, val_score, ps_score);
    return hexa_struct_alloc((long[]){(long)(sector_name), (long)(total), (long)(kw_score), (long)(val_score), (long)(ps_score)}, 5);
}


int main(int argc, char** argv) {
    hexa_main_argc = argc; hexa_main_argv = argv;
    result = classify_v2(0.8, 0.6, 0.4, "cosmology");
    printf("%s\n", "=== classify verified ===");
    printf("%s %ld\n", "sector:", (long)(/* unknown field sector */ 0));
    printf("%s %ld\n", "confidence:", (long)(/* unknown field confidence */ 0));
    return 0;
}
