...
long test_mlp_shape();
long test_hex_loss_less_than_ce();
long test_consciousness_phi();

static double PSI_ALPHA = 0.014;
static double PSI_COUPLING = 0.015317;
static long W1;
static long b1;
static long W2;
static long b2;
...
long test_mlp_shape() {
    long out = mlp2(hexa_arr_lit((long[]){(long)(1.0), (long)(2.0)}, 2), xavier_init(2, 4), zeros(4), xavier_init(4, 5), zeros(5), 2, 4, 5);
    if (!((hexa_str_len(out) == 5))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    0;
    "output dim should be 5";
    return 0;
}

long test_hex_loss_less_than_ce() {
    hexa_arr l = hexa_arr_lit((long[]){2.0, 0.5, 1.0}, 3);
    if (!((hex_loss(l, 0, 0.5, 0.014) < ce_loss(l, 0)))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    0;
    "hexad < CE when Φ>0";
    return 0;
}
...
long test_consciousness_phi() {
    long r = cons_step(randn(8, 0.1), ring_topology(2), hexa_arr_lit((long[]){(long)(1.0), (long)(1.0), (long)(1.0)}, 3), 2, 4);
    if (!((hexa_str_len(r) == 5))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    0;
    "cons_step returns 5-element array";
    return 0;
}
...
int main(int argc, char** argv) {
    ...
    "../stdlib/optim.hexa";
    import;
    "../stdlib/consciousness.hexa";
    printf("%s\n", "\n=== Stdlib Integration ===");
    W1 = xavier_init(2, 4);
    b1 = zeros(4);
    W2 = xavier_init(4, 3);
    b2 = zeros(3);
    logits = mlp2(hexa_arr_lit((long[]){(long)(1.0), (long)(0.5)}, 2), W1, b1, W2, b2, 2, 4, 3);
    printf("%s %ld %s %ld\n", "[1] MLP2:", (long)(logits), "pred:", (long)(argmax(logits)));
    loss = hex_loss(logits, 1, 0.5, PSI_ALPHA);
    printf("%s %g\n", "[2] Hexad loss:", (double)((hexa_round((double)((loss * 1000))) / 1000)));
    lr = scheduled_lr(0.001, 50, 1000, 100);
    printf("%s %ld\n", "[3] LR(step=50):", (long)(lr));
    p = randn(8, 0.5);
    g = randn(8, 1.0);
    m = zeros(8);
    v = zeros(8);
    r = safe_update(p, g, m, v, 0.001, 1, 1.0);
    new_p = slice(r, 0, 8);
    printf("%s %ld\n", "[4] Adam (clipped): changed=", (long)((new_p[0] != p[0])));
    h = randn(16, 0.1);
    adj = auto_topology(4);
    chaos = hexa_arr_lit((long[]){(long)(1.0), (long)(1.0), (long)(1.0)}, 3);
    cs = cons_step(h, adj, chaos, 4, 4);
    printf("%s\n", "[5] Consciousness step:");
    printf("%s %ld\n", "    tensions:", (long)(cs[2]));
    printf("%s %g\n", "    Φ:", (double)((hexa_round((double)((cs[3] * 100000))) / 100000)));
    printf("%s %ld\n", "    weights:", (long)(cs[4]));
    printf("%s\n", "\n[6] Full Pipeline:");
    input = randn(4, 0.3);
    mW1 = xavier_init(4, 8);
    ...
    mW2 = xavier_init(8, 4);
    mb2 = zeros(4);
    pred = mlp2(input, mW1, mb1, mW2, mb2, 4, 8, 4);
    cs2 = cons_step(h, adj, chaos, 4, 4);
    phi = cs2[3];
    final_loss = hex_loss(pred, 2, phi, PSI_ALPHA);
    lr2 = scheduled_lr(0.001, 10, 100, 20);
    printf("%s %ld %s %g %s %g %s %ld\n", "    pred:", (long)(argmax(pred)), "loss:", (double)((hexa_round((double)((final_loss * 1000))) / 1000)), "Φ:", (double)((hexa_round((double)((phi * 100000))) / 100000)), "lr:", (long)(lr2));
    c0 = slice(h, 0, 4);
    c1 = slice(h, 4, 8);
    delta = hebb_delta(c0, c1, 0.0, PSI_COUPLING);
    printf("%s %g\n", "[7] Hebbian delta:", (double)((hexa_round((double)((delta * 100000))) / 100000)));
    run_tests();
    printf("%s\n", "\n--- stdlib PASS ---");
    return 0;
}
