...
const char* indent(long depth);
const char* format_json(const char* src);
...
const char* format_json(const char* src) {
    ...
    long prev_bs = 0;
    while ((i < n)) {
        const char* c = hexa_substr(src, i, (i + 1));
        if ((in_str == 1)) {
            out = hexa_concat(out, c);
            if (((strcmp(c, "\\") == 0) && (prev_bs == 0))) {
                prev_bs = 1;
            } else {
                if (((strcmp(c, "\"") == 0) && (prev_bs == 0))) {
                    in_str = 0;
        ...
        if (((strcmp(c, "{") == 0) || (strcmp(c, "[") == 0))) {
            out = hexa_concat(out, c);
            depth = (depth + 1);
            out = hexa_concat(hexa_concat(out, "\n"), indent(depth));
            i = (i + 1);
            continue;
        }
        if (((strcmp(c, "}") == 0) || (strcmp(c, "]") == 0))) {
            depth = (depth - 1);
            out = hexa_concat(hexa_concat(hexa_concat(out, "\n"), indent(depth)), c);
            i = (i + 1);
            continue;
        }
        if ((strcmp(c, ",") == 0)) {
...
int main(int argc, char** argv) {
    ...
    if ((hexa_str_len(input) == 0)) {
        printf("%s\n", "usage: echo '<json>' | json_pretty");
        exit(1);
    }
    pretty = format_json(input);
    printf("%s\n", pretty);
    return 0;
}
