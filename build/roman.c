...
const char* to_roman(long n);
long from_roman(const char* s);


const char* to_roman(long n) {
    long num = n;
    const char* out = "";
    hexa_arr values = hexa_arr_lit((long[]){1000, 900, 500, 400, 100, 90, 50, 40, 10, 9, 5, 4, 1}, 13);
    hexa_arr symbols = hexa_arr_lit((long[]){(long)("M"), (long)("CM"), (long)("D"), (long)("CD"), (long)("C"), (long)("XC"), (long)("L"), (long)("XL"), (long)("X"), (long)("IX"), (long)("V"), (long)("IV"), (long)("I")}, 13);
    long i = 0;
    while ((i < 13)) {
        while ((num >= values.d[i])) {
            out = hexa_concat(out, ((const char*)symbols.d[i]));
            num = (num - values.d[i]);
        }
        i = (i + 1);
    }
...
long from_roman(const char* s) {
    ...
    while ((i < n)) {
        const char* c = hexa_substr(s, i, (i + 1));
        long v = 0;
        if ((strcmp(c, "I") == 0)) {
            v = 1;
        }
        ...
        if (((i + 1) < n)) {
            const char* nc = hexa_substr(s, (i + 1), (i + 2));
            long nv = 0;
            if ((strcmp(nc, "I") == 0)) {
                nv = 1;
            }
...
int main(int argc, char** argv) {
    ...
    if ((hexa_argc() >= 2)) {
        long n = hexa_parse_int(hexa_arg(1));
        const char* r = to_roman(n);
        printf("%ld %s %s\n", (long)(n), "→", r);
        long back = from_roman(r);
        printf("%s %s %ld\n", r, "→", (long)(back));
    } else {
        printf("%s\n", "=== roman tests ===");
        long i = 0;
        hexa_arr nums = hexa_arr_lit((long[]){1, 4, 9, 14, 19, 40, 90, 400, 900, 1994, 2024, 3888}, 12);
        while ((i < 12)) {
            long n = nums.d[i];
            const char* r = to_roman(n);
            long back = from_roman(r);
            const char* ok = (((n == back)) ? ("✓") : ("✗"));
            printf("%ld %s %s %s %ld %s\n", (long)(n), "→", r, "→", (long)(back), ok);
            i = (i + 1);
        }
    }
    printf("%s\n", "done");
