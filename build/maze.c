...
long inbounds(long r, long c);
long next_rand();
long carve(long r, long c);

static long W = 15;
static long H = 8;
static long SIZE = 120;
...
long carve(long r, long c) {
    grid.d[idx(r, c)] = 0;
    hexa_arr order = hexa_arr_lit((long[]){0, 1, 2, 3}, 4);
    long k = 3;
    while ((k > 0)) {
        long j = (next_rand() % (k + 1));
        long t = order.d[k];
        order.d[k] = order.d[j];
        order.d[j] = t;
        k = (k - 1);
    ...
    while ((i < 4)) {
        ...
        long nc = (c + dc);
        if ((inbounds(nr, nc) == 1)) {
            if ((grid.d[idx(nr, nc)] == 1)) {
                grid.d[idx((r + (dr / 2)), (c + (dc / 2)))] = 0;
                long _rr = carve(nr, nc);
            }
        }
        i = (i + 1);
    }
...
int main(int argc, char** argv) {
    ...
    grid = hexa_arr_fill((long)(120), (long)(1));
    _r = carve(1, 1);
    printf("%s\n", "=== maze (seed 12345) ===");
    while ((r < H)) {
        const char* line = "";
