...
long tau_fn(long n);
long sigma_fn(long n);
long phi_fn(long n);
long sopfr(long n);
long is_perfect(long n);
long is_n6_exact(long n);
...
long is_n6_exact(long n) {
    return ((sigma_fn(n) * phi_fn(n)) == (n * tau_fn(n)));
}


int main(int argc, char** argv) {
    hexa_main_argc = argc; hexa_main_argv = argv;
    if (!(is_perfect(6))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    if (!(is_n6_exact(6))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    if (!((tau_fn(6) == 4))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    if (!((sigma_fn(6) == 12))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    if (!((phi_fn(6) == 2))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    if (!((sopfr(6) == 5))) { fprintf(stderr, "assertion failed\n"); exit(1); }
    printf("%s\n", "=== pure_math verified ===");
    printf("%s %ld\n", "sigma(6)=", (long)(sigma_fn(6)));
    printf("%s %ld\n", "phi(6)=", (long)(phi_fn(6)));
    printf("%s %ld\n", "tau(6)=", (long)(tau_fn(6)));
    printf("%s %ld\n", "perfect(6):", (long)(is_perfect(6)));
    printf("%s %ld\n", "n6_exact(6):", (long)(is_n6_exact(6)));
    return 0;
}
