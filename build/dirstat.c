...
int main(int argc, char** argv) {
    ...
    if ((hexa_argc() < 2)) {
        printf("%s\n", "usage: dirstat <directory>");
        exit(1);
    }
    dir = hexa_arg(1);
    output = hexa_exec(hexa_concat(hexa_concat("du -sk ", dir), "/* 2>/dev/null | sort -rn | head -20"));
    lines = hexa_split(output, "\n");
    printf("%s %s\n", "Top 20 largest in", dir);
    printf("%s\n", "─────────────────────────────────");
    long __fi_line_1 = 0;
    while ((__fi_line_1 < lines.n)) {
        ...
        if ((hexa_str_len(line) > 0)) {
            ...
            if ((parts.n >= 2)) {
                long size_kb = hexa_parse_int(((const char*)parts.d[0]));
                const char* path = ((const char*)parts.d[1]);
                total = (total + size_kb);
                long mb = (size_kb / 1024);
                printf("%ld %s %s\n", (long)(mb), "MB  ", path);
            }
        }
        i = (i + 1);
    ...
    printf("%s\n", "─────────────────────────────────");
    printf("%s %ld %s\n", "Total top 20:", (long)((total / 1024)), "MB");
    return 0;
}
