...
const char* apply_inline(const char* s);
...
static long in_list = 0;
static long in_para = 0;
static long i = 0;

const char* escape_html(const char* s) {
    const char* a = hexa_replace(s, "&", "&amp;");
    const char* b = hexa_replace(a, "<", "&lt;");
    const char* c = hexa_replace(b, ">", "&gt;");
    return c;
}

const char* apply_inline(const char* s) {
    const char* a = hexa_replace(s, "**", "<STRONG>");
    const char* b = hexa_replace(a, "<STRONG>", "<strong>");
    const char* c = hexa_replace(b, "`", "<code>");
    return c;
}
...
int main(int argc, char** argv) {
    ...
    in_path = hexa_arg(1);
    out_path = (((hexa_argc() >= 3)) ? (hexa_arg(2)) : ("/tmp/out.html"));
    content = hexa_read_file(in_path);
    lines = hexa_split(content, "\n");
    while ((i < lines.n)) {
        const char* raw = ((const char*)lines.d[i]);
        const char* line = hexa_trim(raw);
        i = (i + 1);
        if ((hexa_str_len(line) == 0)) {
            if ((in_list == 1)) {
                html = hexa_concat(html, "</ul>\n");
                in_list = 0;
            }
            if ((in_para == 1)) {
                html = hexa_concat(html, "</p>\n");
                in_para = 0;
            }
            continue;
        }
        if ((hexa_starts_with(line, "# ") == 1)) {
            if ((in_list == 1)) {
                html = hexa_concat(html, "</ul>\n");
                in_list = 0;
            }
            ...
            const char* rest = hexa_substr(line, 2, hexa_str_len(line));
            html = hexa_concat(hexa_concat(hexa_concat(html, "<h1>"), apply_inline(escape_html(rest))), "</h1>\n");
            continue;
        }
        if ((hexa_starts_with(line, "## ") == 1)) {
            if ((in_list == 1)) {
            ...
            const char* rest = hexa_substr(line, 3, hexa_str_len(line));
            html = hexa_concat(hexa_concat(hexa_concat(html, "<h2>"), apply_inline(escape_html(rest))), "</h2>\n");
            continue;
        }
        if ((hexa_starts_with(line, "- ") == 1)) {
            if ((in_para == 1)) {
            ...
            const char* item = hexa_substr(line, 2, hexa_str_len(line));
            html = hexa_concat(hexa_concat(hexa_concat(html, "  <li>"), apply_inline(escape_html(item))), "</li>\n");
            continue;
        }
        if ((in_list == 1)) {
            html = hexa_concat(html, "</ul>\n");
        ...
        if ((in_para == 0)) {
            ...
            html = hexa_concat(html, " ");
        }
        html = hexa_concat(html, apply_inline(escape_html(line)));
    }
    if ((in_list == 1)) {
        html = hexa_concat(html, "</ul>\n");
    }
    ...
    html = hexa_concat(html, "</body></html>\n");
    hexa_write_file(out_path, html);
    printf("%s %s %ld %s\n", "wrote", out_path, (long)(hexa_str_len(html)), "bytes");
    return 0;
}
