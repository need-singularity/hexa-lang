...
long mandel(double cr, double ci);
const char* shade(long it);

static long W = 60;
static long H = 24;
static long MAX_ITER = 50;
static long row = 0;
...
int main(int argc, char** argv) {
    ...
    printf("%s\n", "=== Mandelbrot (ASCII) ===");
    while ((row < 24)) {
        double ci = ((0.0 - 1.2) + (row * 0.1));
        const char* line = "";
        long col = 0;
        while ((col < 60)) {
            double cr = ((0.0 - 2.2) + (col * 0.05));
            long it = mandel(cr, ci);
            line = hexa_concat(line, shade(it));
            col = (col + 1);
        }
        printf("%s\n", line);
        row = (row + 1);
