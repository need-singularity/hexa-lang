...
const char* strip_tags(const char* s);
const char* extract_title(const char* html);
...
const char* extract_title(const char* html) {
    const char* lo = hexa_to_lower(html);
    long start = hexa_index_of(lo, "<title>");
    if ((start < 0)) {
        return "(no title)";
    }
    long open_end = (start + 7);
    long end = hexa_index_of(lo, "</title>");
    if ((end < 0)) {
        return "(broken title)";
    }
    return hexa_trim(hexa_substr(html, open_end, end));
}
...
int main(int argc, char** argv) {
    ...
    url = hexa_arg(1);
    printf("%s %s\n", "fetching:", url);
    cmd = hexa_concat("curl -sL --max-time 10 ", url);
    body = hexa_exec(cmd);
    if ((hexa_str_len(body) == 0)) {
        printf("%s\n", "(empty response)");
        exit(1);
    }
    printf("%s\n", "─────────────");
    printf("%s %ld %s\n", "status: OK (", (long)(hexa_str_len(body)), "bytes )");
    printf("%s %s\n", "title:", extract_title(body));
    head = hexa_substr(body, 0, ((1000) < (hexa_str_len(body)) ? (1000) : (hexa_str_len(body))));
    text = strip_tags(head);
    trimmed = hexa_trim(text);
    printf("%s\n", "─── preview ───");
    if ((hexa_str_len(trimmed) > 400)) {
        printf("%s\n", hexa_concat(hexa_substr(trimmed, 0, 400), "..."));
    } else {
        printf("%s\n", trimmed);
    }
    printf("%s\n", "done");
