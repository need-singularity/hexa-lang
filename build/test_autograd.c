...
double mul_wx(double w, double x);
double mse_loss(double w, double x, double y);
double tension_lr(double base_lr, double tension);

static long g1;
static long g2;
static long grads;
static long loss_grads;
static long tlr_grads;
...
double tension_lr(double base_lr, double tension) {
    return (base_lr * (1.0 + (0.5 * tension)));
}
...
int main(int argc, char** argv) {
    ...
    g1 = square_grad(3.0);
    printf("%s %ld\n", "f(x) = x^2, f'(3.0) =", (long)(g1));
    g2 = linear_grad(5.0);
    printf("%s %ld\n", "f(x) = 3x+1, f'(5.0) =", (long)(g2));
    grads = mul_wx_grad(2.0, 5.0);
    printf("%s %ld\n", "f(w,x) = w*x, grad(2.0, 5.0) =", (long)(grads));
    loss_grads = mse_loss_grad(1.5, 2.0, 4.0);
    printf("%s %ld\n", "MSE loss grad(w=1.5, x=2.0, y=4.0) =", (long)(loss_grads));
    tlr_grads = tension_lr_grad(0.01, 0.8);
    printf("%s %ld\n", "tension_lr grad(0.01, 0.8) =", (long)(tlr_grads));
    printf("%s\n", "--- @autograd 5/5 PASS ---");
    return 0;
}
