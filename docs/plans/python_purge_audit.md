...
| 3 | `loop-rules.json:8` (keywords) | **실 호출** — `python3 -c json.loads(...)` grammar.jsonl 파싱 | **PURGE** → `scripts/count_grammar_tokens.hexa` |
| 4 | `loop-rules.json:15` (operators) | **실 호출** — 동일 | **PURGE** (#3 과 같은 스크립트, 인자만 다름) |
| 5 | `config/emergence_patterns.json:168` | **실 호출** — `$N6_ARCH/tools/nexus` import (외부 리포) | **보류** — n6-architecture 툴 의존, hexa-lang 범위 밖 |
...
| 10 | `shared/hexa-lang/grammar.jsonl`, `shared/config/commands.json`, `shared/hexa-lang/ml-pipeline.json` | 벤치 비교값/설명 | **유지** |
...
| P3 | `loop-rules.json:8` keywords python3 json.loads | `./hexa scripts/count_grammar_tokens.hexa keywords` | 신규 스크립트 `scripts/count_grammar_tokens.hexa` |
| P4 | `loop-rules.json:15` operators 동일 | `./hexa scripts/count_grammar_tokens.hexa operators` | 동일 스크립트, 인자만 다름 |
...
**P5: config/emergence_patterns.json:168 `python3 -c "import sys; ... import nexus; print(nexus.analyze(...))"`**
