...
- Every `match x { Color::Red => …, Color::Green => …, _ => … }`
  form compiled through `scripts/native_build.hexa` now dispatches
...
7. **P7-6 end-to-end scale test** — once 1+5 land, attempt
   `hexa scripts/native_build.hexa self/hexa_full.hexa -o build/hexa_new`
