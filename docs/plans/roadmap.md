...
## 2026-04-11 셀프호스팅 44모듈 포팅 완료 + 트리 정리

44 모듈 self/ ← src/ 전량 포팅 완료. codegen/opt/lower 서브디렉토리 포함 전체 셀프호스팅 코드 이관.

### 셀프호스팅 포팅 (44모듈 100%)
- [x] **src/ → self/ 44모듈 포팅 완료** — ~12K LOC 신규 Hexa 코드
  - codegen/ 8모듈 포팅
  - opt/ 17모듈 포팅
  - lower/ 4모듈 포팅
  - 기타 15모듈 포팅
...
### 트리 정리 (7.4G 절감)
- [x] **ready/ 삭제** — 6.3G stale 마이그레이션 아티팩트 제거
- [x] **PGO 빌드 아티팩트 삭제** — 28M
- [x] **stale worktrees 삭제** — 1.1G
- [x] **broken symlink 삭제**
...
- [x] **compiler::test_compile_if** — `try_const_fold` If arm 회귀 (aaeb12c), 이전 세션에서 수정 완료
- [x] **interpreter::test_evolve_stats_tracking** — 인라인 fast-path에 evolve 통계 훅, 이전 세션에서 수정 완료
- [x] **interpreter::test_yield_gen_buffer** — `parse_primary` `Token::Or` 처리, 이전 세션에서 수정 완료
...
- [x] **잔존 참조 갱신** — 4파일+1심링크 수정 완료 (CLAUDE.md, roadmaps, convergence, breakthroughs, grammar symlink)

### 셀프호스팅 포팅
- [x] **self/formatter.hexa 포팅 완료** — 641 LOC, 34 stmt + 31 expr variant 커버
- [x] **44모듈 전량 포팅** — 2026-04-11 상단 섹션 참조
...
- [x] **INT-5 @contract 토큰 enum + parser 매핑** — self-host parse OK, 의미론은 stub
...
- [x] `scripts/bench/t1_run.hexa` — htz SSH 실행 + JSONL 기록
- [x] `self/interpreter.hexa` — `read_file_bytes` / `file_size` 바이너리 I/O 빌트인
...
## Current State (2026-04-11 — Rust-Free Self-Host)

> **Milestone**: `src/` 디렉터리 완전 삭제. `self/*.hexa` 605 파일이 유일한 소스.

| 모듈 | 파일 | LOC | 상태 |
|------|------|-----|------|
| Interpreter | self/interpreter.hexa | 8,427 | ✅ 완전 + self-host bootstrap |
| Parser | self/parser.hexa | 3,785 | ✅ 83/83 AST kinds |
| Lexer | self/lexer.hexa | 587 | ✅ 53 keywords + 24 ops |
| Compiler | self/compiler.hexa | — | ✅ AST → bytecode |
| VM (Bytecode) | self/vm.hexa | — | ✅ stack-based |
| JIT bridge | self/jit.hexa | — | ✅ Cranelift 인터페이스 |
| LSP Server | self/lsp.hexa | — | ✅ 기본 구현 |
| Package Manager | self/package.hexa | — | ✅ 기본 구현 |
| HEXA-IR | self/ir/*.hexa | — | ✅ J₂=24 opcodes (τ=4) |
| Trace JIT | self/trace_jit.hexa | — | ✅ hot loop detection |
| Stdlib | self/std_*.hexa | 14 modules | ✅ net/fs/io/time/crypto/... |
| **Total** | **605 .hexa files** | **~40K+** | **205/205 regression PASS** |
...
- [x] **Phase 2**: 인터프리터 포팅 (8.5K LOC → Hexa) — 44모듈 완료 (2026-04-11)
- [x] **Phase 3**: VM 포팅 (1.5K LOC → Hexa) — self/vm.hexa 완료
- [x] **Phase 4**: C 코드 생성 백엔드 — self/build_c.hexa + codegen_c2.hexa (clang 백엔드)
- [x] **Phase 5**: 셀프 컴파일 달성 → src/ 전체 삭제 (2026-04-11), Cargo.toml/cranelift/wasm-bindgen 0
- [ ] **Phase 6**: 생태계 복원 (LSP, 패키지매니저=hexa.toml, WASM)
...
- [x] @simd — simd_hint.rs 분석모듈 + interpreter parallel ops + 7 tests (2026-04-11)
- [x] @evolve — EvolveFn AST + self-modifying + generation tracking + 6 tests (2026-04-11)
