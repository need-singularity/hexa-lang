...
(a) running `scripts/lax_baseline_autotrim.hexa` to drop strict-clean paths and
...
`@attr` parser gate migration (`scripts/migrate_fn_attr.hexa`), and the 2124-
line format was called out as an "exploration artifact".

Downstream consequences:

- `scripts/ai_native_lint.hexa`'s `load_lax_baseline()` /
...
- `scripts/lax_baseline_autotrim.hexa` walks `load_baseline_lines(path)` and
...
Dry-run invocation `hexa scripts/lax_baseline_autotrim.hexa` exited after 300s
...
1. **Split the two concerns into separate files.** Move naked-fn counter to
   e.g. `.hexa-naked-fn-count` (scripts/migrate_fn_attr.hexa + CI gate
...
2. **Retire the path-list baseline mechanism entirely.** Delete the baseline
   loader from `scripts/ai_native_lint.hexa`, delete `lax_baseline_autotrim.hexa`,
...
- `scripts/lax_baseline_autotrim.hexa` left with its prior (non-session)
...
- `/Users/ghost/Dev/hexa-lang/scripts/lax_baseline_autotrim.hexa`
- `/Users/ghost/Dev/hexa-lang/scripts/ai_native_lint.hexa` (lines 311–361)
- `/Users/ghost/Dev/hexa-lang/scripts/migrate_fn_attr.hexa` (naked-fn counter consumer)
