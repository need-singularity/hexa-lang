...
| `scripts/native_build.hexa` | 1,598 | wired | Zero-tool driver (`hexa -> native`) — inlines rt-routing helpers (commit a474928c) |
| `scripts/verify_fixpoint.hexa` | 122 | skeleton | 3-stage SHA256 gate already codes to `build/hexa_fix{1,2,3}` |
...
Run `hexa scripts/verify_fixpoint.hexa --target=linux-x86_64` on Mac, then `hexa scripts/verify_fixpoint.hexa` on ubu. Compare `hexa_3` from both — must be byte-identical if determinism holds.
...
- B4 Green-gate: `hexa scripts/native_build.hexa self/hexa_full.hexa -o build/hexa_fix_probe` exits 0, produces executable.
...
- D1 `hexa scripts/verify_fixpoint.hexa --target=darwin-arm64`.
...
## 4. Test Harness Design — `scripts/test_p7_7_fixpoint.hexa` sketch
...
```hexa
// scripts/test_p7_7_fixpoint.hexa
...
fn native_compile(src: string, out: string, seed: string) -> int {
    let cmd = seed + " scripts/native_build.hexa " + src + " -o " + out
...
fn gate_self_compile_one_step() -> bool {
    ...
    let rc = exec_with_status("build/hexa_fix_probe scripts/native_build.hexa "
...
|---|------|---|---|---|
| R1 | **Symbol-ordering non-determinism** — HashMap iteration produces different symbol table order run-to-run | HIGH | byte divergence | Enforce sorted emission in `FuncSymbol` list; lint in scripts/test_p7_7_fixpoint.hexa. Also applies to string literal pool. |
...
| R6 | **xpcproxy flood on 3x rebuild** (HX8) | MED | mac panic | verify_fixpoint already uses stage0 seed path; ensure `scripts/rebuild_stage0.hexa` wrapper is NOT invoked during the 3x compile — native_build is direct. |
...
| R10 | **rt ABI drift** — rt_*_v wrappers signatures desync with callers | MED | link error | Auto-generate from rt_symbols.hexa SSOT; add lint in scripts/check_ssot_sync.hexa. |
...
```
hexa scripts/native_build.hexa self/rt/test_math.hexa -o /tmp/probe_rt
cmp <(hexa scripts/native_build.hexa self/rt/test_math.hexa -o /dev/stdout) \
    <(hexa scripts/native_build.hexa self/rt/test_math.hexa -o /dev/stdout)
```
...
- `scripts/verify_fixpoint.hexa` (existing 3-stage SHA gate)
