...
4. **interpreter.hexa 와의 SSOT 정합성**: hexa_full.hexa 가 변하면 interpreter.hexa 와 drift. T1 분석에서 "byte-identical" 이라 했으니, 추출 후 자동 sync 필요 (`scripts/check_ssot_sync.hexa` 활용).
