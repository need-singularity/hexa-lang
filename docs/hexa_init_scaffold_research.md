...
- `scripts/init_project.hexa` (현행 구현, 1 파일만 생성)
...
`scripts/init_project.hexa` 는 85 줄짜리 단일 함수 호출 도구다. 실행 결과:
...
| 28 | 현행 hexa | `hexa scripts/init_project.hexa` | `project.hexa` | 1 | 극저 | **예** | 아니오 |
...
- **영향**: scaffold 에 `build.sh` 같은 쉘 스크립트 대신 `scripts/*.hexa` 만.
...
| `scripts/` | **선택** | 보조 hexa 스크립트. lang 프로젝트에 관례적. |
...
**scripts/init_project.hexa 확장 필요** (다른 agent 담당):
...
**새 파일 불필요**. 기존 `scripts/init_project.hexa` 만 수정.
...
**도메인 추가 UX**: `hexa init . --preset domain_only --axis life --id dog-robot` → `domains/life/dog-robot/{dog-robot.hexa,CLAUDE.md,.sealed.hash}` 추가. 기존 scripts/add_domain.hexa 류와 통합 여지.
