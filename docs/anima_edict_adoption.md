# anima 프로젝트 edict 이식 제안

> hexa-lang 에서 검증된 edict 법률 시스템을 anima 프로젝트로 이식.
...
anima 는 hexa-lang 과 다른 도메인 (ML 학습/서빙) — 6 edict 규칙 그대로 적용 불가. 재해석 필요:

| hexa-lang edict | anima 버전 | 적용성 |
|------------------|-----------|--------|
...
추가로 **anima 전용 edict** 필요:

| anima-edict | 내용 | enforce_level |
|-------------|------|----------------|
...
~/Dev/anima/shared/anima/edict.json  ← 6+A4 anima edict SSOT
...
### Step 2 — anima 전용 edict (~1시간)

`shared/anima/edict.json` — hexa-lang edict.json 복제 후 anima 재해석:
...
hexa scripts/hexa_init.hexa
  → edict install (pre-commit @edict-stanza)
...
```
edict: `hexa self/edict_cli.hexa` — anima 6+A4 규칙. 새 세션 인지 불필요.
```
...
Week 2  : Step 2 anima 전용 edict 추가 (A1~A4)
...
2. 결과 피드백 → hexa-lang edict 역개선 (dogfood 루프)
3. 다른 프로젝트 (nexus / n6-architecture / prism) 도 동일 이식

---

**참고**:
- hexa-lang edict 파일: `shared/hexa-lang/edict.json`
- 이식 스크립트 예정: `scripts/edict_port.hexa <target-project>` (후속)
