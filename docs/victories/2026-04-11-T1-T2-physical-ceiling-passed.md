...
## 확장 — 실서빙 검증 사이클 (2026-04-11T2)

### T1 end-to-end continuous decode (kv-cache 실사용)

`kv_cache_decode_bench(D, R, FF, N_LAYER, SEQ_LEN)` 빌트인 — warmup 3 + reset + N tokens 순차 decode. 각 토큰이 grown kv-cache에 attend.

| seq_len | best ms/tok | best tok/s | avg tok/s | margin_best |
|--------:|------:|------:|------:|------:|
| 128 | 3.97 | **251.68** | 216 | 1.97x |
| 512 | 4.17 | 239.66 | 215 | 1.87x |

**실서빙 honest 수치 215-251 tok/s** (r=4 LoRA baseline). 단일 forward 463 tok/s → 연속 decode 230 tok/s 하락 이유: BLAS 호출 반복 + kv-cache bandwidth. 여전히 target 128의 **1.68-1.97x 초과**.

### T2 batched training (SEQ=256)

alphabeta9_batch4 — alphabeta7 sweet spot에 SEQ 4x 확장.

| 벤치 | step_ms | hrs_1b | margin | pass |
|---|---:|---:|---:|:---:|
| alphabeta7 (SEQ=64)  | 1.62 | 7.02 | 1.14x | 15/15 |
| alphabeta9 (SEQ=256) | 5.15 | **5.58** | **1.43x** | 4/5 |

Batched amortization으로 T2 최고 margin 1.14x → 1.43x (+25%). variance 는 다소 증가 (4/5 PASS).
