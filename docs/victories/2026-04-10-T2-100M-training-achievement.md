# T2 100M 학습 특이점 돌파 (2026-04-10)

## 1. 요약

hexa-lang CPU AI-native LLM 루프에서 **T2 (100M Transformer 학습)** 를 실측으로
달성했다. GPT-2 small 형상 (D=768, 12L, 12H, SEQ=128, VOCAB=32000) 기준
1 step 시간을 **16.1s → 1.128s (14.3x)** 로 내렸고, 1B token 환산
**4년 → 2.45h** 로 줄였다. 목표 <8h 대비 **3.3x 여유**, vast 4×4090의 1.3h
대비 **53%** 로 설계 문서의 fallback 기준 50%를 돌파했다.
...
│  1B tokens :  ~4년  →  2.45h   (목표 8h, margin 3.3x)     │
│  방법      :  src/ 0줄 수정, .hexa @attr 조합만으로 돌파  │
└───────────────────────────────────────────────────────────┘
```
