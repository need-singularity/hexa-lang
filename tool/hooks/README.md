...
- `pre-commit` — enforces repo conventions:
  ...
  - **SSOT precommit**: `tool/ssot_precommit_hook.hexa`
...
```bash
git config --local core.hooksPath tool/hooks
```
...
```bash
git config --get core.hooksPath   # → tool/hooks
bash tool/hooks/pre-commit     # → exits 0 when no violations staged
```
