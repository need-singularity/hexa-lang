...
/* ═══════════════════════════════════════════════════════════════════
 * Legacy hexa_v2 transpiler bootstrap-gap shim (2026-04-22, hxa-004)
 *
 * The shipped self/native/hexa_v2 binary is older than the codegen_c2
 * SSOT and still emits bare `setenv` / `exec_capture` identifiers as
 * TAG_FN operands to hexa_call2 / hexa_call1. clang then fails because
 * `setenv` collides with the libc prototype and `exec_capture` has no
 * declaration at all — only `hx_setenv` / `hx_exec_capture` (TAG_FN
 * globals, see _hexa_init_fn_shims) exist under the prefix.
 *
 * These macros are placed at the very end of runtime.c so:
 *   - the real libc `setenv(3)` call inside hexa_setenv() (line ~5721)
 *     is already compiled against the libc prototype;
 *   - user code generated by hexa_v2 (appended AFTER `#include "runtime.c"`
 *     in app.c) sees the prefixed HexaVal globals.
 *
 * Remove once hexa_v2 is regenerated from current codegen_c2.hexa.
 * See docs/hexa_lang_beta_main_acceleration_20260422.md — Phase 2 regen.
 * ═══════════════════════════════════════════════════════════════════ */
