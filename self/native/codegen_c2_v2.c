...
//
// ─────────────────────────────────────────────────────────────────────────
// LEGACY PARITY ONLY — SEE self/native/hexa_cc.c FOR LIVE BUILD
// ─────────────────────────────────────────────────────────────────────────
// This file is NOT referenced by any build script, CI workflow, or test
// harness. The live toolchain builds from self/native/hexa_cc.c (consolidated
// successor). Scheduled for hard-delete at doc/plans/runtime_c_purge.md
// Phase C (alongside lexer_v2.c / parser_v2.c / type_checker_v2.c).
// SSOT = self/codegen_c2.hexa. Do NOT hand-patch — regen only.
// See: self/native/codegen_c2_v2.c.hexanoport for full disposition.
// ─────────────────────────────────────────────────────────────────────────
#include "runtime.c"
...
HexaVal gen2_expr(HexaVal node) {
    ...
    if (hexa_truthy(hexa_eq(k, hexa_str("Call")))) {
        ...
        if (hexa_truthy(hexa_bool(hexa_truthy(hexa_bool(!hexa_truthy(hexa_eq(hexa_type_of(callee), hexa_str("string"))))) && hexa_truthy(hexa_eq(hexa_map_get(callee, "kind"), hexa_str("Ident")))))) {
            ...
            /* FIX (2026-04-20): variadic println/print — match hexa_cc.c fix.
             * Was dropping args[1..]; now chains hexa_print_val per arg and
             * routes the final through hexa_println (single trailing \n). */
            if (hexa_truthy(hexa_eq(name, hexa_str("println")))) {
                HexaVal _pln_args = hexa_map_get(node, "args");
                int64_t _pln_n = hexa_len(_pln_args);
                if (_pln_n == 0) {
                    return hexa_str("(printf(\"\\n\"), hexa_void())");
                }
                if (_pln_n == 1) {
                    return hexa_add(hexa_add(hexa_str("hexa_println("), gen2_expr(hexa_index_get(_pln_args, hexa_int(0)))), hexa_str(")"));
                }
            ...
            if (hexa_truthy(hexa_eq(name, hexa_str("print")))) {
                HexaVal _prn_args = hexa_map_get(node, "args");
                int64_t _prn_n = hexa_len(_prn_args);
                if (_prn_n == 0) {
                    return hexa_str("hexa_void()");
                }
                if (_prn_n == 1) {
                    return hexa_add(hexa_add(hexa_str("(hexa_print_val("), gen2_expr(hexa_index_get(_prn_args, hexa_int(0)))), hexa_str("), hexa_void())"));
                }
            ...
            if (hexa_truthy(hexa_eq(name, hexa_str("eprintln")))) {
                HexaVal _epl_args = hexa_map_get(node, "args");
                int64_t _epl_n = hexa_len(_epl_args);
                if (_epl_n == 0) {
                    return hexa_str("(fprintf(stderr, \"\\n\"), hexa_void())");
                }
                if (_epl_n == 1) {
                    return hexa_add(hexa_add(hexa_str("(hexa_eprint_val("), gen2_expr(hexa_index_get(_epl_args, hexa_int(0)))), hexa_str("), fprintf(stderr, \"\\n\"), hexa_void())"));
                }
