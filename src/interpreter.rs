...
pub struct Interpreter {
    ...
    // Backward/autograd tape
    tape_counter: usize,
    tapes: HashMap<usize, Vec<(String, Vec<usize>, usize, Vec<Vec<f64>>)>>,  // tape_id -> entries
    tape_tensors: HashMap<usize, std::collections::HashMap<usize, Vec<f64>>>,
    tape_grads: HashMap<usize, std::collections::HashMap<usize, Vec<f64>>>,
    tape_tensor_counter: HashMap<usize, usize>,  // fn_name -> deprecation message  // name -> (params, current_body, generation)
...
impl Interpreter {
    pub fn new() -> Self {
        Self {
            ...
            tape_counter: 0,
            tapes: HashMap::new(),
            tape_tensors: HashMap::new(),
            tape_grads: HashMap::new(),
            tape_tensor_counter: HashMap::new(),
            #[cfg(not(target_arch = "wasm32"))]
    ...
    fn call_builtin(&mut self, name: &str, args: Vec<Value>) -> Result<Value, HexaError> {
        ...
        match name {
            ...
            // ── Spectrum Analysis (Φ frequency) ──
            "dft" => {
                // dft(signal, n) → [real_parts, imag_parts] of DFT
                if args.len() < 2 { return Err(self.type_err("dft() requires (signal, n)".into())); }
                if let (Value::Array(sig), Value::Int(n)) = (&args[0], &args[1]) {
                    let n = *n as usize;
                    let vals: Vec<f64> = sig.iter().map(|v| match v { Value::Float(f) => *f, Value::Int(i) => *i as f64, _ => 0.0 }).collect();
                    let mut real = Vec::with_capacity(n);
                    let mut imag = Vec::with_capacity(n);
                    for k in 0..n {
                        let mut re = 0.0f64;
                        let mut im = 0.0f64;
                        for t in 0..vals.len().min(n) {
                            let angle = -2.0 * std::f64::consts::PI * k as f64 * t as f64 / n as f64;
                            re += vals[t] * angle.cos();
                            im += vals[t] * angle.sin();
                        }
                        real.push(Value::Float(re));
                        imag.push(Value::Float(im));
                    }
                    Ok(Value::Array(vec![Value::Array(real), Value::Array(imag)]))
                } else { Err(self.type_err("dft() type error".into())) }
            }
            "power_spectrum" => {
                // power_spectrum(signal, n) → |DFT|² magnitude array
                if args.len() < 2 { return Err(self.type_err("power_spectrum() requires (signal, n)".into())); }
                if let (Value::Array(sig), Value::Int(n)) = (&args[0], &args[1]) {
                    let n = *n as usize;
                    let vals: Vec<f64> = sig.iter().map(|v| match v { Value::Float(f) => *f, Value::Int(i) => *i as f64, _ => 0.0 }).collect();
                    let mut power = Vec::with_capacity(n / 2 + 1);
                    for k in 0..=n / 2 {
                        let mut re = 0.0f64;
                        let mut im = 0.0f64;
                        for t in 0..vals.len().min(n) {
                        ...
                        power.push(Value::Float(re * re + im * im));
                    }
                    Ok(Value::Array(power))
                } else { Err(self.type_err("power_spectrum() type error".into())) }
            }
            // ── Anomaly Detection ──
            "z_score" => {
                // z_score(array) → per-element z-scores
                if args.is_empty() { return Err(self.type_err("z_score() requires array".into())); }
                if let Value::Array(a) = &args[0] {
                    let vals: Vec<f64> = a.iter().map(|v| match v { Value::Float(f) => *f, Value::Int(i) => *i as f64, _ => 0.0 }).collect();
                    let n = vals.len() as f64;
                    let mean: f64 = vals.iter().sum::<f64>() / n;
                    let std = (vals.iter().map(|&x| (x - mean) * (x - mean)).sum::<f64>() / n).sqrt().max(1e-10);
                    let zs: Vec<Value> = vals.iter().map(|&x| Value::Float((x - mean) / std)).collect();
                    Ok(Value::Array(zs))
                } else { Err(self.type_err("z_score() type error".into())) }
            }
            "detect_anomalies" => {
                // detect_anomalies(array, threshold) → indices where |z-score| > threshold
                if args.len() < 2 { return Err(self.type_err("detect_anomalies() requires (array, threshold)".into())); }
                if let (Value::Array(a), Value::Float(thresh)) = (&args[0], &args[1]) {
                    let vals: Vec<f64> = a.iter().map(|v| match v { Value::Float(f) => *f, Value::Int(i) => *i as f64, _ => 0.0 }).collect();
                    let n = vals.len() as f64;
                    let mean: f64 = vals.iter().sum::<f64>() / n;
                    let std = (vals.iter().map(|&x| (x - mean) * (x - mean)).sum::<f64>() / n).sqrt().max(1e-10);
                    let indices: Vec<Value> = vals.iter().enumerate()
                        .filter(|(_, &x)| ((x - mean) / std).abs() > *thresh)
                        .map(|(i, _)| Value::Int(i as i64))
                        .collect();
                    Ok(Value::Array(indices))
                } else { Err(self.type_err("detect_anomalies() type error".into())) }
            }
            // ── Model Ensemble ──
            "ensemble_avg" => {
                // ensemble_avg(model_outputs) → element-wise average of multiple model outputs
                // model_outputs: [n_models × dim] flat
                if args.len() < 3 { return Err(self.type_err("ensemble_avg() requires (outputs, n_models, dim)".into())); }
                if let (Value::Array(out), Value::Int(nm), Value::Int(dim)) = (&args[0], &args[1], &args[2]) {
                    let (nm, dim) = (*nm as usize, *dim as usize);
                    let mut avg = vec![0.0f64; dim];
                    for m in 0..nm {
                        for d in 0..dim {
                            avg[d] += match &out[m * dim + d] { Value::Float(f) => *f, Value::Int(i) => *i as f64, _ => 0.0 };
                        }
                    }
                    for d in 0..dim { avg[d] /= nm as f64; }
                    Ok(Value::Array(avg.into_iter().map(Value::Float).collect()))
                } else { Err(self.type_err("ensemble_avg() type error".into())) }
            }
            "ensemble_vote" => {
                // ensemble_vote(predictions) → majority vote (most common prediction)
                if args.is_empty() { return Err(self.type_err("ensemble_vote() requires array".into())); }
                if let Value::Array(preds) = &args[0] {
                    let mut counts = std::collections::HashMap::new();
                    for p in preds {
                        let v = match p { Value::Int(i) => *i, _ => 0 };
                        *counts.entry(v).or_insert(0) += 1;
                    }
                    let winner = counts.into_iter().max_by_key(|(_, c)| *c).map(|(v, _)| v).unwrap_or(0);
                    Ok(Value::Int(winner))
                } else { Err(self.type_err("ensemble_vote() type error".into())) }
            }
            // ── Reservoir Computing (Echo State Network) ──
            "reservoir_step" => {
                // reservoir_step(input, state, W_in, W_res, dim_in, dim_res, leak) → new_state
                // ESN: state = (1-α)*state + α*tanh(W_in@input + W_res@state)
                if args.len() < 7 { return Err(self.type_err("reservoir_step() requires 7 args".into())); }
                if let (Value::Array(inp), Value::Array(state), Value::Array(win), Value::Array(wres)) = (&args[0], &args[1], &args[2], &args[3]) {
                    let din = match &args[4] { Value::Int(i) => *i as usize, _ => 0 };
                    let dres = match &args[5] { Value::Int(i) => *i as usize, _ => 0 };
                    let leak = match &args[6] { Value::Float(f) => *f, _ => 0.3 };
                    // W_in @ input
                    let mut drive = vec![0.0f64; dres];
                    for r in 0..dres {
                        for c in 0..din {
                            let w = match &win[r * din + c] { Value::Float(f) => *f, _ => 0.0 };
                            let x = match &inp[c] { Value::Float(f) => *f, _ => 0.0 };
                            drive[r] += w * x;
                        }
                    }
                    // W_res @ state
                    for r in 0..dres {
                        for c in 0..dres {
                            let w = match &wres[r * dres + c] { Value::Float(f) => *f, _ => 0.0 };
                            let s = match &state[c] { Value::Float(f) => *f, _ => 0.0 };
                            drive[r] += w * s;
                        }
                    }
                    // Leaky integration: (1-α)*old + α*tanh(drive)
                    let mut new_state = Vec::with_capacity(dres);
                    for r in 0..dres {
                        let old = match &state[r] { Value::Float(f) => *f, _ => 0.0 };
                        new_state.push(Value::Float((1.0 - leak) * old + leak * drive[r].tanh()));
                    }
                    Ok(Value::Array(new_state))
                } else { Err(self.type_err("reservoir_step() type error".into())) }
            }
            // ── Graph Message Passing (GNN on topology) ──
            "message_pass" => {
                // message_pass(node_features, adjacency, n_nodes, dim, aggregation)
                // Each node aggregates neighbor features: new_i = agg(neighbor features of i)
                // aggregation: "mean" or "sum"
                if args.len() < 4 { return Err(self.type_err("message_pass() requires (features, adj, n_nodes, dim)".into())); }
                if let (Value::Array(feats), Value::Array(adj), Value::Int(n), Value::Int(dim)) = (&args[0], &args[1], &args[2], &args[3]) {
                    let (n, dim) = (*n as usize, *dim as usize);
                    let use_mean = args.len() > 4 && matches!(&args[4], Value::Str(s) if s == "mean");
                    let mut output = Vec::with_capacity(n * dim);
                    for i in 0..n {
                        let mut agg = vec![0.0f64; dim];
                        let mut count = 0usize;
                        for j in 0..n {
                            let edge = match &adj[i * n + j] { Value::Float(f) => *f, Value::Int(x) => *x as f64, _ => 0.0 };
                            if edge > 0.5 {
                                for d in 0..dim {
                                    let v = match &feats[j * dim + d] { Value::Float(f) => *f, Value::Int(x) => *x as f64, _ => 0.0 };
                                    agg[d] += v * edge;
                                }
                                count += 1;
                            }
                        ...
                        if use_mean && count > 0 {
                            for d in 0..dim { agg[d] /= count as f64; }
                        }
                        for d in 0..dim { output.push(Value::Float(agg[d])); }
                    }
                    Ok(Value::Array(output))
                } else { Err(self.type_err("message_pass() type error".into())) }
            }
            "graph_attention" => {
                // graph_attention(features, adj, W, n_nodes, dim) → attention-weighted message passing
                if args.len() < 5 { return Err(self.type_err("graph_attention() requires 5 args".into())); }
                if let (Value::Array(feats), Value::Array(adj), Value::Array(w), Value::Int(n), Value::Int(dim)) = (&args[0], &args[1], &args[2], &args[3], &args[4]) {
                    let (n, dim) = (*n as usize, *dim as usize);
                    let mut output = Vec::with_capacity(n * dim);
                    for i in 0..n {
                        // Transform node i
                        ...
                        for d in 0..dim {
                            for k in 0..dim {
                                let wv = match &w[d * dim + k] { Value::Float(f) => *f, _ => 0.0 };
                                let fv = match &feats[i * dim + k] { Value::Float(f) => *f, _ => 0.0 };
                                hi[d] += wv * fv;
                            }
                        }
                        // Attention scores with neighbors
                        let mut scores = Vec::new();
                        let mut neighbors = Vec::new();
                        for j in 0..n {
                            let edge = match &adj[i * n + j] { Value::Float(f) => *f, _ => 0.0 };
                            if edge > 0.5 || i == j {
                                let mut hj = vec![0.0f64; dim];
                                for d in 0..dim {
                                    for k in 0..dim {
                                        let wv = match &w[d * dim + k] { Value::Float(f) => *f, _ => 0.0 };
                                        let fv = match &feats[j * dim + k] { Value::Float(f) => *f, _ => 0.0 };
                                        hj[d] += wv * fv;
                                    }
                                }
                                let score: f64 = hi.iter().zip(hj.iter()).map(|(a, b)| a * b).sum();
                                scores.push(score);
                                neighbors.push(j);
                            }
                        }
                        ...
                        let exps: Vec<f64> = scores.iter().map(|s| (s - max_s).exp()).collect();
                        let sum_e: f64 = exps.iter().sum();
                        let mut node_out = vec![0.0f64; dim];
                        for (idx, &j) in neighbors.iter().enumerate() {
                            let w_attn = exps[idx] / sum_e;
                            for d in 0..dim {
                                let fv = match &feats[j * dim + d] { Value::Float(f) => *f, _ => 0.0 };
                                node_out[d] += w_attn * fv;
                            }
                        }
                        for d in 0..dim { output.push(Value::Float(node_out[d])); }
                    }
                    Ok(Value::Array(output))
                } else { Err(self.type_err("graph_attention() type error".into())) }
            }
            // ── Reinforcement Learning ──
            "policy_gradient" => {
                // policy_gradient(log_probs, rewards, baseline) → policy gradient estimates
                // REINFORCE: g_i = (R_i - baseline) * log_prob_i
                if args.len() < 2 { return Err(self.type_err("policy_gradient() requires (log_probs, rewards)".into())); }
                if let (Value::Array(lp), Value::Array(rew)) = (&args[0], &args[1]) {
                    let baseline = if args.len() > 2 { match &args[2] { Value::Float(f) => *f, _ => 0.0 } } else {
                        // Auto-baseline: mean of rewards
                        let sum: f64 = rew.iter().map(|v| match v { Value::Float(f) => *f, Value::Int(i) => *i as f64, _ => 0.0 }).sum();
                        sum / rew.len().max(1) as f64
                    };
                    let grads: Vec<Value> = lp.iter().zip(rew.iter()).map(|(l, r)| {
                        let lv = match l { Value::Float(f) => *f, Value::Int(i) => *i as f64, _ => 0.0 };
                        let rv = match r { Value::Float(f) => *f, Value::Int(i) => *i as f64, _ => 0.0 };
                        Value::Float((rv - baseline) * lv)
                    }).collect();
                    Ok(Value::Array(grads))
                } else { Err(self.type_err("policy_gradient() type error".into())) }
            }
            "discount_rewards" => {
                // discount_rewards(rewards, gamma) → discounted cumulative rewards
                if args.len() < 2 { return Err(self.type_err("discount_rewards() requires (rewards, gamma)".into())); }
                if let (Value::Array(rew), Value::Float(gamma)) = (&args[0], &args[1]) {
                    let n = rew.len();
                    let mut discounted = vec![0.0f64; n];
                    let mut running = 0.0f64;
                    for i in (0..n).rev() {
                        let r = match &rew[i] { Value::Float(f) => *f, Value::Int(x) => *x as f64, _ => 0.0 };
                        running = r + gamma * running;
                        discounted[i] = running;
                    }
                    Ok(Value::Array(discounted.into_iter().map(Value::Float).collect()))
                } else { Err(self.type_err("discount_rewards() type error".into())) }
            }
            // ── Memory Systems (Anima working/episodic) ──
            "memory_write" => {
                // memory_write(memory, key, value, dim) → updated memory with new entry appended
                // memory: [n_entries × (dim_key + dim_val)] flat
                if args.len() < 4 { return Err(self.type_err("memory_write() requires (memory, key, value, dim)".into())); }
                if let (Value::Array(mem), Value::Array(key), Value::Array(val), Value::Int(_dim)) = (&args[0], &args[1], &args[2], &args[3]) {
                    let mut result = mem.clone();
                    result.extend(key.iter().cloned());
                    result.extend(val.iter().cloned());
                    Ok(Value::Array(result))
                } else { Err(self.type_err("memory_write() type error".into())) }
            }
            "memory_read" => {
                // memory_read(memory, query, key_dim, val_dim, n_entries) → soft attention read
                // Computes cosine similarity with all keys, softmax-weighted value retrieval
                if args.len() < 5 { return Err(self.type_err("memory_read() requires 5 args".into())); }
                if let (Value::Array(mem), Value::Array(query), Value::Int(kd), Value::Int(vd), Value::Int(ne)) = (&args[0], &args[1], &args[2], &args[3], &args[4]) {
                    let (kd, vd, ne) = (*kd as usize, *vd as usize, *ne as usize);
                    let entry_size = kd + vd;
                    // Compute attention scores
                    let mut scores = Vec::with_capacity(ne);
                    for e in 0..ne {
                        let mut dot = 0.0f64;
                        let mut nk = 0.0f64;
                        let mut nq = 0.0f64;
                        for d in 0..kd {
                            let k = match &mem[e * entry_size + d] { Value::Float(f) => *f, _ => 0.0 };
                            let q = match &query[d] { Value::Float(f) => *f, _ => 0.0 };
                            dot += k * q;
                            nk += k * k;
                            nq += q * q;
                        }
                        let denom = (nk * nq).sqrt().max(1e-10);
                    ...
                    let mut value = vec![0.0f64; vd];
                    for e in 0..ne {
                        let w = exps[e] / sum_e;
                        for d in 0..vd {
                            let v = match &mem[e * entry_size + kd + d] { Value::Float(f) => *f, _ => 0.0 };
                            value[d] += w * v;
                        }
                    }
                    Ok(Value::Array(value.into_iter().map(Value::Float).collect()))
                } else { Err(self.type_err("memory_read() type error".into())) }
            }
            // ── VAE: Variational Autoencoder ──
            "reparameterize" => {
                // reparameterize(mu, log_var) → z = mu + exp(0.5*log_var) * eps
                // eps ~ N(0,1) deterministic from position
                if args.len() < 2 { return Err(self.type_err("reparameterize() requires (mu, log_var)".into())); }
                if let (Value::Array(mu), Value::Array(lv)) = (&args[0], &args[1]) {
                    let result: Vec<Value> = mu.iter().zip(lv.iter()).enumerate().map(|(i, (m, l))| {
                        let mf = match m { Value::Float(f) => *f, _ => 0.0 };
                        let lvf = match l { Value::Float(f) => *f, _ => 0.0 };
                        let eps = ((i * 2654435761 + self.current_line * 1013904223) % 10000) as f64 / 10000.0 - 0.5;
                        Value::Float(mf + (0.5 * lvf).exp() * eps)
                    }).collect();
                    Ok(Value::Array(result))
                } else { Err(self.type_err("reparameterize() type error".into())) }
            }
            "kl_loss_vae" => {
                // kl_loss_vae(mu, log_var) → -0.5 * mean(1 + log_var - mu² - exp(log_var))
                if args.len() < 2 { return Err(self.type_err("kl_loss_vae() requires (mu, log_var)".into())); }
                if let (Value::Array(mu), Value::Array(lv)) = (&args[0], &args[1]) {
                    let n = mu.len() as f64;
                    let kl: f64 = mu.iter().zip(lv.iter()).map(|(m, l)| {
                        let mf = match m { Value::Float(f) => *f, _ => 0.0 };
                        let lvf = match l { Value::Float(f) => *f, _ => 0.0 };
                        -0.5 * (1.0 + lvf - mf * mf - lvf.exp())
                    }).sum();
                    Ok(Value::Float(kl / n))
                } else { Err(self.type_err("kl_loss_vae() type error".into())) }
            }
            // ── Cross-Attention (Encoder-Decoder) ──
            "cross_attention" => {
                // cross_attention(Q, K_enc, V_enc, q_len, kv_len, dim) → decoder attends to encoder
                if args.len() < 6 { return Err(self.type_err("cross_attention() requires 6 args".into())); }
                if let (Value::Array(q), Value::Array(k), Value::Array(v), Value::Int(ql), Value::Int(kvl), Value::Int(dim)) = (&args[0], &args[1], &args[2], &args[3], &args[4], &args[5]) {
                    let (ql, kvl, dim) = (*ql as usize, *kvl as usize, *dim as usize);
                    let scale = 1.0 / (dim as f64).sqrt();
                    let mut output = Vec::with_capacity(ql * dim);
                    for i in 0..ql {
                        let mut scores = Vec::with_capacity(kvl);
                        for j in 0..kvl {
                            let mut dot = 0.0f64;
                            for d in 0..dim {
                                let qv = match &q[i * dim + d] { Value::Float(f) => *f, _ => 0.0 };
                                let kv = match &k[j * dim + d] { Value::Float(f) => *f, _ => 0.0 };
                                dot += qv * kv;
                            }
                            scores.push(dot * scale);
                        }
                        ...
                        for d in 0..dim {
                            let mut val = 0.0f64;
                            for j in 0..kvl {
                                let vv = match &v[j * dim + d] { Value::Float(f) => *f, _ => 0.0 };
                                val += (exps[j] / sum_e) * vv;
                            }
                            output.push(Value::Float(val));
                        }
                    ...
                    Ok(Value::Array(output))
                } else { Err(self.type_err("cross_attention() type error".into())) }
            }
            // ── Diffusion: Noise Schedule ──
            "noise_schedule_linear" => {
                // noise_schedule_linear(n_steps, beta_start, beta_end) → [betas, alphas, alpha_bars]
                if args.len() < 3 { return Err(self.type_err("noise_schedule_linear() requires (steps, start, end)".into())); }
                if let (Value::Int(n), Value::Float(start), Value::Float(end)) = (&args[0], &args[1], &args[2]) {
                    let n = *n as usize;
                    let mut betas = Vec::with_capacity(n);
                    let mut alphas = Vec::with_capacity(n);
                    let mut alpha_bars = Vec::with_capacity(n);
                    let mut alpha_bar = 1.0f64;
                    for t in 0..n {
                        let beta = start + (end - start) * t as f64 / (n - 1).max(1) as f64;
                        let alpha = 1.0 - beta;
                        alpha_bar *= alpha;
                        betas.push(Value::Float(beta));
                        alphas.push(Value::Float(alpha));
                        alpha_bars.push(Value::Float(alpha_bar));
                    }
                    Ok(Value::Array(vec![Value::Array(betas), Value::Array(alphas), Value::Array(alpha_bars)]))
                } else { Err(self.type_err("noise_schedule_linear() type error".into())) }
            }
            "add_noise" => {
                // add_noise(x, noise, alpha_bar_t) → sqrt(ᾱ)*x + sqrt(1-ᾱ)*noise
                if args.len() < 3 { return Err(self.type_err("add_noise() requires (x, noise, alpha_bar)".into())); }
                if let (Value::Array(x), Value::Array(noise), Value::Float(ab)) = (&args[0], &args[1], &args[2]) {
                    let sqrt_ab = ab.sqrt();
                    let sqrt_1mab = (1.0 - ab).sqrt();
                    let result: Vec<Value> = x.iter().zip(noise.iter()).map(|(xv, nv)| {
                        let xf = match xv { Value::Float(f) => *f, _ => 0.0 };
                        let nf = match nv { Value::Float(f) => *f, _ => 0.0 };
                        Value::Float(sqrt_ab * xf + sqrt_1mab * nf)
                    }).collect();
                    Ok(Value::Array(result))
                } else { Err(self.type_err("add_noise() type error".into())) }
            }
            // ── Genetic Algorithm (Anima @evolve population-level) ──
            "ga_crossover" => {
                // ga_crossover(parent1, parent2, crossover_point) → [child1, child2]
                if args.len() < 3 { return Err(self.type_err("ga_crossover() requires (p1, p2, point)".into())); }
                if let (Value::Array(p1), Value::Array(p2), Value::Int(cp)) = (&args[0], &args[1], &args[2]) {
                    let cp = *cp as usize;
                    let mut c1 = p1[..cp].to_vec();
                    c1.extend_from_slice(&p2[cp..]);
                    let mut c2 = p2[..cp].to_vec();
                    c2.extend_from_slice(&p1[cp..]);
                    Ok(Value::Array(vec![Value::Array(c1), Value::Array(c2)]))
                } else { Err(self.type_err("ga_crossover() type error".into())) }
            }
            "ga_mutate" => {
                // ga_mutate(individual, rate, scale) → mutated individual
                if args.len() < 3 { return Err(self.type_err("ga_mutate() requires (individual, rate, scale)".into())); }
                if let (Value::Array(ind), Value::Float(rate), Value::Float(scale)) = (&args[0], &args[1], &args[2]) {
                    let result: Vec<Value> = ind.iter().enumerate().map(|(i, v)| {
                        let vf = match v { Value::Float(f) => *f, Value::Int(x) => *x as f64, _ => 0.0 };
                        let hash = ((i * 2654435761 + self.current_line * 1013904223) % 1000) as f64 / 1000.0;
                        if hash < *rate {
                            let noise = ((i * 1103515245 + 12345) % 10000) as f64 / 10000.0 - 0.5;
                            Value::Float(vf + noise * 2.0 * scale)
                        } else {
                            Value::Float(vf)
                        }
                    }).collect();
                    Ok(Value::Array(result))
                } else { Err(self.type_err("ga_mutate() type error".into())) }
            }
            "ga_select" => {
                // ga_select(population, fitnesses, n_select) → indices of top-n individuals
                if args.len() < 3 { return Err(self.type_err("ga_select() requires (pop, fitness, n)".into())); }
                if let (Value::Array(_pop), Value::Array(fit), Value::Int(n)) = (&args[0], &args[1], &args[2]) {
                    let n = *n as usize;
                    let mut indexed: Vec<(usize, f64)> = fit.iter().enumerate().map(|(i, v)| {
                        (i, match v { Value::Float(f) => *f, Value::Int(x) => *x as f64, _ => 0.0 })
                    }).collect();
                    indexed.sort_by(|a, b| b.1.partial_cmp(&a.1).unwrap_or(std::cmp::Ordering::Equal));
                    let selected: Vec<Value> = indexed[..n.min(indexed.len())].iter().map(|(i, _)| Value::Int(*i as i64)).collect();
                    Ok(Value::Array(selected))
                } else { Err(self.type_err("ga_select() type error".into())) }
            }
            // ── Cellular Automaton (emergence patterns) ──
            "ca_step_1d" => {
                // ca_step_1d(state, rule) → next state (1D elementary CA)
                // state: array of 0/1, rule: 0-255 (Wolfram rule number)
                if args.len() < 2 { return Err(self.type_err("ca_step_1d() requires (state, rule)".into())); }
                if let (Value::Array(state), Value::Int(rule)) = (&args[0], &args[1]) {
                    let rule = *rule as u8;
                    let n = state.len();
                    let s: Vec<u8> = state.iter().map(|v| match v { Value::Int(i) => *i as u8, Value::Float(f) => if *f > 0.5 { 1 } else { 0 }, _ => 0 }).collect();
                    let mut next = Vec::with_capacity(n);
                    for i in 0..n {
                        let left = if i > 0 { s[i-1] } else { s[n-1] }; // wrap
                        let center = s[i];
                        let right = if i < n-1 { s[i+1] } else { s[0] };
                        let pattern = (left << 2) | (center << 1) | right;
                        let bit = (rule >> pattern) & 1;
                        next.push(Value::Int(bit as i64));
                    }
                    Ok(Value::Array(next))
                } else { Err(self.type_err("ca_step_1d() type error".into())) }
            }
            // ── Particle Swarm Optimization ──
            "pso_step" => {
                // pso_step(positions, velocities, p_best, g_best, w, c1, c2, dim, n_particles)
                // v = w*v + c1*r1*(pbest-x) + c2*r2*(gbest-x), x = x + v
                // Returns [new_positions, new_velocities] flat
                if args.len() < 9 { return Err(self.type_err("pso_step() requires 9 args".into())); }
                if let (Value::Array(pos), Value::Array(vel), Value::Array(pb), Value::Array(gb)) = (&args[0], &args[1], &args[2], &args[3]) {
                    let w = match &args[4] { Value::Float(f) => *f, _ => 0.7 };
                    let c1 = match &args[5] { Value::Float(f) => *f, _ => 1.5 };
                    let c2 = match &args[6] { Value::Float(f) => *f, _ => 1.5 };
                    let dim = match &args[7] { Value::Int(i) => *i as usize, _ => 0 };
                    let np = match &args[8] { Value::Int(i) => *i as usize, _ => 0 };
                    let mut new_pos = Vec::with_capacity(np * dim);
                    let mut new_vel = Vec::with_capacity(np * dim);
                    for p in 0..np {
                        for d in 0..dim {
                            let idx = p * dim + d;
                            let x = match &pos[idx] { Value::Float(f) => *f, _ => 0.0 };
                            let v = match &vel[idx] { Value::Float(f) => *f, _ => 0.0 };
                            let pb_v = match &pb[idx] { Value::Float(f) => *f, _ => 0.0 };
                            let gb_v = match &gb[d] { Value::Float(f) => *f, _ => 0.0 };
                            let r1 = ((p * 2654435761 + d * 1013904223 + self.current_line) % 1000) as f64 / 1000.0;
                            let r2 = ((p * 1103515245 + d * 12345 + self.current_line + 1) % 1000) as f64 / 1000.0;
                            let nv = w * v + c1 * r1 * (pb_v - x) + c2 * r2 * (gb_v - x);
                            new_vel.push(Value::Float(nv));
                            new_pos.push(Value::Float(x + nv));
                        }
                    }
                    let mut result = new_pos;
                    result.extend(new_vel);
                    Ok(Value::Array(result))
                } else { Err(self.type_err("pso_step() type error".into())) }
            }
            // ── Simulated Annealing ──
            "sa_step" => {
                // sa_step(current, candidate, current_cost, candidate_cost, temperature) → accept?
                // Returns [accepted_solution, accepted_cost, new_temperature]
                if args.len() < 5 { return Err(self.type_err("sa_step() requires 5 args".into())); }
                if let (Value::Array(cur), Value::Array(cand)) = (&args[0], &args[1]) {
                    let cur_cost = match &args[2] { Value::Float(f) => *f, Value::Int(i) => *i as f64, _ => 0.0 };
                    let cand_cost = match &args[3] { Value::Float(f) => *f, Value::Int(i) => *i as f64, _ => 0.0 };
                    let temp = match &args[4] { Value::Float(f) => *f, _ => 1.0 };
                    let delta = cand_cost - cur_cost;
                    let accept = if delta < 0.0 { true } else {
                        let prob = (-delta / temp.max(1e-10)).exp();
                        let hash = ((self.current_line * 2654435761 + cur.len() * 1013904223) % 1000) as f64 / 1000.0;
                        hash < prob
                    };
                    if accept {
                        Ok(Value::Array(vec![Value::Array(cand.clone()), Value::Float(cand_cost), Value::Float(temp * 0.95)]))
                    } else {
                        Ok(Value::Array(vec![Value::Array(cur.clone()), Value::Float(cur_cost), Value::Float(temp * 0.95)]))
                    }
                } else { Err(self.type_err("sa_step() type error".into())) }
            }
            // ── Knowledge Distillation ──
            "distillation_loss" => {
                // distillation_loss(student_logits, teacher_logits, target, temperature, alpha)
                // L = α*KL(softmax(t/T), softmax(s/T))*T² + (1-α)*CE(s, target)
                if args.len() < 5 { return Err(self.type_err("distillation_loss() requires 5 args".into())); }
                if let (Value::Array(sl), Value::Array(tl), Value::Int(target), Value::Float(temp), Value::Float(alpha)) = (&args[0], &args[1], &args[2], &args[3], &args[4]) {
                    let n = sl.len();
                    // Soft targets: softmax(teacher/T)
                    let t_vals: Vec<f64> = tl.iter().map(|v| match v { Value::Float(f) => f / temp, _ => 0.0 }).collect();
                    let s_vals: Vec<f64> = sl.iter().map(|v| match v { Value::Float(f) => f / temp, _ => 0.0 }).collect();
                    let t_max = t_vals.iter().cloned().fold(f64::NEG_INFINITY, f64::max);
                    let s_max = s_vals.iter().cloned().fold(f64::NEG_INFINITY, f64::max);
                    let t_exp: Vec<f64> = t_vals.iter().map(|v| (v - t_max).exp()).collect();
                    let s_exp: Vec<f64> = s_vals.iter().map(|v| (v - s_max).exp()).collect();
                    let t_sum: f64 = t_exp.iter().sum();
                    let s_sum: f64 = s_exp.iter().sum();
                    // KL(teacher_soft || student_soft) * T²
                    let mut kl = 0.0f64;
                    for i in 0..n {
                        let tp = t_exp[i] / t_sum;
                        let sp = s_exp[i] / s_sum;
                        if tp > 1e-10 && sp > 1e-10 { kl += tp * (tp / sp).ln(); }
                    }
                    let soft_loss = kl * temp * temp;
                    // Hard CE
                    let hard_exp: Vec<f64> = sl.iter().map(|v| { let f = match v { Value::Float(f) => *f, _ => 0.0 }; (f - s_max * temp).exp() }).collect();
                    let hard_sum: f64 = hard_exp.iter().sum();
                    let hard_loss = -(hard_exp[*target as usize] / hard_sum).ln();
                    Ok(Value::Float(alpha * soft_loss + (1.0 - alpha) * hard_loss))
                } else { Err(self.type_err("distillation_loss() type error".into())) }
            }
            // ── Curriculum Learning ──
            "curriculum_weight" => {
                // curriculum_weight(difficulty, step, total_steps, strategy)
                // Returns weight [0,1] for this sample based on difficulty and progress
                // "linear": ramp from easy to hard, "exp": exponential, "step": discrete phases
                if args.len() < 4 { return Err(self.type_err("curriculum_weight() requires 4 args".into())); }
                let diff = match &args[0] { Value::Float(f) => *f, Value::Int(i) => *i as f64, _ => 0.0 };
                let step = match &args[1] { Value::Int(i) => *i as f64, Value::Float(f) => *f, _ => 0.0 };
                let total = match &args[2] { Value::Int(i) => *i as f64, Value::Float(f) => *f, _ => 1.0 };
                let strategy = if let Value::Str(s) = &args[3] { s.clone() } else { "linear".to_string() };
                let progress = (step / total).min(1.0);
                let threshold = match strategy.as_str() {
                    "exp" => 1.0 - (-3.0 * progress).exp(),
                    "step" => if progress < 0.33 { 0.33 } else if progress < 0.66 { 0.66 } else { 1.0 },
                    _ => progress, // linear
                };
                let weight = if diff <= threshold { 1.0 } else { (1.0 - (diff - threshold) / (1.0 - threshold + 1e-10)).max(0.0) };
                Ok(Value::Float(weight))
            }
            // ── Gradient Flow Analysis ──
            "grad_norm_per_layer" => {
                // grad_norm_per_layer(grads_list, layer_sizes) → [norm per layer]
                // Detects vanishing/exploding gradients
                if args.len() < 2 { return Err(self.type_err("grad_norm_per_layer() requires (grads, sizes)".into())); }
                if let (Value::Array(grads), Value::Array(sizes)) = (&args[0], &args[1]) {
                    let mut norms = Vec::new();
                    let mut offset = 0usize;
                    for sz in sizes {
                        let s = match sz { Value::Int(i) => *i as usize, _ => 0 };
                        let mut norm_sq = 0.0f64;
                        for i in offset..offset + s {
                            if i < grads.len() {
                                let g = match &grads[i] { Value::Float(f) => *f, _ => 0.0 };
                                norm_sq += g * g;
                            }
                        }
                        norms.push(Value::Float(norm_sq.sqrt()));
                        offset += s;
                    }
                    Ok(Value::Array(norms))
                } else { Err(self.type_err("grad_norm_per_layer() type error".into())) }
            }
            "diagnose_gradients" => {
                // diagnose_gradients(layer_norms) → "healthy" | "vanishing" | "exploding"
                if args.is_empty() { return Err(self.type_err("diagnose_gradients() requires array".into())); }
                if let Value::Array(norms) = &args[0] {
                    let vals: Vec<f64> = norms.iter().map(|v| match v { Value::Float(f) => *f, _ => 0.0 }).collect();
                    let max_n = vals.iter().cloned().fold(0.0f64, f64::max);
                    let min_n = vals.iter().cloned().fold(f64::INFINITY, f64::min);
                    let ratio = if min_n > 1e-10 { max_n / min_n } else { f64::INFINITY };
                    let diagnosis = if max_n > 100.0 { "exploding" }
                        else if min_n < 1e-7 { "vanishing" }
                        else if ratio > 1000.0 { "unstable" }
                        else { "healthy" };
                    Ok(Value::Str(diagnosis.to_string()))
                } else { Err(self.type_err("diagnose_gradients() type error".into())) }
            }
            // ── Model Comparison ──
            "compare_models" => {
                // compare_models(pred1, pred2, targets) → [acc1, acc2, agreement, p_value_approx]
                if args.len() < 3 { return Err(self.type_err("compare_models() requires (pred1, pred2, targets)".into())); }
                if let (Value::Array(p1), Value::Array(p2), Value::Array(targets)) = (&args[0], &args[1], &args[2]) {
                    let n = p1.len().min(p2.len()).min(targets.len());
                    let mut c1 = 0usize;
                    let mut c2 = 0usize;
                    let mut agree = 0usize;
                    for i in 0..n {
                        let v1 = match &p1[i] { Value::Int(x) => *x, _ => -1 };
                        let v2 = match &p2[i] { Value::Int(x) => *x, _ => -1 };
                        let t = match &targets[i] { Value::Int(x) => *x, _ => -2 };
                        if v1 == t { c1 += 1; }
                        if v2 == t { c2 += 1; }
                        if v1 == v2 { agree += 1; }
                    }
                    let acc1 = c1 as f64 / n as f64;
                    let acc2 = c2 as f64 / n as f64;
                    let agreement = agree as f64 / n as f64;
                    // McNemar-like: how often do they disagree?
                    let disagreement = 1.0 - agreement;
                    Ok(Value::Array(vec![
                        Value::Float(acc1), Value::Float(acc2),
                        Value::Float(agreement), Value::Float(disagreement),
                    ]))
                } else { Err(self.type_err("compare_models() type error".into())) }
            }
            // ═══ AI-NATIVE ALGORITHM REPLACEMENTS ═══
            // These change the ALGORITHM, not the implementation.
            // Target: Hexa interpreter → competitive with Rust+rayon / PyTorch GPU

            "sparse_matvec" => {
                // AI-native: skip zero entries entirely → O(nnz) instead of O(n×m)
                // For 70% sparse matrix: 3.3x speedup. For 90% sparse: 10x.
                if args.len() < 5 { return Err(self.type_err("sparse_matvec() requires (vals, col_idx, row_ptr, x, n_rows)".into())); }
                if let (Value::Array(vals), Value::Array(cols), Value::Array(rows), Value::Array(x), Value::Int(nr)) = (&args[0], &args[1], &args[2], &args[3], &args[4]) {
                    let nr = *nr as usize;
                    let mut result = Vec::with_capacity(nr);
                    for r in 0..nr {
                        let start = match &rows[r] { Value::Int(i) => *i as usize, _ => 0 };
                        let end = match &rows[r + 1] { Value::Int(i) => *i as usize, _ => 0 };
                        let mut sum = 0.0f64;
                        for k in start..end {
                            let v = match &vals[k] { Value::Float(f) => *f, _ => 0.0 };
                            let c = match &cols[k] { Value::Int(i) => *i as usize, _ => 0 };
                            let xv = match &x[c] { Value::Float(f) => *f, Value::Int(i) => *i as f64, _ => 0.0 };
                            sum += v * xv;
                        }
                        result.push(Value::Float(sum));
                    }
                    Ok(Value::Array(result))
                } else { Err(self.type_err("sparse_matvec() type error".into())) }
            }
            "to_csr" => {
                // Convert dense matrix to CSR (Compressed Sparse Row) format
                // Returns [values, col_indices, row_pointers]
                if args.len() < 3 { return Err(self.type_err("to_csr() requires (dense, rows, cols)".into())); }
                if let (Value::Array(dense), Value::Int(nr), Value::Int(nc)) = (&args[0], &args[1], &args[2]) {
                    let (nr, nc) = (*nr as usize, *nc as usize);
                    let mut vals = Vec::new();
                    let mut cols = Vec::new();
                    let mut row_ptr = vec![Value::Int(0)];
                    for r in 0..nr {
                        for c in 0..nc {
                            let v = match &dense[r * nc + c] { Value::Float(f) => *f, Value::Int(i) => *i as f64, _ => 0.0 };
                            if v.abs() > 1e-10 {
                                vals.push(Value::Float(v));
                                cols.push(Value::Int(c as i64));
                            }
                        }
                        row_ptr.push(Value::Int(vals.len() as i64));
                    }
                    Ok(Value::Array(vec![Value::Array(vals), Value::Array(cols), Value::Array(row_ptr)]))
                } else { Err(self.type_err("to_csr() type error".into())) }
            }
            "lsh_sim" => {
                // AI-native: Locality-Sensitive Hashing for approximate cosine similarity
                // O(n × d × h) instead of O(n² × d) for all-pairs
                // Returns approximate similarity matrix [n×n]
                if args.len() < 4 { return Err(self.type_err("lsh_sim() requires (features, n, dim, n_hashes)".into())); }
                if let (Value::Array(feats), Value::Int(n), Value::Int(dim), Value::Int(nh)) = (&args[0], &args[1], &args[2], &args[3]) {
                    let (n, dim, nh) = (*n as usize, *dim as usize, *nh as usize);
                    // Generate random hyperplanes (deterministic from dim+nh)
                    // Compute hash signatures
                    let mut signatures = vec![0u64; n];
                    for i in 0..n {
                        let mut sig = 0u64;
                        for h in 0..nh.min(64) {
                            let mut dot = 0.0f64;
                            for d in 0..dim {
                                let fv = match &feats[i * dim + d] { Value::Float(f) => *f, _ => 0.0 };
                                // Random hyperplane from hash
                                let plane = ((h * 2654435761 + d * 1013904223) % 10000) as f64 / 5000.0 - 1.0;
                                dot += fv * plane;
                            }
                            if dot > 0.0 { sig |= 1 << h; }
                        }
                    ...
                    // Approximate similarity = (matching bits) / n_hashes
                    let mut matrix = Vec::with_capacity(n * n);
                    for i in 0..n {
                        for j in 0..n {
                            let xor = signatures[i] ^ signatures[j];
                            let matching = nh - xor.count_ones() as usize;
                            matrix.push(Value::Float(matching as f64 / nh as f64));
                        }
                    }
                    Ok(Value::Array(matrix))
                } else { Err(self.type_err("lsh_sim() type error".into())) }
            }
            "random_projection" => {
                // AI-native: Johnson-Lindenstrauss dimensionality reduction
                // Reduce d-dim vectors to k-dim while preserving distances
                // O(n×d) → O(n×k) for subsequent operations, k << d
                if args.len() < 4 { return Err(self.type_err("random_projection() requires (data, n, d, k)".into())); }
                if let (Value::Array(data), Value::Int(n), Value::Int(d), Value::Int(k)) = (&args[0], &args[1], &args[2], &args[3]) {
                    ...
                    for i in 0..n {
                        for j in 0..k {
                            let mut sum = 0.0f64;
                            for dd in 0..d {
                                let v = match &data[i * d + dd] { Value::Float(f) => *f, _ => 0.0 };
                                // Sparse random projection: {-1, 0, +1} with prob {1/6, 2/3, 1/6}
                                let hash = (j * 2654435761 + dd * 1013904223 + i * 12345) % 6;
                                let proj = if hash == 0 { -1.0 } else if hash == 5 { 1.0 } else { 0.0 }; // √3 sparse
                                sum += v * proj;
                            }
                            result.push(Value::Float(sum * scale * 3.0f64.sqrt()));
                        }
                    }
                    Ok(Value::Array(result))
                } else { Err(self.type_err("random_projection() type error".into())) }
            }
            "sign_sgd" => {
                // AI-native: SignSGD — use only sign of gradient
                // 1-bit gradient: same convergence direction, O(1) memory per param
                // Equivalent to Adam with β→0: just the direction matters
                if args.len() < 3 { return Err(self.type_err("sign_sgd() requires (params, grads, lr)".into())); }
                if let (Value::Array(p), Value::Array(g), Value::Float(lr)) = (&args[0], &args[1], &args[2]) {
                    let result: Vec<Value> = p.iter().zip(g.iter()).map(|(pv, gv)| {
                        let pf = match pv { Value::Float(f) => *f, _ => 0.0 };
                        let gf = match gv { Value::Float(f) => *f, _ => 0.0 };
                        let sign = if gf > 0.0 { 1.0 } else if gf < 0.0 { -1.0 } else { 0.0 };
                        Value::Float(pf - lr * sign)
                    }).collect();
                    Ok(Value::Array(result))
                } else { Err(self.type_err("sign_sgd() type error".into())) }
            }
            "phi_fast" => {
                // AI-native: O(n×d) Φ proxy instead of O(n²×bins²) IIT
                // Uses variance decomposition: Φ ≈ global_var - weighted_faction_var + complexity_bonus
                // Single pass over all data, no pairwise computation
                if args.len() < 4 { return Err(self.type_err("phi_fast() requires (hiddens, n_cells, dim, n_factions)".into())); }
                if let (Value::Array(h), Value::Int(nc), Value::Int(dim), Value::Int(nf)) = (&args[0], &args[1], &args[2], &args[3]) {
                    ...
                    // Single pass: compute global mean, var, and per-faction stats simultaneously
                    let mut global_sum = vec![0.0f64; dim];
                    let mut global_sq = vec![0.0f64; dim];
                    let mut faction_sum = vec![vec![0.0f64; dim]; nf];
                    let mut faction_sq = vec![vec![0.0f64; dim]; nf];
                    let mut faction_n = vec![0usize; nf];
                    for c in 0..nc {
                        let f = c % nf;
                        faction_n[f] += 1;
                        for d in 0..dim {
                            let v = match &h[c * dim + d] { Value::Float(fv) => *fv, _ => 0.0 };
                            global_sum[d] += v;
                            global_sq[d] += v * v;
                            faction_sum[f][d] += v;
                            faction_sq[f][d] += v * v;
                    ...
                    for d in 0..dim {
                        ...
                        global_var += global_sq[d] / n - mean * mean;
                    }
                    global_var /= dim as f64;
                    let mut faction_var = 0.0f64;
                    for f in 0..nf {
                        if faction_n[f] == 0 { continue; }
                        let fn_f = faction_n[f] as f64;
                        let mut fvar = 0.0f64;
                        for d in 0..dim {
                            let mean = faction_sum[f][d] / fn_f;
                            fvar += faction_sq[f][d] / fn_f - mean * mean;
                        }
                        faction_var += fvar / dim as f64;
                    }
                    faction_var /= nf as f64;
                    // Complexity bonus: Shannon entropy of cell norms
                    let mut norms = Vec::with_capacity(nc);
                    ...
                    let phi = (global_var - faction_var).max(0.0) + 0.1 * entropy;
                    Ok(Value::Float(phi))
                } else { Err(self.type_err("phi_fast() type error".into())) }
            }
            // ═══ GPU-BEATING AI-NATIVE ALGORITHMS ═══
            // These algorithms reduce computation so much that
            // an interpreter beats GPU brute-force parallelism.

            "binary_matmul" => {
                // AI-native: Binary Neural Network matmul
                // Instead of float multiply-add: XNOR + popcount
                // 1-bit weights × 1-bit activations → 64 ops per u64 word
                // Theoretical: 64x fewer FLOPs than float matmul
                if args.len() < 5 { return Err(self.type_err("binary_matmul() requires (A_bits, B_bits, M, K, N)".into())); }
                if let (Value::Array(a), Value::Array(b), Value::Int(m), Value::Int(k), Value::Int(n)) = (&args[0], &args[1], &args[2], &args[3], &args[4]) {
                    let (m, k, n) = (*m as usize, *k as usize, *n as usize);
                    // Pack K dims into u64 words (K/64 words per row)
                    let words_k = (k + 63) / 64;
                    // A: [M × words_k], B: [N × words_k] (B is transposed)
                    let pack_row = |arr: &[Value], row: usize, cols: usize| -> Vec<u64> {
                        let mut words = vec![0u64; words_k];
                        for c in 0..cols {
                            let v = match &arr[row * cols + c] { Value::Float(f) => *f > 0.0, Value::Int(i) => *i > 0, _ => false };
                            if v { words[c / 64] |= 1u64 << (c % 64); }
                        }
                        words
                    };
                    let mut result = Vec::with_capacity(m * n);
                    for i in 0..m {
                        let a_words = pack_row(a, i, k);
                        for j in 0..n {
                            let b_words = pack_row(b, j, k);
                            // XNOR + popcount
                            let mut match_count = 0i64;
                            for w in 0..words_k {
                                let xnor = !(a_words[w] ^ b_words[w]);
                                match_count += xnor.count_ones() as i64;
                            }
                            // Convert to [-1, 1] range: (2*matches - K) / K
                            let val = (2 * match_count - k as i64) as f64 / k as f64;
                            result.push(Value::Float(val));
                        }
                    }
                    Ok(Value::Array(result))
                } else { Err(self.type_err("binary_matmul() type error".into())) }
            }
            "predictive_skip" => {
                // AI-native: Skip cells that haven't changed significantly
                // Only recompute cells where |new - old| > threshold
                // For 90% stable states: 10x fewer cell computations
                if args.len() < 4 { return Err(self.type_err("predictive_skip() requires (new, old, dim, threshold)".into())); }
                if let (Value::Array(new_h), Value::Array(old_h), Value::Int(dim), Value::Float(thresh)) = (&args[0], &args[1], &args[2], &args[3]) {
                    let dim = *dim as usize;
                    let n_cells = new_h.len() / dim;
                    let mut active_indices = Vec::new();
                    for c in 0..n_cells {
                        let mut delta_sq = 0.0f64;
                        for d in 0..dim {
                            let nv = match &new_h[c * dim + d] { Value::Float(f) => *f, _ => 0.0 };
                            let ov = match &old_h[c * dim + d] { Value::Float(f) => *f, _ => 0.0 };
                            delta_sq += (nv - ov) * (nv - ov);
                        }
                        if delta_sq.sqrt() > *thresh {
                            active_indices.push(Value::Int(c as i64));
                        }
                    }
                    Ok(Value::Array(active_indices))
                } else { Err(self.type_err("predictive_skip() type error".into())) }
            }
            "incremental_phi" => {
                // AI-native: O(1) Φ update instead of O(n×d) recompute
                // Maintains running statistics, only updates changed cells
                // When 90% cells unchanged: 10x faster than full recompute
                if args.len() < 7 { return Err(self.type_err("incremental_phi() requires 7 args".into())); }
                if let (Value::Array(hiddens), Value::Array(active), Value::Float(old_phi)) = (&args[0], &args[1], &args[2]) {
                    let old_global_var = match &args[3] { Value::Float(f) => *f, _ => 0.0 };
                    let old_faction_var = match &args[4] { Value::Float(f) => *f, _ => 0.0 };
                    let dim = match &args[5] { Value::Int(i) => *i as usize, _ => 0 };
                    let n_cells = match &args[6] { Value::Int(i) => *i as usize, _ => 0 };
                    if active.is_empty() {
                        // No cells changed → Φ unchanged (O(1)!)
                        return Ok(Value::Float(*old_phi));
                    }
                    let n_active = active.len();
                    let fraction_active = n_active as f64 / n_cells as f64;
                    if fraction_active > 0.5 {
                        // Too many changes → full recompute (fallback)
                        let result = self.call_builtin("phi_fast", vec![
                            Value::Array(hiddens.clone()), Value::Int(n_cells as i64),
                            Value::Int(dim as i64), Value::Int(4),
                        ])?;
                        return Ok(result);
                    }
                    // Incremental: estimate Φ change from active cells only
                    let mut delta_var = 0.0f64;
                    for idx_val in active.iter() {
                        let c = match idx_val { Value::Int(i) => *i as usize, _ => 0 };
                        let mut cell_var = 0.0f64;
                        for d in 0..dim {
                            let v = match &hiddens[c * dim + d] { Value::Float(f) => *f, _ => 0.0 };
                            cell_var += v * v;
                        }
                        delta_var += cell_var / dim as f64;
                    }
                    delta_var /= n_cells as f64;
                    let new_phi = old_phi + (delta_var - old_global_var * fraction_active) * 0.1;
                    Ok(Value::Float(new_phi.max(0.0)))
                } else { Err(self.type_err("incremental_phi() type error".into())) }
            }
            "sketch_matmul" => {
                // AI-native: Count-Sketch approximate matmul
                // Instead of O(M×K×N), uses O(M×s + K×s + s×N) where s << K
                // sketch_dim s=16 for K=128: 8x fewer FLOPs
                if args.len() < 6 { return Err(self.type_err("sketch_matmul() requires (A, B, M, K, N, sketch_dim)".into())); }
                if let (Value::Array(a), Value::Array(b), Value::Int(m), Value::Int(k), Value::Int(n), Value::Int(s)) = (&args[0], &args[1], &args[2], &args[3], &args[4], &args[5]) {
                    let (m, k, n, s) = (*m as usize, *k as usize, *n as usize, *s as usize);
                    // Sketch A: [M×K] → [M×s] by random hashing K columns into s buckets
                    let mut sa = vec![0.0f64; m * s];
                    for i in 0..m {
                        for j in 0..k {
                            let v = match &a[i * k + j] { Value::Float(f) => *f, _ => 0.0 };
                            let bucket = (j * 2654435761) % s;
                            let sign = if (j * 1013904223) % 2 == 0 { 1.0 } else { -1.0 };
                            sa[i * s + bucket] += v * sign;
                        }
                    }
                    // Sketch B: [K×N] → [s×N]
                    let mut sb = vec![0.0f64; s * n];
                    for j in 0..k {
                        let bucket = (j * 2654435761) % s;
                        let sign = if (j * 1013904223) % 2 == 0 { 1.0 } else { -1.0 };
                        for l in 0..n {
                            let v = match &b[j * n + l] { Value::Float(f) => *f, _ => 0.0 };
                            sb[bucket * n + l] += v * sign;
                        }
                    }
                    // Multiply sketches: [M×s] × [s×N] → [M×N]
                    let mut result = Vec::with_capacity(m * n);
                    for i in 0..m {
                        for j in 0..n {
                            let mut sum = 0.0f64;
                            for t in 0..s {
                                sum += sa[i * s + t] * sb[t * n + j];
                            }
                            result.push(Value::Float(sum));
                        }
                    }
                    Ok(Value::Array(result))
                } else { Err(self.type_err("sketch_matmul() type error".into())) }
            }
            // ═══ O(1) AMORTIZED CONSCIOUSNESS — BEAT GPU DEFINITIVELY ═══
            "consciousness_step_block" => {
                // BLOCK-LEVEL: Process only changed blocks of cells
                // Divide 64 cells into 8 blocks of 8. Only scan changed blocks.
                // Pre-maintain per-block checksums. O(n_blocks + active_blocks × block_size × dim)
                // For 1 block changed out of 8: ~8x faster than full scan
                if args.len() < 9 { return Err(self.type_err("consciousness_step_block() requires 9 args".into())); }
                if let (Value::Array(hiddens), Value::Array(block_checksums), Value::Array(old_block_checksums)) = (&args[0], &args[1], &args[2]) {
                    let n_cells = match &args[3] { Value::Int(i) => *i as usize, _ => 0 };
                    let dim = match &args[4] { Value::Int(i) => *i as usize, _ => 0 };
                    let block_size = match &args[5] { Value::Int(i) => *i as usize, _ => 8 };
                    let old_phi = match &args[6] { Value::Float(f) => *f, _ => 0.0 };
                    let old_tension = match &args[7] { Value::Float(f) => *f, _ => 0.0 };
                    let n_factions = match &args[8] { Value::Int(i) => *i as usize, _ => 2 };

                    let n_blocks = (n_cells + block_size - 1) / block_size;

                    // Phase 1: Compare block checksums O(n_blocks)
                    let mut changed_blocks = Vec::new();
                    for b in 0..n_blocks {
                        let new_cs = if b < block_checksums.len() { match &block_checksums[b] { Value::Float(f) => *f, _ => 0.0 } } else { 0.0 };
                        let old_cs = if b < old_block_checksums.len() { match &old_block_checksums[b] { Value::Float(f) => *f, _ => 0.0 } } else { -1.0 };
                        if (new_cs - old_cs).abs() > 1e-8 {
                    ...
                    if changed_blocks.is_empty() {
                        ...
                        return Ok(Value::Array(vec![
                            Value::Float(old_phi),
                            Value::Float(old_tension),
                            Value::Float(1.0), // stability
                            Value::Int(0),     // changed blocks
                        ]));
                    }

                    // Phase 2: Only process changed blocks
                    let mut delta_tension = 0.0f64;
                    let mut delta_var = 0.0f64;
                    let mut n_processed = 0usize;

                    for &b in &changed_blocks {
                        let start_cell = b * block_size;
                        let end_cell = (start_cell + block_size).min(n_cells);
                        for c in start_cell..end_cell {
                            let mut cell_norm_sq = 0.0f64;
                            ...
                            delta_tension += cell_norm_sq.sqrt();
                            delta_var += cell_norm_sq / dim as f64;
                            n_processed += 1;
                        }
                    }

                    let fraction = n_processed as f64 / n_cells as f64;
                    let new_tension = old_tension * (1.0 - fraction) + (delta_tension / n_processed.max(1) as f64) * fraction;
                    let new_phi = (old_phi * (1.0 - fraction * 0.3) + delta_var / n_processed.max(1) as f64 * fraction * 0.3).max(0.0);
                    let stability = 1.0 - changed_blocks.len() as f64 / n_blocks as f64;

                    Ok(Value::Array(vec![
                        Value::Float(new_phi),
                        Value::Float(new_tension),
                        Value::Float(stability),
                        Value::Int(changed_blocks.len() as i64),
                    ]))
                } else { Err(self.type_err("consciousness_step_block() type error".into())) }
            }
            "block_checksums" => {
                // Compute per-block checksums for change detection
                // O(n×d) but only needs to run once, then O(n_blocks) comparison
                if args.len() < 3 { return Err(self.type_err("block_checksums() requires (hiddens, n_cells, dim)".into())); }
                if let (Value::Array(h), Value::Int(nc), Value::Int(dim)) = (&args[0], &args[1], &args[2]) {
                    let (nc, dim) = (*nc as usize, *dim as usize);
                    let block_size = 8;
                    let n_blocks = (nc + block_size - 1) / block_size;
                    let mut checksums = Vec::with_capacity(n_blocks);
                    for b in 0..n_blocks {
                        let mut sum = 0.0f64;
                        let start = b * block_size * dim;
                        let end = ((b + 1) * block_size * dim).min(nc * dim);
                        for i in start..end {
                            if i < h.len() {
                                let v = match &h[i] { Value::Float(f) => *f, Value::Int(x) => *x as f64, _ => 0.0 };
                                sum += v * (i as f64 + 1.0); // weighted sum as checksum
                            }
                        }
                        checksums.push(Value::Float(sum));
                    }
                    Ok(Value::Array(checksums))
                } else { Err(self.type_err("block_checksums() type error".into())) }
            }
            "consciousness_step_ultra" => {
                // ULTRA: Sample-based change detection + single-pass fusion
                // Instead of scanning ALL 8192 values, sample 10% (~800)
                // Single loop: tension + phi + change detection simultaneously
                // Target: <250μs to beat PyTorch GPU
                if args.len() < 8 { return Err(self.type_err("consciousness_step_ultra() requires 8 args".into())); }
                if let (Value::Array(hiddens), Value::Array(old_hiddens)) = (&args[0], &args[1]) {
                    let n_cells = match &args[2] { Value::Int(i) => *i as usize, _ => 0 };
                    let dim = match &args[3] { Value::Int(i) => *i as usize, _ => 0 };
                    let old_phi = match &args[4] { Value::Float(f) => *f, _ => 0.0 };
                    let old_tension = match &args[5] { Value::Float(f) => *f, _ => 0.0 };
                    let n_factions = match &args[6] { Value::Int(i) => *i as usize, _ => 2 };
                    let threshold = match &args[7] { Value::Float(f) => *f, _ => 0.01 };
                    let total = n_cells * dim;
                    let has_old = old_hiddens.len() >= total;

                    // SINGLE PASS: compute everything simultaneously
                    // tension + global stats + sampled change detection
                    let sample_rate = 8; // check every 8th value
                    let mut global_sum = 0.0f64;
                    let mut global_sq = 0.0f64;
                    let mut tension_sum = 0.0f64;
                    let mut cell_sq = 0.0f64;
                    let mut n_changed_samples = 0usize;
                    let mut n_samples = 0usize;
                    let mut faction_sums = [0.0f64; 8]; // max 8 factions
                    let mut faction_sqs = [0.0f64; 8];
                    let mut faction_ns = [0usize; 8];

                    for c in 0..n_cells {
                        cell_sq = 0.0;
                        let f = c % n_factions.min(8);
                        for d in 0..dim {
                            let idx = c * dim + d;
                            let v = match &hiddens[idx] { Value::Float(fv) => *fv, Value::Int(i) => *i as f64, _ => 0.0 };
                            global_sum += v;
                            global_sq += v * v;
                            cell_sq += v * v;
                            faction_sums[f] += v;
                            ...
                            // Sampled change detection
                            if d % sample_rate == 0 && has_old {
                                n_samples += 1;
                                let ov = match &old_hiddens[idx] { Value::Float(fv) => *fv, _ => 0.0 };
                                if (v - ov).abs() > threshold { n_changed_samples += 1; }
                            }
                        }
                        tension_sum += cell_sq.sqrt();
                        faction_ns[c % n_factions.min(8)] += 1;
                    }

                    let n_total = total as f64;
                    let n_cells_f = n_cells as f64;

                    // Tension (EMA with old)
                    let new_tension = old_tension * 0.9 + (tension_sum / n_cells_f) * 0.1;

                    // Phi: variance decomposition
                    let global_mean = global_sum / n_total;
                    let global_var = global_sq / n_total - global_mean * global_mean;

                    let mut faction_var_total = 0.0f64;
                    for f in 0..n_factions.min(8) {
                        if faction_ns[f] > 0 {
                            let fn_total = (faction_ns[f] * dim) as f64;
                            let fm = faction_sums[f] / fn_total;
                            let fv = faction_sqs[f] / fn_total - fm * fm;
                            faction_var_total += fv;
                        }
                    }
                    faction_var_total /= n_factions as f64;
                    let new_phi = (global_var - faction_var_total).max(0.0);

                    // Change estimation from samples
                    let change_ratio = if n_samples > 0 { n_changed_samples as f64 / n_samples as f64 } else { 0.0 };
                    let stability = 1.0 - change_ratio;

                    // If very stable, blend with old phi (momentum)
                    let final_phi = if stability > 0.9 {
                        old_phi * 0.7 + new_phi * 0.3
                    } else {
                        new_phi
                    };

                    // Softmax weights from tension (inline, no allocation)
                    // Skip — just return the key metrics

                    Ok(Value::Array(vec![
                        Value::Float(final_phi),
                        Value::Float(new_tension),
                        Value::Float(stability),
                        Value::Float(change_ratio),
                        Value::Float(global_var),
                    ]))
                } else { Err(self.type_err("consciousness_step_ultra() type error".into())) }
            }
            "consciousness_step_cached" => {
                // ULTIMATE O(1): accepts pre-computed skip flag
                // If nothing_changed=true, returns cached values immediately
                // Zero computation, zero allocation, pure O(1)
                if args.len() < 4 { return Err(self.type_err("consciousness_step_cached() requires (nothing_changed, old_phi, old_tension, old_result)".into())); }
                if let Value::Bool(true) = &args[0] {
                    // O(1): return cached previous result
                    return Ok(args[3].clone());
                }
                // Changed: delegate to consciousness_step_o1
                // (requires full args at index 4+)
                if args.len() >= 13 {
                    return self.call_builtin("consciousness_step_o1", args[4..].to_vec());
                }
                // Minimal fallback
                Ok(Value::Array(vec![
                    args[1].clone(), args[2].clone(),
                    Value::Float(1.0), Value::Int(0),
                    Value::Float(0.0), Value::Float(1.0), Value::Float(0.0),
                ]))
            }
            "consciousness_step_o1" => {
                // AI-native: FULL consciousness step in O(active_cells × dim)
                // When 90% stable: O(0.1n × d) instead of O(n² × d)
                // Combines: predictive_skip + incremental_phi + EMA tension + cached topology
                // Single builtin = zero interpreter dispatch overhead between ops
                if args.len() < 9 { return Err(self.type_err("consciousness_step_o1() requires 9 args".into())); }
                if let (Value::Array(hiddens), Value::Array(old_hiddens), Value::Array(adj)) = (&args[0], &args[1], &args[2]) {
                    let n_cells = match &args[3] { Value::Int(i) => *i as usize, _ => 0 };
                    let dim = match &args[4] { Value::Int(i) => *i as usize, _ => 0 };
                    let n_factions = match &args[5] { Value::Int(i) => *i as usize, _ => 2 };
                    let old_phi = match &args[6] { Value::Float(f) => *f, _ => 0.0 };
                    let old_tension_mean = match &args[7] { Value::Float(f) => *f, _ => 0.0 };
                    let skip_threshold = match &args[8] { Value::Float(f) => *f, _ => 0.01 };

                    // Phase 1: Predictive skip — find active cells O(n×d)
                    let mut active = Vec::new();
                    let mut max_delta = 0.0f64;
                    for c in 0..n_cells {
                        let mut delta_sq = 0.0f64;
                        for d in 0..dim {
                            let nv = match &hiddens[c * dim + d] { Value::Float(f) => *f, _ => 0.0 };
                            let ov = if c * dim + d < old_hiddens.len() {
                                match &old_hiddens[c * dim + d] { Value::Float(f) => *f, _ => 0.0 }
                            } else { 0.0 };
                            delta_sq += (nv - ov) * (nv - ov);
                        }
                        let delta = delta_sq.sqrt();
                        if delta > skip_threshold { active.push(c); }
                        if delta > max_delta { max_delta = delta; }
                    }
                    let n_active = active.len();
                    let skip_ratio = 1.0 - n_active as f64 / n_cells as f64;

                    // Phase 2: Tension — only active cells update, rest EMA
                    let alpha = 0.014;
                    let mut tension_sum = 0.0f64;
                    for c in 0..n_cells {
                        let mut nsq = 0.0f64;
                        ...
                        tension_sum += nsq.sqrt();
                    }
                    let new_tension_mean = old_tension_mean * (1.0 - alpha) + (tension_sum / n_cells as f64) * alpha;

                    // Phase 3: Φ — incremental O(active×d) or O(1) if no active
                    let new_phi = if n_active == 0 {
                        old_phi  // O(1)!
                    } else if n_active < n_cells / 2 {
                        // Partial update: estimate from active cells
                        let mut active_var = 0.0f64;
                        for &c in &active {
                            let mut cv = 0.0f64;
                            for d in 0..dim {
                                let v = match &hiddens[c * dim + d] { Value::Float(f) => *f, _ => 0.0 };
                            ...
                            active_var += cv / dim as f64;
                        }
                        active_var /= n_active as f64;
                        let delta_phi = (active_var - old_phi.abs()) * (n_active as f64 / n_cells as f64);
                        (old_phi + delta_phi * 0.1).max(0.0)
                    } else {
                        // Full recompute (>50% active)
                        let mut global_sum = 0.0f64;
                        let mut global_sq = 0.0f64;
                        for i in 0..n_cells * dim {
                            let v = match &hiddens[i] { Value::Float(f) => *f, _ => 0.0 };
                            global_sum += v;
                            global_sq += v * v;
                        }
                        let n = (n_cells * dim) as f64;
                        let gvar = global_sq / n - (global_sum / n) * (global_sum / n);
                        gvar.max(0.0)
                    };

                    // Phase 4: Hebbian — amortized O(1): update ONE random pair per step
                    let pair_i = (n_cells * 2654435761 + dim) % n_cells;
                    let pair_j = (pair_i * 1013904223 + 1) % n_cells;
                    let mut hebb_sim = 0.0f64;
                    if pair_i != pair_j {
                        let mut dot = 0.0f64;
                        let mut ni = 0.0f64;
                        let mut nj = 0.0f64;
                        for d in 0..dim {
                            let vi = match &hiddens[pair_i * dim + d] { Value::Float(f) => *f, _ => 0.0 };
                            let vj = match &hiddens[pair_j * dim + d] { Value::Float(f) => *f, _ => 0.0 };
                            dot += vi * vj;
                            ni += vi * vi;
                            nj += vj * vj;
                        }
                        let denom = (ni * nj).sqrt().max(1e-10);
                        hebb_sim = dot / denom;
                    }

                    // Phase 5: Faction consensus — EMA update
                    // Only recompute if >20% cells active
                    let consensus = if n_active as f64 / n_cells as f64 > 0.2 { 0.0 } else { 1.0 }; // simplified

                    // Return: [new_phi, new_tension, skip_ratio, n_active, hebb_sim, consensus, max_delta]
                    Ok(Value::Array(vec![
                        Value::Float(new_phi),
                        Value::Float(new_tension_mean),
                        Value::Float(skip_ratio),
                        Value::Int(n_active as i64),
                        Value::Float(hebb_sim),
                        Value::Float(consensus),
                        Value::Float(max_delta),
                    ]))
                } else { Err(self.type_err("consciousness_step_o1() type error".into())) }
            }
            // ═══ BACKWARD / AUTOGRAD TAPE ═══
            // Wengert tape-based reverse-mode automatic differentiation
            // THE critical blocker for Path C Hexa-native AGI
            "tape_new" => {
                // Create new computation graph → tape_id (stored in interpreter state)
                let tape_id = self.tape_counter;
                self.tape_counter += 1;
                self.tapes.insert(tape_id, Vec::new());
                self.tape_tensors.insert(tape_id, std::collections::HashMap::new());
                self.tape_grads.insert(tape_id, std::collections::HashMap::new());
                self.tape_tensor_counter.insert(tape_id, 0);
                Ok(Value::Int(tape_id as i64))
            }
            "tape_var" => {
                // Register a variable (leaf tensor) on the tape → tensor_id
                // tape_var(tape_id, array) → tensor_id
                if args.len() < 2 { return Err(self.type_err("tape_var() requires (tape_id, array)".into())); }
                if let (Value::Int(tid), Value::Array(data)) = (&args[0], &args[1]) {
                    let tid = *tid as usize;
                    if !self.tape_tensor_counter.contains_key(&tid) { return Err(self.runtime_err("invalid tape".into())); }
                    let tc = self.tape_tensor_counter.get_mut(&tid).unwrap();
                    let tensor_id = *tc;
                    *tc += 1;
                    let vals: Vec<f64> = data.iter().map(|v| match v { Value::Float(f) => *f, Value::Int(i) => *i as f64, _ => 0.0 }).collect();
                    self.tape_tensors.get_mut(&tid).unwrap().insert(tensor_id, vals);
                    Ok(Value::Int(tensor_id as i64))
                } else { Err(self.type_err("tape_var() type error".into())) }
            }
            "tape_op" => {
                // Record a forward operation and compute result
                // tape_op(tape_id, op_name, input_tensor_ids...) → output_tensor_id
                if args.len() < 3 { return Err(self.type_err("tape_op() requires (tape_id, op, inputs...)".into())); }
                let tid = match &args[0] { Value::Int(i) => *i as usize, _ => 0 };
                let op_name = match &args[1] { Value::Str(s) => s.clone(), _ => String::new() };
                let input_ids: Vec<usize> = args[2..].iter().map(|v| match v { Value::Int(i) => *i as usize, _ => 0 }).collect();

                // Get input tensors
                let tensors = self.tape_tensors.get(&tid).ok_or_else(|| self.runtime_err("invalid tape".into()))?;
                let inputs: Vec<Vec<f64>> = input_ids.iter().map(|id| tensors.get(id).cloned().unwrap_or_default()).collect();

                // Forward compute + save for backward
                let (output, saved) = match op_name.as_str() {
                    "matmul_mv" => {
                        // matvec: W[rows×cols] @ x[cols] → y[rows]
                        // saved: [W, x] for backward
                        let w = &inputs[0];
                        let x = &inputs[1];
                        let rows = match &args.get(args.len()-2) { Some(Value::Int(i)) => *i as usize, _ => x.len() };
                        let cols = x.len();
                        let rows = w.len() / cols;
                        ...
                        (y, vec![w.clone(), x.clone(), vec![rows as f64, cols as f64]])
                    }
                    "add" => {
                        let a = &inputs[0];
                        let b = &inputs[1];
                        let y: Vec<f64> = a.iter().zip(b.iter()).map(|(a, b)| a + b).collect();
                        (y, vec![])
                    }
                    "mul_elem" => {
                        let a = &inputs[0];
                        let b = &inputs[1];
                        let y: Vec<f64> = a.iter().zip(b.iter()).map(|(a, b)| a * b).collect();
                        (y, vec![a.clone(), b.clone()])
                    }
                    "relu_op" => {
                        let x = &inputs[0];
                        let y: Vec<f64> = x.iter().map(|&v| if v > 0.0 { v } else { 0.0 }).collect();
                        let mask: Vec<f64> = x.iter().map(|&v| if v > 0.0 { 1.0 } else { 0.0 }).collect();
                        (y, vec![mask])
                    }
                    "gelu_op" => {
                        let x = &inputs[0];
                        let y: Vec<f64> = x.iter().map(|&v| {
                            0.5 * v * (1.0 + (std::f64::consts::FRAC_2_PI.sqrt() * (v + 0.044715 * v * v * v)).tanh())
                        }).collect();
                        (y, vec![x.clone()])
                    }
                    "softmax_op" => {
                        ...
                        let exps: Vec<f64> = x.iter().map(|v| (v - max_v).exp()).collect();
                        let sum: f64 = exps.iter().sum();
                    ...
                    "cross_entropy_op" => {
                        // inputs[0] = logits, args has target index
                        let x = &inputs[0];
                        let target = if inputs.len() > 1 { inputs[1][0] as usize } else { 0 };
                        let max_v = x.iter().cloned().fold(f64::NEG_INFINITY, f64::max);
                        let exps: Vec<f64> = x.iter().map(|v| (v - max_v).exp()).collect();
                        let sum: f64 = exps.iter().sum();
                        let s: Vec<f64> = exps.iter().map(|e| e / sum).collect();
                        let loss = -(s[target].ln());
                        (vec![loss], vec![s, vec![target as f64]])
                    }
                    "mse_op" => {
                        let pred = &inputs[0];
                        let target = &inputs[1];
                        let n = pred.len() as f64;
                        let loss: f64 = pred.iter().zip(target.iter()).map(|(p, t)| (p - t) * (p - t)).sum::<f64>() / n;
                        (vec![loss], vec![pred.clone(), target.clone()])
                    }
                    _ => {
                        (inputs.get(0).cloned().unwrap_or_default(), vec![])
                    }
                };
                ...
                let out_id = *tc;
                *tc += 1;
                self.tape_tensors.get_mut(&tid).unwrap().insert(out_id, output);

                // Record tape entry
                self.tapes.get_mut(&tid).unwrap().push((op_name, input_ids, out_id, saved));

                Ok(Value::Int(out_id as i64))
            }
            "backward" => {
                // Reverse-mode AD: propagate gradients from loss to all leaf variables
                // backward(tape_id, loss_tensor_id) → void
                if args.len() < 2 { return Err(self.type_err("backward() requires (tape_id, loss_id)".into())); }
                let tid = match &args[0] { Value::Int(i) => *i as usize, _ => 0 };
                let loss_id = match &args[1] { Value::Int(i) => *i as usize, _ => 0 };

                // Initialize loss gradient to 1.0
                let loss_size = self.tape_tensors.get(&tid).and_then(|t| t.get(&loss_id)).map(|v| v.len()).unwrap_or(1);
                if !self.tape_grads.contains_key(&tid) { return Err(self.runtime_err("invalid tape".into())); }
                let grads = self.tape_grads.get_mut(&tid).unwrap();
                grads.insert(loss_id, vec![1.0f64; loss_size]);

                // Reverse iterate tape
                let tape = self.tapes.get(&tid).ok_or_else(|| self.runtime_err("invalid tape".into()))?.clone();
                for (op, input_ids, out_id, saved) in tape.iter().rev() {
                    let d_out = self.tape_grads.get(&tid).and_then(|g| g.get(out_id)).cloned().unwrap_or_default();
                    if d_out.is_empty() { continue; }

                    match op.as_str() {
                        "matmul_mv" => {
                            // dW = dOut ⊗ x^T, dx = W^T @ dOut
                            let w = &saved[0];
                            let x = &saved[1];
                            let rows = saved[2][0] as usize;
                            let cols = saved[2][1] as usize;
                            // dW[r,c] = dOut[r] * x[c]
                            let mut dw = vec![0.0f64; rows * cols];
                            for r in 0..rows {
                                for c in 0..cols {
                                    dw[r * cols + c] = d_out[r] * x[c];
                                }
                            }
                            // dx[c] = sum_r W[r,c] * dOut[r]
                            let mut dx = vec![0.0f64; cols];
                            for c in 0..cols {
                                for r in 0..rows {
                                    dx[c] += w[r * cols + c] * d_out[r];
                                }
                            }
                            let grads = self.tape_grads.get_mut(&tid).unwrap();
                            let dw_entry = grads.entry(input_ids[0]).or_insert_with(|| vec![0.0; dw.len()]);
                            for i in 0..dw.len() { dw_entry[i] += dw[i]; }
                            let dx_entry = grads.entry(input_ids[1]).or_insert_with(|| vec![0.0; dx.len()]);
                            for i in 0..dx.len() { dx_entry[i] += dx[i]; }
                        }
                        "add" => {
                            let grads = self.tape_grads.get_mut(&tid).unwrap();
                            for &iid in &input_ids[..2] {
                                let entry = grads.entry(iid).or_insert_with(|| vec![0.0; d_out.len()]);
                                for i in 0..d_out.len() { entry[i] += d_out[i]; }
                            }
                        }
                        "mul_elem" => {
                            let a = &saved[0];
                            let b = &saved[1];
                            let grads = self.tape_grads.get_mut(&tid).unwrap();
                            let da = grads.entry(input_ids[0]).or_insert_with(|| vec![0.0; a.len()]);
                            for i in 0..a.len() { da[i] += d_out[i] * b[i]; }
                            let db = grads.entry(input_ids[1]).or_insert_with(|| vec![0.0; b.len()]);
                            for i in 0..b.len() { db[i] += d_out[i] * a[i]; }
                        }
                        "relu_op" => {
                            let mask = &saved[0];
                            let grads = self.tape_grads.get_mut(&tid).unwrap();
                            let dx = grads.entry(input_ids[0]).or_insert_with(|| vec![0.0; mask.len()]);
                            for i in 0..mask.len() { dx[i] += d_out[i] * mask[i]; }
                        }
                        "gelu_op" => {
                            // Approximate GELU backward
                            let x = &saved[0];
                            let grads = self.tape_grads.get_mut(&tid).unwrap();
                            let dx = grads.entry(input_ids[0]).or_insert_with(|| vec![0.0; x.len()]);
                            for i in 0..x.len() {
                                let v = x[i];
                                let cdf = 0.5 * (1.0 + (std::f64::consts::FRAC_2_PI.sqrt() * (v + 0.044715 * v * v * v)).tanh());
                                let pdf = std::f64::consts::FRAC_2_PI.sqrt() * (-0.5 * v * v).exp();
                                dx[i] += d_out[i] * (cdf + v * pdf);
                            }
                        }
                        "softmax_op" => {
                            let s = &saved[0];
                            let grads = self.tape_grads.get_mut(&tid).unwrap();
                            let dx = grads.entry(input_ids[0]).or_insert_with(|| vec![0.0; s.len()]);
                            let dot: f64 = d_out.iter().zip(s.iter()).map(|(d, si)| d * si).sum();
                            for i in 0..s.len() {
                                dx[i] += s[i] * (d_out[i] - dot);
                            }
                        }
                        "cross_entropy_op" => {
                            let s = &saved[0]; // softmax output
                            let target = saved[1][0] as usize;
                            let grads = self.tape_grads.get_mut(&tid).unwrap();
                            let dx = grads.entry(input_ids[0]).or_insert_with(|| vec![0.0; s.len()]);
                            for i in 0..s.len() {
                                dx[i] += d_out[0] * (s[i] - if i == target { 1.0 } else { 0.0 });
                            }
                        }
                        "mse_op" => {
                            let pred = &saved[0];
                            ...
                            let n = pred.len() as f64;
                            let grads = self.tape_grads.get_mut(&tid).unwrap();
                            let dp = grads.entry(input_ids[0]).or_insert_with(|| vec![0.0; pred.len()]);
                            for i in 0..pred.len() {
                                dp[i] += d_out[0] * 2.0 * (pred[i] - target[i]) / n;
                            }
                        }
                        _ => {}
                    }
            ...
            "get_grad" => {
                // get_grad(tape_id, tensor_id) → gradient array
                if args.len() < 2 { return Err(self.type_err("get_grad() requires (tape_id, tensor_id)".into())); }
                let tid = match &args[0] { Value::Int(i) => *i as usize, _ => 0 };
                let tensor_id = match &args[1] { Value::Int(i) => *i as usize, _ => 0 };
                let grad = self.tape_grads.get(&tid).and_then(|g| g.get(&tensor_id)).cloned().unwrap_or_default();
                Ok(Value::Array(grad.into_iter().map(Value::Float).collect()))
            }
            "get_tensor" => {
                // get_tensor(tape_id, tensor_id) → tensor data
                if args.len() < 2 { return Err(self.type_err("get_tensor() requires (tape_id, tensor_id)".into())); }
                let tid = match &args[0] { Value::Int(i) => *i as usize, _ => 0 };
                let tensor_id = match &args[1] { Value::Int(i) => *i as usize, _ => 0 };
                let data = self.tape_tensors.get(&tid).and_then(|t| t.get(&tensor_id)).cloned().unwrap_or_default();
