...
| 7 | `@optimize` | DP → 슬라이딩 윈도우 (메모리 O(n)→O(1)) | ✅ M973 DP lookback detection |
...
| 13 | `@specialize` | 런타임 타입 프로파일 → 특화 코드 생성 | ✅ M962 literal-binop detection |
| 14 | `@predict` | 분기 패턴 학습 → 분기 제거/정렬 | 📋 |
| 15 | `@compact` | 데이터 구조 자동 압축 (SoA 변환) | ✅ M972 AoS field-on-index detection |
| 16 | `@lazy` | 불필요 계산 자동 지연 | ✅ M963 zero-arg+@pure safety + Rust thunk |

### Tier 4 — 수학적 등가 변환
| # | Attr | 설명 | 상태 |
|---|------|------|------|
| 17 | `@symbolic` | x*2 → x<<1, x/const → 곱셈+시프트 | ✅ M480 strength reduction |
| 18 | `@reduce` | 연립방정식 → 닫힌 해 직접 계산 | ✅ M964 accumulator fold detection |
| 19 | `@approximate` | 정밀도 내 근사 (fast inverse sqrt 류) | ✅ M971 expensive math detection |
| 20 | `@algebraic` | 교환/결합법칙 → 연산 순서 재배치 | ✅ M970 commutative/associative detection |

### Tier 5 — 자기진화 (특이점)
| # | Attr | 설명 | 상태 |
|---|------|------|------|
| 21 | `@evolve` | 실행 프로파일 → 함수 세대별 진화 (call count + timing via `evolve_stats()`) | ✅ |
| 22 | `@learn` | 입력 분포 학습 → 분기 예측 자체 수정 | 📋 |
| 23 | `@contract` | pre/post 조건 → 런타임 검증 자동 삽입/제거 | 📋 |
| 24 | `@profile` | 핫스팟 자동 감지 → 선택적 재컴파일 | 📋 |

### Tier 6 — Self-Format Injection (AI-native attr 메타)
| # | Attr | 설명 | 상태 |
|---|------|------|------|
| 25 | `self-format injection` | 프로젝트별 `.<attr>-rules.json` 로 양식·규칙·임계값 독립 주입 — 하드코딩된 섹션 리스트 완전 제거 | ✅ M-SF1 (HX11) |

**원리 (Tier 6 — AI-native attr 메타):**

```
전통: 섹션 리스트 하드코딩
   fn required_sections(doc_type) {
       if doc_type == "paper" { return ["WHY", "COMPARE", ...] }
       ...
   }
   → 프로젝트 A 가 다른 섹션을 원하면 → attr 자체를 fork 해야 함

AI-native (Tier 6):
   fn find_doc_rules(source) {
       # source 상위로 .<attr>-rules.json 탐색
       # 없으면 글로벌 fallback
   }
   fn required_sections(doc_type, rules) {
       return rules.rules[doc_type].sections  # 데이터 주도
   }
   → 동일 attr, 프로젝트마다 다른 양식. Zero-fork.
```

**적용 attr (2026-04-18 기준):**
...
- zero-fork — 동일 `self/attrs/*.hexa` 가 모든 프로젝트에서 재사용
- 선언적 ruleset — AI 가 JSON 분석으로 규칙 자동 제안 가능
- `_meta.inherit: false` (default) = 완전 독립, `inherit: true` = 글로벌 병합 (프로젝트 override 우선)

**강제 (HX11):**
신규 attr 구현 시 `find_<attr>_rules(source: string)` helper 필수. 하드코딩 섹션 리스트 감지 시 review 실패.
...
## M962~M964 — @specialize / @lazy / @reduce 감지 패스 (2026-04-09)

| # | 내용 | 위치 |
|---|------|------|
| M962 | **@specialize** 감지 — 파라미터 보유 fn에서 IntLit-binop 사이트 수집, zero-arg 경고 | `self/ai_native_pass.hexa` |
| M963 | **@lazy** 안전성 — zero-arg + @pure 강제, 위반 시 WARN | `self/ai_native_pass.hexa` |
| M964 | **@reduce** 감지 — `acc = acc + x` / `acc = acc * x` 누적기 카운트 → 병렬 fold 후보 | `self/ai_native_pass.hexa` |

**헬퍼 추가:** `expr_has_intlit_binop`, `count_binops_with_intlit` — 재귀 BinOp/UnaryOp/Call 순회로 리터럴 피연산자 계측.

**검증:** `self/test_ai_native_v2.hexa` — 32/32 PASS
- @specialize × 9 (mul/add/poly/shift/clamp edge)
- @lazy × 6 (pi/answer/compute/str + idempotent; Rust thunk force-eval 경로)
- @reduce × 11 (sum/product/sum_squares/count_odd/pow2 edge)
- @pure 조합 × 6

**의미:** 총 구현 벡터 7/24 — 추가 #13 @specialize, #16 @lazy, #18 @reduce. T3 티어(Tier-3 의미 힌트) 거의 완주 (@fuse/@specialize/@lazy 완료, 남은 T3: @predict, @compact).

### Session 2026-04-09b — v4 벡터 4종 (M970~M973)

**구현:** 4 미구현 벡터 패턴 감지 + 진단 추가 (43 tests PASS)

| M# | Attr | 내용 |
|-----|------|------|
| M970 | `@algebraic` | 교환/결합법칙 재배치 감지 — non-canonical commutative (IntLit left), associative chain constant folding |
| M971 | `@approximate` | expensive math 감지 — division, sqrt/pow/exp/log/trig calls, loop-interior division 2x weight |
| M972 | `@compact` | SoA 변환 감지 — arr[i].field (FieldAccess on Index) AoS 패턴 |
| M973 | `@optimize` | DP sliding-window 감지 — dp[i] = f(dp[i-1], dp[i-2]) lookback 패턴 in while loops |

**의미:** 총 구현 벡터 11/24 — 추가 #7 @optimize(sliding), #15 @compact, #19 @approximate, #20 @algebraic. T3/T4 완주 (@compact/@algebraic/@approximate 완료, 남은 T3: @predict only).
