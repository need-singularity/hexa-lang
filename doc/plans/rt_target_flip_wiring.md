...
`self/codegen_c2.hexa` 는 L2984-3047 등에서 리터럴 `"hexa_math_sqrt"` 문자열을 직접 emit. `tool/native_build.hexa:252` 도 `"hexa_str_concat"` 리터럴.

## 장벽 — codegen_c2.hexa 는 모노리스

```
// codegen_c2.hexa 는 use "..." 를 주석 취급 (per tool/flatten_imports.hexa:9)
```
...
`tool/rebuild_stage0.hexa` 또는 flatten 의존 그래프에 rt/*.hexa 를 포함시켜 transpile 된 C 에 `rt_math_sqrt_v` 등의 정의가 같이 들어가게 함. 현재는 runtime.c 만 링크됨.
