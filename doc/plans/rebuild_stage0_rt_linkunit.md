...
1. **Flatten phase** (`tool/rebuild_stage0.hexa:L657–756`):
...
2. **Transpile phase** (`tool/rebuild_stage0.hexa:L759–771`; `build_stage0.hexa:L208–209`):
...
3. **Compile phase** (`tool/rebuild_stage0.hexa:L804–820`; `build_stage0.hexa:L224–241`):
...
4. **Post-link** (`tool/rebuild_stage0.hexa:L872–883`; `build_stage0.hexa:L288–299`):
...
2. **Does flatten_imports resolve `use self::rt::math` correctly?** — Partial: `use "self/rt/math"` works, but `use self::` syntax (with `::`) may need parsing update. **Action:** Test `hexa tool/flatten_imports.hexa self/rt/math_abi.hexa /tmp/flat_test.hexa` to confirm.
