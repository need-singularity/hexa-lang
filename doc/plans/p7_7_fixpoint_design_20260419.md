...
Chain (matches `tool/fixpoint_v3_v4.hexa` and `tool/verify_fixpoint.hexa`):
...
| Fixpoint check | `tool/fixpoint_v3_v4.hexa` | v3==v4 gate | landed, never green-logged |
...
ROI #153 is marked `done` in `doc/roi.json` at 2026-04-19T17:28, target `self/tools/fixpoint_v3_v4.hexa` (actual path is `tool/fixpoint_v3_v4.hexa`). The tool exists; the **closure** (green run logged as an invariant) does not.

## 3. Gap analysis — what blocks v3==v4

Three classes of non-determinism exist today:

1. **Mach-O LC_UUID** — clang embeds a fresh UUID per link. `fixpoint_v3_v4.hexa` prints v3/v4 UUID but doesn't neutralise it. On Darwin this is the typical first delta.
...
Current `fixpoint_v3_v4.hexa` only **reports** these; it does not **enforce** determinism.

## 4. Implementation plan (design only — do not edit yet)

### Phase P0 — Baseline capture (no code change, 30 min)
1. Run `./hexa tool/fixpoint_v3_v4.hexa KEEP=1` once from clean cache.
...
1. **Option A (link flag):** add `-Wl,-no_uuid` to the Darwin `ldflags` branch in `fixpoint_v3_v4.hexa` L177. Reruns will emit zero-UUID Mach-O. Minimal risk.
...
2. Run `./hexa tool/fixpoint_v3_v4.hexa FORCE_FLATTEN=1` twice back-to-back, cmp the two `flat.hexa` outputs: must be byte-identical.
...
2. On IDENTICAL result, `fixpoint_v3_v4.hexa` appends a JSON line:
   `{"ts":…, "v2_sha":…, "v3_sha":…, "v4_sha":…, "size":…, "identical":1}`.
3. Update ROI #153 note field with the green SHA triple + log pointer.
4. Rename target path in `doc/roi.json` from `self/tools/fixpoint_v3_v4.hexa` to the actual `tool/fixpoint_v3_v4.hexa` (one-char spec-reality drift).

### Phase P4 — CI gate (optional, 1h)
1. Add `tool/fixpoint_v3_v4.hexa` to `.claude/hooks/pre-commit`-eligible invariants (run on tag only, not per-commit — takes minutes).
...
$ KEEP=1 ./hexa tool/fixpoint_v3_v4.hexa
# observe:
...
| R2 | ROI #152 IC-slot regression reappears on full `hexa_full.hexa` transpile | LOW (marked fixed fcf275f4) | v3!=v4 by construction | keep `tool/p7_7_fixpoint_check.hexa --ic-scan` in the harness |
...
1. `tool/fixpoint_v3_v4.hexa` exits 0 three consecutive sessions.
...
- `tool/fixpoint_v3_v4.hexa` — the actual gate (285 LOC, lands 2026-04-19)
- `tool/verify_fixpoint.hexa` — P7-9 3-stage SHA variant (hexa_2==hexa_3)
- `tool/p7_7_fixpoint_check.hexa` — IC-slot scanner scaffold (ROI #152 guard)
