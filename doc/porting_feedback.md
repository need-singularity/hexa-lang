...
- **해결**: `self/core/nan_sentinel.hexa` — sentinel 기반 NaN/Inf 표현
...
- **해결**: `self/core/str_utils.hexa` — `chars(s)`, `char_at(s, i)` helper
...
- N=10000 스캔 ~10초+ 문제
- **해결**: `self/core/sieve.hexa` — `sigma_sieve(N)` / `phi_sieve(N)` / `tau_sieve(N)` 빌트인

### I10. Set 타입 ✅
- 영향: `r_chain_dynamics`, `task1_*`
- **해결**: `self/core/set_emu.hexa` — Set emulation lib
...
### I12. String slicing ✅
- **해결**: `self/core/str_utils.hexa` — `slice(s, start, end)` helper
